...
function add_dept(dept_id,dept_name){
    ...
    if(TO_VAL.indexOf(','+dept_id+',')<0 && TO_VAL.indexOf(dept_id+',')!=0 && (parent_window.form1.<?php echo $toid;?>.value!='ALL_AREA'))
  {
...
<?php 
$msql->query( "select * from area  where del=0 order by ID" );
$add_all_str="";
...
<?php echo $add_all_str;?>
 parent_window.form1.<?php echo $toid; ?>.value="ALL_AREA";
parent.close();   
...
function add_all()
{
  if(parent_window.form1.<?php echo $toid; ?>.value!="ALL_AREA")
  {
      }
  parent_window.form1.<?php echo $toid; ?>.value="ALL_AREA";
  parent.close();  
