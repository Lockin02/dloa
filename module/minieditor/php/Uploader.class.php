...
class Uploader
{
    ...
    public function __construct( $fileField , $config , $base64 = false )
    {
		...
		$this->homePath="../../../attachment/ueditor/".$_SESSION['USER_ID'].'/';
		if ( !file_exists( $this->homePath ) ) {
            if ( !mkdir( $this->homePath , 0777 , true ) ) {
                return false;
            }
        }
        $this->config = $config;
    ...
    private function upFile( $base64 )
    {
        ...
        if ( $this->stateInfo == $this->stateMap[ 0 ] ) {
            if ( !move_uploaded_file( $file[ "tmp_name" ] , $this->homePath.$this->fullName ) ) {
    ...
    private function base64ToImage( $base64Data )
    {
        ...
        if ( !file_put_contents($this->homePath.$this->fullName , $img ) ) {
    ...
    private function getFolder()
    {
        ...
        //$pathStr .= date( "Y" );
        if ( !file_exists( $this->homePath.$pathStr ) ) {
            if ( !mkdir( $this->homePath.$pathStr , 0777 , true ) ) {
                return false;
            }
        }
        return $pathStr;
