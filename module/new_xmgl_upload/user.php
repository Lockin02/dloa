...
@mkdir( "../../attachment/xmgl/".$dirs."/$mod", 511 );
if ( $flag == "Addnew" && $attachfile != "" )
{
    $temp = $_FILES['attachfile'];
    $SourceFileName = $temp['tmp_name'];
    $FileName = $temp['name'];
    $msql->query( "select * from xmgl_uploadlist  where FLOW_ID='".$FLOW_ID."' and Filename='$FileName' and PRCS_ID='$mod'" );
    if ( $msql->next_record( ) )
    {
        succ( "请不要重复添加上传文件", "user.php?FLOW_ID=".$FLOW_ID );
    }
    else
    {
        $fsql->query( "insert into xmgl_uploadlist values('0','".$FLOW_ID."','$mod','$FileName','$USER_ID')" );
        @copy( $SourceFileName, "../../attachment/xmgl/".$dirs."/$mod/$FileName" );
    }
}
if ( $flag == "deleted" )
{
    ...
    @unlink( "../../attachment/xmgl/".$dirs."/$mod/$delfile" );
...
$msql->query( "select * from xmgl_uploadlist  where FLOW_ID='".$FLOW_ID."' and Write_code='$USER_ID' and PRCS_ID='$mod' order by ID" );
...
echo "<tr class='TableContent'>\r\n  <td class='menulines' onclick=\"javascript:add_all();\" style=\"cursor:hand\" align='center'><b>上传文件</b></td>\r\n</tr>\r\n<thead class=\"TableControl\">\r\n  <th bgcolor=\"#d6e7ef\" colspan=\"2\"><b>文件列表</b></th>\r\n</thead>\r\n</table>\r\n<input type=\"hidden\" name=\"flag\">\r\n<input type=\"hidden\" name=\"code\">\r\n<input type=\"hidden\" name=\"mcode\">  \r\n<input type=\"hidden\" name=\"fcode\">   \r\n</form>\r\n\r\n<script Language=\"JavaScript\">\r\nfunction add_all()\r\n{\r\n";
echo $add_all_str;
echo "  \r\n\r\n}\r\n</script>\r\n\r\n</body>\r\n</html>\r\n";
?>
