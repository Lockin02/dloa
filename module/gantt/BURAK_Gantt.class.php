...
//		$this->colors["day"] = "FFFFFF";
		...
		$this->colors["line"] = "696969";
		$this->colors["group"] = "323232";
		$this->colors["holiday"] = "FEE5C8";
		$this->data_weekend = array(0,6);
		$this->data_count = array("G"=>0,"T"=>0,"M"=>0);
		$this->grid = "AUTO";
		$this->charLocal =  file_exists('SIMKAI.TTF') ? 'SIMKAI.TTF' : WEB_TOR.'module/gantt/SIMKAI.TTF';
		$this->monthCN = array('01'=>'一月','02'=>'二月','03'=>'三月','04'=>'四月','05'=>'五月','06'=>'六月','07'=>'七月','08'=>'八月','09'=>'九月','10'=>'十月','11'=>'十一月','12'=>'十二月');
	}
	...
	*/
	function addGroup($id,$start,$end = null ,$label = null ,$gid=null){
		$this->data_count["G"]++;

		$s = strtotime($start);
		$e = strtotime($end)+86400;
...
//		print_r($this->data_gantt);
	...
	*/
	function createCanvas(){
		...
		// 计算甘特图宽度
		$this->gantt_width = ((($this->gantt_end - $this->gantt_start) / 86400) * $this->inc_x) + $this->inc_x +1;
		// 创建图形
...
//			$label = gmstrftime("%Y %m",$s);
			$label = gmstrftime("%Y",$s);
			$label .= " ".$this->monthCN[gmstrftime("%m",$s)];
...
//				imagestring($this->im,2,$x_current+5,$y_current+3,$k,$this->colors["font"]);
				imagettftext($this->im , 9 , 0 ,$x_current+5, $y_current+16, $this->colors["font"] , $this->charLocal , $this->createText($k));
				if($this->n > 5){
...
//		imagestring($this->im,2,($pos["x1"]+5),($pos["y1"]-$this->heights["task"]-3),$d,$this->colors["font"]);
		imagettftext($this->im , 10 , 0 ,($pos["x1"]+3),  ($pos["y1"]-2), $this->colors["font"] , $this->charLocal , $this->createText($d));
		// top left
...
//		imagestring($this->im,2,($pos["x1"]+5),($pos["y1"]-$this->heights["task"]-3),$d,$this->colors["font"]);
	...
	*/
	function calX($start){
		return (floor(($start - $this->gantt_start)/86400)+1) * $this->inc_x;
	}
