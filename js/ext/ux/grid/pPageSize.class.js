...
Ext.ux.grid.pPageSize = function(config) {
	Ext.apply(this, config);
};
Ext.extend(Ext.ux.grid.pPageSize, Ext.util.Observable, {
			/**
			 * 
			...
			setPageSize : function(value, forced) {
				...
				value = parseInt(value) || parseInt(this.combo.getRawValue());// 不仅支持选择，而且支持输入
...
//						pt.afterTextEl.el.innerHTML = String.format(
//								pt.afterPageText, d.pages);
//						pt.field.dom.value = ap;
					...
					this.pagingToolbar.pageSize = value;
					this.pagingToolbar.doLoad(Math
							.floor(this.pagingToolbar.cursor
			...
			onRender : function() {
				...
				this.combo.on('blur', this.setPageSize, this);
				this.combo.on('keydown', function(t, e) {
							if (e.getKey() == 13) {
								this.setPageSize(t.getRawValue());
							}
						}, this);
				this.updateStore();
				if (this.addBefore) {
