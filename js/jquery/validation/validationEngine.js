...
(function ($) {
    ...
    $.fn.validationEngine = function (settings) {
        ...
        $(this).bind("submit", function (caller) {   // ON FORM SUBMIT, CONTROL AJAX FUNCTION IF SPECIFIED ON DOCUMENT READY
            ...
            //add by chengl 2012-03-17
            if (settings.disableButton) {
                $("input[type='button']").attr("disabled", "disabled");
                $("input[type='submit']").attr("disabled", "disabled");
            }

            if (settings.afterSubmitFn) {
                setting.afterSubmitFn();
            }
        })
    };
    $.validationEngine = {
        ...
        loadValidation: function (caller) {		// GET VALIDATIONS TO BE EXECUTED
            ...
            if (getRules) {		//2012年7月9日 19:27:43  国梁添加
                str = getRules[1];
                pattern = /\[|,|\]/;
        ...
        buildPrompt: function (caller, promptText, type, ajaxed) {			// ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
            ...
            //update by chengl 2012-01-05 定时移除提示
            setTimeout(function () {
                $(divFormError).animate({"opacity": 0.87}, function () {
                    if ($(this).attr("class").indexOf("ajaxed") == -1) {
                        $(this).remove();
                    }
                });
            }, 5000);
