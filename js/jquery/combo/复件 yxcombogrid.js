...
(function($) {
	$.woo.yxcombo.subclass('woo.yxcombogrid', {
		...
		_create : function() {
			...
			// 修正位置偏移，还是有点问题
			// if ($(el).offset()) {
			// //$(t.clearButton).height($(el).height());
			// t.clearButton.offset({
			// top : $(el).offset().top,
		...
		create : function() {
			...
			var fn = function() {
				if ($(el).val() != "") {
					...
					var data = $.ajax({
								url : '?model=' + p.gridOptions.model
										+ '&action=' + p.focusoutCheckAction,
								data : param,
								async : false
							}).responseText;
					if (data == 0) {
			...
			p.gridOptions.event.afterload = function(e, data) {
				...
				if (p.isFocusoutCheck) {
					...
					$(el).focusout(function(e) {
						...
						var cl = $(t.container).offset().left;
						var cw = $(t.container).width();
						var ct = $(t.container).offset().top;
						var ch = $(t.container).height();
						var w = cl + cw;
						var h = ct + ch + 20;
						if (e.pageX > w || e.pageX < cl || e.pageY > h
								|| e.pageY < ct) {
							// var idVal=$("#" + p.hiddenId).val();
							fn();
						} else {
							// 只执行一次
			...
			// 自动选择第一条数据
			// p.gridOptions.event.row_success = function(e, data) {
			// t.grid.selectFirstRow();
			// }
