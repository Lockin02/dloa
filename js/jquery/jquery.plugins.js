...
(function($) {
	...
	function addTab(param) {
		...
		// update by chengl 2012-05-29 增加延迟加载
		var html = '<div  id="tabs_' + param.id
				+ '" ><a href="javascript:;" id="tabs_link_' + param.id
				+ '" class="tab" index="' + param.id + '" closable="'
				+ param.closable + '" hidefocus="hidefocus">' + param.title
				+ '</a>';
		if (param.closable)
		...
		html += '</div>';
		var $h = $(html);
		$h.appendTo(tabsContainer);
		var content = param.content;
		if (param.isLoad === false) {
			content = "";
		}
		var $tabConetnt = $('<div id="tabs_' + param.id
				+ '_panel" class="tabs-panel" style="' + param.style + '">'
				+ content + '</div>');
		$tabConetnt.data("content", param.content);
		$tabConetnt.appendTo(panelsContainer);
	...
	function closeTab(id) {
		...
		// add by chengl 2012-05-29 关闭菜单删除记忆信息
		$.post('settabs.php', {
					systemFlag : 'del',
					parentId : id
				}, function(data) {
	...
	function selectTab(id) {
		//add by chengl 2012-05-29
		var t = $("#tabs_" + id + "_panel");
		if (!t.html()) {
	...
	$.fn.extend({
		jscroll : function(j) {
			return this.each(function() {
				...
				var dw = $(_self).width(), dh = $(_self).height();// dh =
				// $(_self).height()-1;
				...
				if ($(_self).children(".jscroll-c").height() == null) {// 存在性检测
					...
					// $(_self).children(".jscroll-c").prepend("<div
				...
				jscrollu.hover(function() {
							if (Isup == 0)
								$(this).addClass('jscroll-u-hover')
						}, function() {
							if (Isup == 0)
								$(this).removeClass('jscroll-u-hover')
						})
				jscrolld.hover(function() {
							if (Isup == 0)
								$(this).addClass('jscroll-d-hover')
						}, function() {
							if (Isup == 0)
								$(this).removeClass('jscroll-d-hover')
						})
