	...
	$.woo.component.subclass('woo.draggable',{
		...
		options:{
			...
			*/
			...
			cursor: "auto",
			/**
			 *设置的偏移量相对于拖动辅助鼠标光标，坐标可以给出一个使用一个或两个组合键的哈希：{上，左，右，下}
			 * @type {Boolean}
			 */
			cursorAt: false,
			/**
			 *拖动辅助捕捉到一个网格，每个像素的X和Y。数组值：[x,y]
			 * @type {Boolean}
			 */
			grid: false,
			/**
			...
			handle: false,
			/**
			 *允许一个辅助元素用于拖动显示。可能的值：'original'，'clone'，Function。如果指定一个函数，它必须返回一个dom结构
			 * @type {String,Function}
			 */
			helper: "original",
			/**
			 *捕捉拖动过程中防止iframe的MouseMove事件
			 * @type {Boolean}
			 */
			iframeFix: false,
			/**
			...
			opacity: false,
			/**
			 *如果设置为true，所有投放是根据每一个MouseMove事件来计算的。注意：会降低性能。
			 * @type {Boolean}
			 */
			refreshPositions: false,
			/**
			 *如果设置为true，则该元素回到其起始位置时，拖动停止
			 * @type {Boolean}
			 */
			revert: false,
			/**
			...
			scrollSpeed: 20,
			/**
			 *如果设置选择为真，可拖动将捕捉到选定元素的边缘时，粘贴到的元素的边缘。
			 * @type {Boolean}
			 */
			snap: false,
			/**
			 *确定单元元素的哪些边将被捕捉到。如果忽略模式则设置为false。可能的值：'inner'，'outer'，'both'
			 * @type {String}
			 */
			snapMode: "both",
			snapTolerance: 20,
			/**
			 *控制设置了z-index属性的元素指向选择器
			 * @type {Boolean}
			 */
			stack: false,
			/**
			 *是否设置拖动时候辅助目标的z-index属性
			 * @type {Boolean}
			 */
			zIndex: false
		},
