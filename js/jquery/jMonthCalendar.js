	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		do {
			...
			for (var i=0; i<7; i++) {
				...
				//DateBox Events
				var dateLink = jQuery('<div class="DateLabel"><a href="#">' + _currentDate.getDate() + '</a></div>');
/*
				.click(function(e) {
...
//alert(ev.Title)
						var link = jQuery('<a href=#>' + ev.Title + '</a>');
						...
						/**
						 * 以下为chengl 2010 10 12修改，加入数据过多显示更多链接及相关功能
						 */
						//event.hide();
						cell.append(event);
						var eventNum=cell.children().size()-1;
						if(eventNum>4){
							event.hide();
						}
						if(eventNum==5){
							var moreLink = jQuery('<a href="#" >更多...</a>');
							moreLink.bind('click', function() {
  								$(this).hide();
  								//同级兄弟显示
  								$(this).siblings().show();
  								var lessLink = jQuery('<a href="#" >关闭...</a>');
  								event.parent().append(lessLink);
  								lessLink.bind('click', function() {
  									//同级兄弟隐藏
  									$(this).siblings().hide();
  									//显示前五条数据
  									moreLink.prevAll().show();
	  								//显示更多
	  								moreLink.show();
	  								$(this).remove();
								});
