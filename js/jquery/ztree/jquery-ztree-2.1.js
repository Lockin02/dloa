...
(function($) {
	...
	//yxtree构造函数
	$.fn.yxtree = function(yxtreeSetting) {
		...
		if (setting.yxtreeNodes) {
			setting.root[setting.nodesCol] = setting.yxtreeNodes;
		}
		...
		return new yxtreePlugin().init(this);
	...
	function initTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0; i < treeNodes.length; i++) {
			...
			node.level = level;
			node.tId = setting.treeObjId + "_" + (++yxtreeId);
		...
		function(eventMouseDown) {
			...
			$(doc).mousemove(function(event) {
				...
				if (setting.dragStatus == 0) {
					...
					curNode = $("<ul class='yxtreeDragUL'></ul>").append(tmpNode);
	...
	function showIfameMask(showSign) {
		...
		if (showSign) {
			...
			for (var i = 0; i < iframeList.length; i++) {
				...
				var dragMask = $("<div id='yxtreeMask_" + i + "' class='yxtreeMask' style='top:" + r[1] + "px; left:" + r[0] + "px; width:" + obj.offsetWidth + "px; height:" + obj.offsetHeight + "px;'></div>");
	...
	function yxtreePlugin(){
		return {
			...
			getSetting : function() {
				var treeObjId = this.container.attr("id");
				if (!treeObjId) return;
				var yxtreeSetting = settings[treeObjId];
			...
			updateSetting : function(yxtreeSetting) {
				var treeObjId = this.container.attr("id");
				if (!treeObjId || !yxtreeSetting) return;
