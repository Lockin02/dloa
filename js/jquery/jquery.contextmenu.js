...
/*
 * -------------------------------------------------- 参数说明 option:
 * {width:Number, items:Array, onShow:Function, rule:JSON} 成员语法(三种形式) --
 * para.items -> {text:String, icon:String, type:String, alias:String,
 * width:Number, items:Array} -- 菜单组 -> {text:String, icon:String, type:String,
 * alias:String, action:Function } -- 菜单项 -> {type:String} -- 菜单分隔线
 * --------------------------------------------------
 */
(function($) {
	...
	$.fn.contextmenu = function(option) {
		...
		var ruleName = null, target = null, groups = {}, mitems = {}, actions = {}, showGroups = [], itemTpl = "<div class='b-m-$[type]' unselectable=on><nobr unselectable=on><img src='$[icon]' align='absmiddle'/><span style='color:black' unselectable=on>$[text]</span></nobr></div>";
		var gTemplet = $("<div/>").addClass("b-m-mpanel").attr("unselectable",
				"on").css("display", "none");
		...
		var buildGroup = function(obj) {
			obj.width = obj.width ? obj.width : 150;// 设置菜单组默认宽度150
			...
			$(this).width(obj.width).click(returnfalse).mousedown(returnfalse)
					.appendTo($("body"));
			obj = null;
			return this;
		};
		var buildItem = function(obj) {
			...
			T.innerHTML = itemTpl.replace(/\$\[([^\]]+)\]/g, function() {
						if (arguments[1] == 'icon' && !obj[arguments[1]]) {
							return "images/ico1.gif";// 默认菜单图片
						}
...
//					+ (width > 0 ? 25 : 0)) : pos.top;
		...
		// 隐藏菜单项
		function hideMenuItem(alias) {
			var item = mitems[alias];
			item.style.display = 'none';
		};
		// 显示菜单项
		function showMenuItem(alias) {
			var item = mitems[alias];
			item.style.display = '';
		};
		...
		function showMenu(e, menutarget) {
			...
			for (var i in mitems) {
				var fn = mitems[i].data.showMenuFn;
				if ($.isFunction(fn) && fn(target) == false) {
					hideMenuItem(i);
				} else {
					showMenuItem(i);
				}
		...
		var me = $(this).each(function() {
			return $(this).bind('contextmenu', function(e) {
				var bShowContext = (option.onContextMenu && $
						.isFunction(option.onContextMenu))
						? option.onContextMenu.call(this, e)
						: true;
				if (bShowContext) {
