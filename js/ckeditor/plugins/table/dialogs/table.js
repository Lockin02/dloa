...
(function() {
	...
	function c(e) {
		...
		g = 0;
		for (var h = 0,
		i, j = e.$.rows.length; h < j; h++) {
			i = e.$.rows[h],
			f = 0;
			for (var k = 0,
			l, m = i.cells.length; k < m; k++) {
				l = i.cells[k];
				f += l.colSpan;
			}
			f > g && (g = f);
	...
	function d(e, f) {
		...
		return {
			...
			onLoad: function() {
				var i = this,
				j = i.getContentElement('advanced', 'advStyles');
				if (j) j.on('change',
				function(k) {
					var l = this.getStyle('width', ''),
					m = i.getContentElement('info', 'txtWidth');
					m && m.setValue(l, true);
					var n = this.getStyle('height', ''),
					o = i.getContentElement('info', 'txtHeight');
					o && o.setValue(n, true);
				});
			},
			onShow: function() {
				...
				j = i.getRanges(),
				k = null,
				l = q.getContentElement('info', 'txtRows'),
				m = q.getContentElement('info', 'txtCols'),
				n = q.getContentElement('info', 'txtWidth'),
				o = q.getContentElement('info', 'txtHeight');
			...
			onOk: function() {
				var i = e.getSelection(),
				j = this._.selectedElement && i.createBookmarks(),
				k = this._.selectedElement || g('table'),
				l = this,
				m = {};
				...
				if (m.info) {
					...
					if (!this._.selectedElement) {
						var o = k.append(g('tbody')),
						p = parseInt(n.txtRows, 10) || 0,
						q = parseInt(n.txtCols, 10) || 0;
						for (var r = 0; r < p; r++) {
							var s = o.append(g('tr'));
							for (var t = 0; t < q; t++) {
								var u = s.append(g('td')).setAttribute('style','border-color : rgb(0, 0, 0)');
				...
				if (!this._.selectedElement) {
					e.insertElement(k);
					setTimeout(function() {
						var B = new CKEDITOR.dom.element(k.$.rows[0].cells[0]),
						C = new CKEDITOR.dom.range(e.document);
					...
					},
					{
						...
						children: [{
							...
							children: [{
								...
								'default': 500,
								getValue: a,
								validate: CKEDITOR.dialog.validate.cssLength(e.lang.common.invalidCssLength.replace('%1', e.lang.common.width)),
								onChange: function() {
									var i = this.getDialog().getContentElement('advanced', 'advStyles');
									i && i.updateStyle('width', this.getValue());
									i && i.updateStyle('border-color', 'rgb(0, 0, 0)');
								},
								setup: function(i) {
						...
						},
						{
							...
							children: [{
								...
								'default': '',
								getValue: a,
								validate: CKEDITOR.dialog.validate.cssLength(e.lang.common.invalidCssLength.replace('%1', e.lang.common.height)),
								onChange: function() {
									var i = this.getDialog().getContentElement('advanced', 'advStyles');
				...
				},
				{
					...
					children: [{
						...
						setup: function(i) {
							...
							if (j.count() > 0) {
								var k = j.getItem(0),
								l = k.getFirst(CKEDITOR.dom.walker.nodeType(CKEDITOR.NODE_ELEMENT));
