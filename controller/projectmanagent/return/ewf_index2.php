...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_contract_return";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_contract_return set ExaStatus = '完成',ExaDT = '".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_contract_return set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"销售退货申请";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_contract_return SET ExaStatus='部门审批' where id='$billId' ");
    $sendToURL="../../../index1.php?model=projectmanagent_return_return&action=toAdd";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=projectmanagent_return_return&action=init&perm=view&id=$billId";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=projectmanagent_return_return&action=toAuditNo";//处理审批后跳转的页面，跳转到待审批页面
