...
if ($actTo == "ewfSelect") {
    ...
    $passSqlCode = isset($passSqlCode) ? $passSqlCode : "update oa_borrow_equ_link set ExaStatus = '完成',ExaDT = now() where id='$billId' ";//审批完成后更新语句
    ...
    //变量定义
    $formName = isset($formName) ? $formName : "借试用发货物料变更";//工作流表单名称
...
if ($actTo == "ewfBuild") {
    ...
    $msql->query("update
        ...
        SET o1.ExaStatus='变更审批中',o2.ExaStatus='变更审批中' where o2.id ='$billId'");//更改原来单据为变更审批中状态
...
if ($actTo == "ewfExam") {
    ...
    $billId = isset($billId) ? $billId : "";//审批表单数据ID
    $detailUrl = isset($detailUrl) ? $detailUrl : "../../index1.php?model=contract_other_other&action=init&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId, $spid, $detailUrl);
}
//提交审批工作流
if ($actTo == "ewfExamSub") {
    $sendToURL = "../../../index1.php?model=contract_other_other";//处理审批后跳转的页面，跳转到待审批页面
...
if ($actTo == "delWork") {
    ...
    $examCode = isset($examCode) ? $examCode : "oa_borrow_equ_link";//审批数据表
    $formName = isset($formName) ? $formName : "借试用发货物料变更";//工作流表单名称
    $returnSta = isset($returnSta) ? $returnSta : "完成";//回滚单据审批状态
