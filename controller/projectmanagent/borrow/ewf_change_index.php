...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_borrow_borrow set ExaStatus='完成',changeTips=0,isSubAppChange=0,dealStatus=1 where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_borrow_borrow set ExaStatus = '打回',changeTips=0,isSubAppChange=0,dealStatus=1 where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    $ewf->setBillDept($billDept);
     //客户信息
    $sql="select c.typeone  , c.arealeaderid  from oa_borrow_borrow b 
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_borrow_borrow SET dealStatus = '1',ExaStatus='部门审批' where id='$billId' ");//提交审批后改变审批状态
    $msql->query(" update oa_borrow_borrow o1 left join oa_borrow_borrow o2 on o1.id=o2.originalId SET o1.dealStatus = '1',o1.needSalesConfirm=0,o1.salesConfirmId=0,o1.ExaStatus='变更审批中' where o2.id ='$billId'");//更改原来单据为变更审批中状态
    $msql->query("update oa_borrow_changlog set ExaStatus = '变更审批中' where objType = 'borrow' and tempId='$billId';");
