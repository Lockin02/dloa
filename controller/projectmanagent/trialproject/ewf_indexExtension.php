...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_trialproject_extension";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_trialproject_extension set ExaStatus = '完成',ExaDT = '".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_trialproject_extension set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //客户信息
    $sql="SELECT typeone , c.areaid  FROM oa_trialproject_extension e
left join oa_trialproject_trialproject  b on (b.id=e.trialprojectId)
...
where e.id='$billId' ";
    ...
    //变量定义
    $formName=isset($formName)?$formName:"试用项目延期申请";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_trialproject_extension SET ExaStatus='部门审批' where id='$billId' ");
    $msql->query(" update oa_trialproject_trialproject set serCon = '5' where id= '".$proSid."' ");
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=projectmanagent_trialproject_trialproject&action=init&perm=view&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=projectmanagent_trialproject_trialproject&action=toAuditNo";//处理审批后跳转的页面，跳转到待审批页面
