...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_trialproject_trialproject set status= 2,ExaStatus = '完成',turnStatus='未转正',ExaDT = '".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_trialproject_trialproject set ExaStatus = '打回',status = 0 where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
