...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_trialproject_trialproject";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_trialproject_trialproject set ExaStatus = '完成',ExaDT = '".date("Y-m-d")."',status = 2 where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_trialproject_trialproject set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //客户信息
    $sql="SELECT typeone  ,  r.exausercon  as arealeaderid , c.areaid FROM oa_trialproject_trialproject  b 
...
where b.id='$billId' ";
    ...
    //变量定义
    $formName=isset($formName)?$formName:"试用项目申请";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_trialproject_trialproject SET status = 1,ExaStatus='部门审批' where id='$billId' ");
