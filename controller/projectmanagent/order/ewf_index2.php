...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_sale_order set ExaStatus='完成',state = '2',ExaDT = now(),ExaDTOne = now() where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_sale_order set ExaStatus = '打回',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_sale_order SET ExaStatus='部门审批',state='1' where id='$billId' ");//提交审批后改变审批状态
    $sendToURL="../../../index1.php?model=projectmanagent_order_order&action=toAdd";//生成工作流后跳转的页面
...
if($actTo=="ewfExamSub"){
	$billId=isset($billId)?$billId:"";//审批表单数据ID
    $sendToURL="../../../index1.php?model=projectmanagent_order_order&action=configOrder&orderId=" + $billId;//处理审批后跳转的页面
