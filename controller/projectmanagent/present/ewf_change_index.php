...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_present_present";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_present_present set ExaStatus='完成',dealStatus='1',ExaDT = '".date("Y-m-d")."' where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_present_present set ExaStatus = '打回',ExaDT = '".date("Y-m-d")."' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //添加审批信息
    $sql="SELECT c.typeone , c.arealeaderid , c.areaid  FROM oa_present_present p
left join customer c on ( p.customernameid=c.id)
where p.id='".$billId."' ";
    $msql->query($sql);
    $msql->next_record();
    $cktype=$msql->f('typeone');
    $ewf->setBillUser($msql->f('arealeaderid'));//区域审批人
    $ewf->setCkType($cktype);//客户类型

    //变量定义
    $formName=isset($formName)?$formName:"赠送变更审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_present_present SET ExaStatus='部门审批',dealStatus = 1 where id='$billId' ");//提交审批后改变审批状态
    $msql->query(" update oa_present_present o1 left join oa_present_present o2 on o1.id=o2.originalId SET o1.ExaStatus='变更审批中',o1.dealStatus = 1 where o2.id ='$billId'");//更改原来单据为变更审批中状态
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=projectmanagent_present_present&action=toViewTab&id=$billId&perm=view&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
	$spid = $_POST ['spid'];
	$sendToURL = "../../../index1.php?model=projectmanagent_present_present&action=toMyBorrowList";
