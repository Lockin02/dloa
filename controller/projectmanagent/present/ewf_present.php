...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_present_present set ExaStatus = '完成',ExaDT = '".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
...
where p.id='".$billId."' ";
    ...
    //变量定义
    $formName=isset($formName)?$formName:"设备赠送";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_present_present SET ExaStatus='部门审批',dealStatus=1 where id='$billId' ");
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=projectmanagent_present_present&action=init&perm=view&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=projectmanagent_present_present&action=toAuditNo";//处理审批后跳转的页面，跳转到待审批页面
