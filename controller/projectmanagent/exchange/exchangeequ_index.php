...
if($actTo=="ewfSelect"){
    ...
    //变量定义
    $formName=isset($formName)?$formName:"换货物料确认审批";//工作流表单名称
...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_exchange_equ_link SET ExaStatus='部门审批' where id='$billId' ");//提交审批后改变审批状态
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=projectmanagent_exchange_exchangeequ&action=toEquView&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=projectmanagent_exchange_exchangeequ&action=shipments";//处理审批后跳转的页面
