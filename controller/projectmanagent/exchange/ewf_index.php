...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_contract_exchangeapply";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_contract_exchangeapply set ExaStatus='完成' where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_contract_exchangeapply set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"换货申请审批";//工作流表单名称
...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_contract_exchangeapply SET ExaStatus='部门审批',state='1' where id='$billId' ");//提交审批后改变审批状态
        $sendToURL="../../../index1.php?model=projectmanagent_exchange_exchange&action=toAdd";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=projectmanagent_exchange_exchange&action=init&perm=view&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=projectmanagent_exchange_exchange";//处理审批后跳转的页面
