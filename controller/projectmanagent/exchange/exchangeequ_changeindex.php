...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_exchange_equ_link";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_exchange_equ_link set ExaStatus='完成',ExaDT = now(),ExaDTOne = now() where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_exchange_equ_link set ExaStatus = '打回',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"换货物料确认变更审批";//工作流表单名称
...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_exchange_equ_link SET ExaStatus='变更审批中' where id='$billId' ");//提交审批后改变审批状态
    $msql->query(" update oa_exchange_equ_link o1 left join oa_exchange_equ_link o2 on o1.id=o2.originalId SET o1.ExaStatus='变更审批中' where o2.id ='$billId'");//更改原来单据为变更审批中状态
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=projectmanagent_exchange_exchangeequ&action=toEquEdit&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=projectmanagent_exchange_exchangeequlink&action=confirmChange&spid=$spid";//处理审批后跳转的页面
