...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_borrow_return_dispose";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_borrow_return_dispose set ExaStatus = '完成',ExaDT = '".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_borrow_return_dispose set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"借试用赔偿单确认";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_borrow_return_dispose SET ExaStatus='部门审批' where id='$billId' ");
