...
class controller_cost_stat_import extends model_cost_stat_import {
    ...
    function c_edit() {
        $id=$_GET['id'];
		if($id){
			$str=$this->getImportData($id);	
			
		}
		  $this->show->assign('list', $str);
		 $this->show->display('cost_stat_import_edit');		
    }
	function c_detail(){
		$id=$_GET['id'];
		if($id){
			$str=$this->getImportData($id,2);	
	...
	function c_del(){
		 echo 	$this->delImportData((int)$_GET['id']);			 	
	}
	
	function c_merger(){
		 echo 	$this->mergerImportData($_GET['id']);	
		
	}
	
	function c_bd(){
		$mid=$_GET['spid']?$_GET['spid']:$_GET['id'];
		if($_GET['spid']){
			$mid=$this->getbillNo($_GET['spid']);
		}
		if($mid){
			echo $this->billDocumentNew($mid);	
		}
	}
	...
	function c_deletes(){
		...
		if($id){
			$mid=$this->model_list_deletes($id);
			echo $mid; 
		}
 ...
 */
	...
	function c_changState2Apply() {
	    $id=(int)($_GET['id']?$_GET['id']:$_POST['id']);
	    $this->changState2Apply($id);
	}
