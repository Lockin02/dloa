 ...
 *
 */
class controller_common_contract_allsource extends controller_base_action {

	function __construct() {
		$this->objName = "allsource";
		$this->objPath = "common_contract";
		parent::__construct ();
	}
...
/*********************************************************************************/
     ...
     * 更新贝讯数据
     */
     function c_updateToBX(){
        $this->view("updateToBX");
     }
     //客户
     function c_customer_bx(){
     	$cusDao = new model_customer_customer_customer();
		$msg = $cusDao->updateCusBX();
     }
     //联系人
     function c_linkman_bx(){
     	$linkmanDao = new model_customer_linkman_linkman();
		$msg = $linkmanDao->updateLinkmanBX();
     }
     //商机
     function c_chance_bx(){
     	$chanceDao = new model_projectmanagent_chance_chance();
     	$msg = $chanceDao->updateChanceBX();
     }
...
//		print_R($service->searchArr);
		...
		foreach ( $rows as $key => $val ) {
			...
			if( $rows [$key] ['productId'] ){
				...
				$rows [$key] ['lockedNum'] = $inventoryDao->getExeNumsByStockType ( $rows [$key] ['productId'], $stockType,1 );
			}else{
				$rows [$key] ['exeNum']=0;
				$rows [$key] ['lockedNum']=0;
			}
		}
		$arr ['collection'] = $rows;
     ...
     function shwoBToOEqu($rows,$rowNum){
		...
		foreach($rows as $key => $val ){
				...
				$str .= <<<EOT
					<tr><td>$line
							<td>$val[productCode]
							<input type="hidden" id="productNo$j" readonly="true" name="outplan[productsdetailBo][$j][productNo]" value="$val[productCode]" class='readOnlyTxtItem' readonly/>
							<input type="hidden" id="BToOTips$j" name="outplan[productsdetailBo][$j][BToOTips]" value="1" class="txtmiddle"/></td>
							</td>
						<td>$val[productName]
							<input type="hidden" id="contEquId$j" name="outplan[productsdetailBo][$j][contEquId]" value="$val[id]"/>
							<input type="hidden" id="productId$j" name="outplan[productsdetailBo][$j][productId]" value="$val[productId]"/>
							<input type="hidden" id="productName$j" name="outplan[productsdetailBo][$j][productName]" value="$val[productName]" class='readOnlyTxtMiddle' readonly/></td>
						<td>$val[productModel]
							<input type="hidden" id="productModel$j" name="outplan[productsdetailBo][$j][productModel]" value="$val[productModel]" class='readOnlyTxtItem' readonly/></td>
						<td>$val[unitName]
							<input type="hidden" name="outplan[productsdetailBo][$j][unitName]" id="unitName$j" value="$val[unitName]" class='txtshort'/>
						</td>
						<td>
						...
						<td>$contRemain
							<input type="hidden" name="outplan[productsdetailBo][$j][number]" id="number$j" value="$contRemain" class='txtshort' onblur="checkThis($j)"/>
						</td>
						<td>
...
/**
	 * 获取产品下的物料信息(用于物料确认开始带出)
	 */
	function c_newGetProductEqu() {
		$deloy = $_POST['deloy'];
		$sourceType = $_GET['sourceType'];//源单类型：合同，借试用，赠送，换货
		$sourceType=empty($sourceType)?"contract":$sourceType;
		$service = $this->service;
		$equArr = $service->newGetProductEqu_d($deloy,$sourceType);
		$inventoryDao = new model_stock_inventoryinfo_inventoryinfo();
		if (is_array($equArr) && count($equArr) > 0) {
			foreach ($equArr as $key => $val) {
				$equArr[$key]['warrantyPeriod'] = $val['warranty'];
...
/***********************************************商机导入***********begin***************************************************/
	...
	function c_upExceltoChance() {
		...
		$objNameArr = array (
			...
			9 => 'winRate', //商机赢率
// 			10 => 'chanceStage', //商机阶段
			10 => 'predictContractDate', //预计合同签署日期
			11 => 'predictExeDate', //预计合同执行日期
			12 => 'contractPeriod', //合同执行周期
			13 => 'contractCode',//合同号
			14 => 'contractTurnDate', //转合同日期
			15 => 'progress',//项目进展描述
			16 => 'competitor'//项目竞争对手
		);
...
//                   	if(empty($SJLX) || empty($SJJD)){
                   if(empty($SJLX)){
                   	  array_push ( $arrinfo, array ("orderCode" => $val['chanceCode'],"cusName" => $val['prinvipalName'],"result" => "导入失败，商机类型错误" ) );
                   }else{
 ...
 * 手工 定时保存 销售三张报表数据
 */
	 function c_handleSaleViewTiming(){
	     $this->view('handleSaleViewTiming');
	 }
	 //保存数据
	function c_saveTimingData(){
		$objType=$_GET['objType'];
		try{
			$this->service->saveTimingData_d($objType);
			echo 1;
		}catch(Exception $e){
			echo $e->getMessage();
		}
	...
	function c_toUpdateObjCodeByContractCode(){
		$this->view("objCode");
	}

	function c_updateObjCodeByContractCode(){
		$r=$this->service->updateObjCodeByContractCode("oa_contract_contract",$_GET['contractCode']);
		echo $r;
	}
