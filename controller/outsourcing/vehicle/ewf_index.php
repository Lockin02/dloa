...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_outsourcing_rentalcar";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_outsourcing_rentalcar c set c.ExaStatus = '完成',c.ExaDT = now() ,c.state=2 where c.id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_outsourcing_rentalcar c set c.ExaStatus = '打回', c.state=3 where c.id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"租车申请审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    //$sendToURL=$baseDir."?actTo=swfList";
     $msql->query(" update oa_outsourcing_rentalcar c SET c.ExaStatus='部门审批', c.state=1 where c.id='$billId' ");
