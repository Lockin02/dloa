...
if ($actTo == "ewfSelect") {
    ...
    $examCode = isset($examCode) ? $examCode : "oa_outsourcing_apply";//审批数据表
    $passSqlCode = isset($passSqlCode) ? $passSqlCode : " update oa_outsourcing_apply set exaStatus = '完成',ExaDT = now(),state = 1 where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode = isset($disPassSqlCode) ? $disPassSqlCode : " update oa_outsourcing_apply set exaStatus = '打回',state = 0 where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName = isset($formName) ? $formName : "项目外包申请审批";//工作流表单名称
...
if ($actTo == "ewfBuild") {
    ...
    $msql->query(" update oa_outsourcing_apply SET exaStatus='部门审批',state = 3 where id='$billId' ");
