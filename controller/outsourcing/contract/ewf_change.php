...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_contract_rentcar";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_contract_rentcar set ExaStatus = '完成' ,ExaDT = now() ,status = 2 where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_contract_rentcar set ExaStatus = '打回' ,status = 0 ,ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"租车合同变更审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
	...
	$msql->query(" update oa_contract_rentcar o1 left join oa_contract_rentcar o2 on o1.id=o2.originalId SET o1.ExaStatus='变更审批中',o1.status = 4 where o2.id ='$billId'");//更改原来单据为变更审批中状态
    $msql->query(" update oa_contract_rentcar_changelog set ExaStatus = '变更审批中',ExaDT = now() where tempId ='$billId';");
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../index1.php?model=outsourcing_contract_rentcar&action=init&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=outsourcing_contract_rentcar&action=toAssetAuditTab";//处理审批后跳转的页面，跳转到待审批页面
