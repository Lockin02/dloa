...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_contract_rentcar c set c.ExaStatus = '完成',c.ExaDT = now() ,c.status=2 where c.id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_contract_rentcar c set c.ExaStatus = '打回' ,c.status=0 where c.id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"租车合同审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    //$sendToURL=$baseDir."?actTo=swfList";
     $msql->query(" update oa_contract_rentcar c SET c.ExaStatus='部门审批',c.status=1 where c.id='$billId' ");
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../index1.php?model=outsourcing_contract_rentcar&action=toView&id=$billId&skey=$skey";//审批查看内容
