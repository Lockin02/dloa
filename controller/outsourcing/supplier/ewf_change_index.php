...
/**
* 审批对象表单字段中必须含有 审批状态：changeExaStatus --varchar(15) 状态为： 编辑（新建时） 部门审批  完成 ; 审批日期 ：ExaDT --datetime
*/
...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_outsourcesupp_supplib";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_outsourcesupp_supplib set changeExaStatus = '完成',ExaStatus = '完成' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_outsourcesupp_supplib set changeExaStatus = '打回',ExaStatus = '完成' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"外包供应商变更审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    //$sendToURL=$baseDir."?actTo=swfList";
     $msql->query(" update oa_outsourcesupp_supplib SET changeExaStatus='部门审批',ExaStatus = '完成' where id='$billId' ");
