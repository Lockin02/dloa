...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_outsourcesupp_supplib set status=1, ExaStatus = '完成',ExaDT = now() where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_outsourcesupp_supplib set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"外包供应商审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    //$sendToURL=$baseDir."?actTo=swfList";
     $msql->query(" update oa_outsourcesupp_supplib SET ExaStatus='部门审批' where id='$billId' ");
