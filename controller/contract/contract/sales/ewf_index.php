...
if($actTo=="ewfSelect"){
    ...
    //变量定义
    $formName=isset($formName)?$formName:"销售合同审批";//工作流表单名称
...
//生成工作流
if($actTo=="ewfBuild"){
    $sendToURL="../../../index1.php?model=contract_sales_sales&action=myApplyTab";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=contract_sales_sales&action=readBaseInfoInEx&id=$billId";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=contract_sales_sales&action=toAuditTab";//处理审批后跳转的页面
