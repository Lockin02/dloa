...
if($actTo=="ewfBuild"){
    ...
    $sendToURL="../../../index1.php?model=contract_change_change&action=myChangeApplyAction";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=contract_change_change&action=showChangeAndNew&id=$billId";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=contract_change_change&action=contractChangeTab";//处理审批后跳转的页面
