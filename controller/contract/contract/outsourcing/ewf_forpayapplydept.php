...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_sale_outsourcing set ExaStatus = '完成',ExaDT = now(),status = 2 where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_sale_outsourcing set ExaStatus = '打回',status = 0,ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"外包合同立项付款申请";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_sale_outsourcing SET ExaStatus='部门审批',status = '1' where id='$billId' ");
    $sendToURL="../../../index1.php?model=contract_outsourcing_outsourcing&action=toAddDept";//生成工作流后跳转的页面
