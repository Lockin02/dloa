...
if ($actTo == "ewfSelect") {
	...
	$passSqlCode = isset ($passSqlCode) ? $passSqlCode : " update oa_contract_contract set ExaStatus='完成',state = '2',dealStatus='2' where id='$billId'"; //审批完成后更新语句
	$disPassSqlCode = isset ($disPassSqlCode) ? $disPassSqlCode : " update oa_contract_contract set ExaStatus = '打回' where id='$billId'"; //审批打回更新语句
	//特殊审批角色： 项目经理 -- 区域经理
...
left join oa_system_region r on (c.areacode = r.id )
 where c.areacode = r.id and  c.id='$billId' ";
	...
	//变量定义
	$formName = isset ($formName) ? $formName : "合同变更审批"; //工作流表单名称
...
if ($actTo == "ewfBuild") {
	...
	$msql->query(" update oa_contract_contract SET ExaStatus='部门审批' where id='$billId' "); //提交审批后改变审批状态
	$msql->query(" update oa_contract_contract o1 left join oa_contract_contract o2 on o1.id=o2.originalId SET o1.ExaStatus='变更审批中',o1.isBecome='1' where o2.id ='$billId'"); //更改原来单据为变更审批中状态
...
if ($actTo == "ewfExam") {
	...
	$billId = isset ($billId) ? $billId : ""; //审批表单数据ID
	$detailUrl = isset ($detailUrl) ? $detailUrl : "../../../index1.php?model=contract_contract_contract&action=init&id=$billId&perm=view&skey=$skey"; //审批查看内容
