...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_sale_lease";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_sale_lease set ExaStatus='完成',state = '2' where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_sale_lease set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"租赁合同审批";//工作流表单名称
...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_sale_lease o1 left join oa_sale_lease o2 on o1.id=o2.originalId SET o1.ExaStatus='变更审批中',o1.isBecome='1' where o2.id ='$billId'");//更改原来单据为变更审批中状态
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=contract_rental_rentalcontract&action=toViewTab&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
	$spid = $_POST ['spid'];
	$sendToURL = "../../../index1.php?model=contract_rental_rentalcontract&action=confirmChangeToApprovalNo&spid=$spid";
//    $sendToURL="../../../index1.php?model=contract_rental_rentalcontract&action=toApprovalNo";//处理审批后跳转的页面
