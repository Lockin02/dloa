...
if($actTo=="ewfSelect"){
    ...
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_contract_contract set ExaStatus = '打回',ExaDT = now(),state = '0',isSubApp = '0',engConfirm = '0',engConfirm = '0',saleConfirm='0',rdproConfirm='0' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
...
left join oa_system_region r on (c.areacode = r.id )
 where c.areacode = r.id and  c.id='$billId' ";
    ...
    //变量定义
    $formName=isset($formName)?$formName:"合同审批B";//工作流表单名称
...
if($actTo=="ewfBuild"){
	$msql->query(" update oa_contract_contract SET ExaStatus='部门审批',state='1' where id='$billId' ");//提交审批后改变审批状态
        $sendToURL="../../../index1.php?model=contract_contract_contract&action=toAdd";//生成工作流后跳转的页面
