...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_contract_contract set ExaStatus='完成',state = '7' where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_contract_contract set ExaStatus = '完成' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
...
left join oa_system_region r on (c.areacode = r.id )
 where c.areacode = r.id and  c.id='$billId' ";
    ...
    //变量定义
    $formName=isset($formName)?$formName:"合同异常关闭";//工作流表单名称
...
//    $msql->query(" update oa_contract_contract SET ExaStatus='变更审批中' where id='$billId' ");//提交审批后改变审批状态
    $sendToURL="../../../index1.php?model=contract_contract_contract&action=closeAppEdit&id=$billId";//生成工作流后跳转的页面
