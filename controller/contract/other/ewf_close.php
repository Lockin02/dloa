...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_sale_other set ExaStatus = '完成',status = '3' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_sale_other set ExaStatus = '完成',status = '2' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"其他合同关闭审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
	...
	$msql->query(" update oa_sale_other SET status='5' where id ='$billId'");//更改原来单据为变更审批中状态
...
if($actTo=="delWork"){
    ...
    $sql="update oa_sale_other set status = '2' where id='$billId'";
    $examCode=isset($examCode)?$examCode:"oa_sale_other";//审批数据表
    $formName=isset($formName)?$formName:"其他合同关闭审批";//工作流表单名称
