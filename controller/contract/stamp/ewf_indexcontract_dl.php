...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_sale_stampapply";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_sale_stampapply set ExaStatus = '完成',ExaDT = now() where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_sale_stampapply set ExaStatus = '打回',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"合同盖章申请审批";//工作流表单名称
