...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_sale_outsourcing";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_sale_outsourcing set ExaStatus = '完成',status = '3',ExaDT = now() where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_sale_outsourcing set ExaStatus = '完成',status = '2',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"外包合同关闭审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
	...
	$msql->query("update oa_sale_outsourcing SET status='5' where id ='$billId'");//更改原来单据为合同关闭审批中状态
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../index1.php?model=contract_outsourcing_outsourcing&action=viewAlong&id=$billId&skey=$skey";//审批查看内容
