...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_hr_certifyapplyassess";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_hr_certifyapplyassess set ExaStatus ='完成',ExaDT = '".date("Y-m-d")."',status = 3 where ID='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_hr_certifyapplyassess set ExaStatus ='打回',ExaDT = '".date("Y-m-d")."',status = 1  where ID='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"任职资格认证审批";//工作流表单名称
...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_hr_certifyapplyassess c SET  c.ExaStatus='部门审批',c.status = 2 where c.id='$billId' ");
