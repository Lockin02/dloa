...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"hr_recontract";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update hr_recontract set ExaStatus='完成',statusId = '6',ExaDT = now() where id='$billId'";//审批完成后更新语句
    ...
    //变量定义
    $formName=isset($formName)?$formName:"合同续签";//工作流表单名称
...
//生成工作流
if($actTo=="ewfBuild"){
	    $msql->query(" update hr_recontract SET ExaStatus='部门审批',statusId='3' where id='$billId' ");//提交审批后改变审批状态
        $sendToURL="../../../index1.php?model=hr_recontract_recontract&action=toParentWindowsClose";//生成工作流后跳转的页面
...
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=contract_contract_contract&action=toAdd";//处理审批后跳转的页面
