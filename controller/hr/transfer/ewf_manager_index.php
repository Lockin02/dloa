...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_hr_personnel_transfer";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_hr_personnel_transfer set status=2, ExaStatus = '完成',ExaDT = '".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_hr_personnel_transfer set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
...
//多部门ID，值='1,2,3',数值为对应的部门ID。只要把对应需要审批的部门ID已这个方式传到$billDept即可。
    ...
    //变量定义
    $formName=isset($formName)?$formName:"调岗申请审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    //$sendToURL=$baseDir."?actTo=swfList";
     $msql->query(" update oa_hr_personnel_transfer SET ExaStatus='部门审批' where id='$billId' ");
    $sendToURL="../../../index1.php?model=hr_transfer_transfer&action=toAddJobTran";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../index1.php?model=hr_transfer_transfer&action=toRead&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=hr_transfer_transfer&action=toAddJobTran";//处理审批后跳转的页面
