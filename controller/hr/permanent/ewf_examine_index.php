...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_hr_permanent_examine set ExaStatus ='完成',ExaDT = '".date("Y-m-d")."',status = 6 where ID='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_hr_permanent_examine set ExaStatus ='打回',ExaDT = '".date("Y-m-d")."'  where ID='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"试用考核评估";//工作流表单名称
...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_hr_permanent_examine c SET  c.ExaStatus='部门审批',status =4  where c.id='$billId' ");
   $sendToURL="../../../index1.php?model=hr_permanent_examine&action=allpage";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=hr_permanent_examine&action=toView&id=$billId";//审批查看内容
...
if($actTo=="delWork"){
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $examCode=isset($examCode)?$examCode:"oa_hr_permanent_examine";//审批数据表
    $formName=isset($formName)?$formName:"试用考核评估";//工作流表单名称
