...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_hr_tutor_reward";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_hr_tutor_reward set ExaStatus='完成',ExaDT = now() where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_hr_tutor_reward set ExaStatus = '打回',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"导师奖励方案";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_hr_tutor_reward SET ExaStatus='部门审批' where id='$billId' ");//提交审批后改变审批状态
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=hr_tutor_reward&action=init&id=$billId&perm=view&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
	$billId=isset($billId)?$billId:"";//审批表单数据ID
    $sendToURL="../../../index1.php?model=hr_tutor_reward&action=configOrder&orderId=" + $billId;//处理审批后跳转的页面
