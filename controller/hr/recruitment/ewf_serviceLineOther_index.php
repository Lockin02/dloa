...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_hr_recruitment_apply";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_hr_recruitment_apply set state=1, ExaStatus = '完成',ExaDT = now() where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_hr_recruitment_apply set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"增员申请(服务线其他)";//工作流表单名称
...
if($actTo=="ewfBuild"){
    //$sendToURL=$baseDir."?actTo=swfList";
     $msql->query(" update oa_hr_recruitment_apply SET ExaStatus='部门审批' where id='$billId' ");
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../index1.php?model=hr_recruitment_apply&action=toRead&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=hr_recruitment_apply&action=page";//处理审批后跳转的页面，跳转到待审批页面
...
if($actTo=="delWork"){
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $examCode=isset($examCode)?$examCode:"oa_hr_recruitment_apply";//审批数据表
    $formName=isset($formName)?$formName:"离职/换岗补充增员申请(服务线)";//工作流表单名称
