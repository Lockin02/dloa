...
if($actTo=="ewfSelect"){
	...
	$passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_hr_recruitment_apply set state=1, ExaStatus = '完成',ExaDT = '".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
	...
	//变量定义
	$formName=isset($formName)?$formName:"增员申请审批(计划外)";//工作流表单名称
...
if($actTo=="ewfBuild"){
	...
	$sendToURL="../../../index1.php?model=hr_recruitment_apply&action=mypage";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
	...
	$billId=isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl=isset($detailUrl)?$detailUrl:"../../index1.php?model=hr_recruitment_apply&action=toView&id=$billId&skey=$skey";//审批查看内容
	$ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
	$sendToURL="../../../index1.php?model=hr_recruitment_apply&action=mypage";//处理审批后跳转的页面，跳转到待审批页面
