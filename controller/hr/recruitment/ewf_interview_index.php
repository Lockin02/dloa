...
if($actTo=="ewfExam"){
	...
	$billId=isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=hr_recruitment_interview&action=toRead&id=$billId";//审批查看内容

	$ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
	$sendToURL="../../../index1.php?model=hr_recruitment_interview&action=toAssetAuditTab";//处理审批后跳转的页面
...
if($actTo=="delWork"){
	...
	$formName=isset($formName)?$formName:"面试评估表";//工作流表单名称
