...
if($actTo=="ewfSelect"){
	...
	$passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_hr_recruitment_interview set ExaStatus ='完成',ExaDT = now(),state = 1 where ID='$billId'";//审批完成后更新语句
	$disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_hr_recruitment_interview set ExaStatus ='打回',ExaDT = now() where ID='$billId'";//审批打回更新语句
	//特殊审批角色： 项目经理 -- 区域经理
	...
	//变量定义
	$formName=isset($formName)?$formName:"面试评估审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
	...
	$sendToURL="../../../index1.php?model=hr_recruitment_interview&action=tolastpage";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
	...
	$billId=isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=hr_recruitment_interview&action=toview&id=$billId";//审批查看内容

	$ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
	$sendToURL="../../../index1.php?model=hr_recruitment_interview&action=tolastpage";//处理审批后跳转的页面
...
if($actTo=="delWork"){
	$billId=isset($billId)?$billId:"";//审批表单数据ID
	$examCode=isset($examCode)?$examCode:"oa_hr_recruitment_interview";//审批数据表
	$formName=isset($formName)?$formName:"面试评估审批(管理岗位)";//工作流表单名称
