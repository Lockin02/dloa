...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_hr_leave";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_hr_leave set ExaStatus='完成',ExaDT = now() where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_hr_leave set ExaStatus = '打回',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    $eUserId=isset($eUserId)?$eUserId:"";//审批表单
    //
    ...
    //变量定义
    $formName=isset($formName)?$formName:"离职申请审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_hr_leave SET ExaStatus='部门审批' where id='$billId' ");//提交审批后改变审批状态
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=hr_leave_leave&action=init&id=$billId&perm=view&skey=$skey";//审批查看内容
...
//	$billId=isset($billId)?$billId:"";//审批表单数据ID
    ...
    $sendToURL="../../../index1.php?model=hr_leave_leave&action=leaveMail&spid=$spid&result=$result&rows[id]=$leaveRows[id]&rows[recipientName]=$recipientName&rows[recipientId]=$recipientId&rows[handContent]=$handContent&rows[requireDate]=$leaveRows[requireDate]";//处理审批后跳转的页面
