...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_leave_handover";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_leave_handover set ExaStatus='完成',ExaDT = now() where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_leave_handover set ExaStatus = '打回',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"离职交接清单确认";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $msql->query(" update oa_leave_handover SET ExaStatus='部门审批' where id='$billId' ");//提交审批后改变审批状态
...
if($actTo=="ewfExamSub"){
	$billId=isset($billId)?$billId:"";//审批表单数据ID
    $sendToURL="../../../index1.php?model=hr_leave_leave&action=configOrder&orderId=" + $billId;//处理审批后跳转的页面
