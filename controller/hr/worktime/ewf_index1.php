...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_hr_worktime_apply";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_hr_worktime_apply set ExaStatus='完成',ExaDT = now() where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_hr_worktime_apply set ExaStatus = '打回',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"节假日加班申请审批";//工作流表单名称
...
//    $msql->query(" update oa_hr_worktime_apply SET ExaStatus='部门审批',applyDate=curdate() where id='$billId' ");//提交审批后改变审批状态
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=hr_worktime_apply&action=toView&id=$billId";//审批查看内容
