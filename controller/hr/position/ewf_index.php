...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_hr_position_description set ExaStatus ='完成',ExaDT = '".date("Y-m-d")."' where ID='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_hr_position_description set ExaStatus ='打回',ExaDT = '".date("Y-m-d")."'  where ID='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
...
//	$msql->query(" update oa_purch_plan_basic c SET  c.ExaStatus='部门审批' where c.id='$billId' ");
   $sendToURL="../../../index1.php?model=hr_position_positiondescript&action=mypage";//生成工作流后跳转的页面
