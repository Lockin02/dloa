...
if($actTo=="ewfSelect"){
	//传入区域参数
    $billArea=isset($billArea)?$billArea:"";//审批表单所属区域
    $ewf->setBillArea($billArea);

    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $examCode=isset($examCode)?$examCode:"cost_summary_list";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update cost_summary_list set ExaStatus = '完成',ExaDT = now(),Status = '出纳付款' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update cost_summary_list set ExaStatus = '打回',Status = if(needExpenseCheck = 1,'部门检查','打回'),ExaDT = now(),IsFinRec = 0 where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    $billCompany=isset($billCompany)?$billCompany:'';//审批表单归属公司
    //
    ...
    //变量定义
    $formName=isset($formName)?$formName:"报销审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
	...
	$msql->query(" update cost_summary_list SET ExaStatus='部门审批',status = '部门审批' where id='$billId' ");
//    $sendToURL="../../../view/reloadParent.php";//生成工作流后跳转的页面
	$sendToURL="../../../index1.php?model=finance_budget_budget&action=updateByExpense&billId=$billId";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=finance_invoiceapply_invoiceapply&action=initAuditing&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=finance_invoiceapply_invoiceapply&action=approvalNo";//处理审批后跳转的页面
...
if($actTo=="delWork"){
    ...
    $examCode=isset($examCode)?$examCode:"cost_summary_list";//审批数据表
    $formName=isset($formName)?$formName:"报销审批";//工作流表单名称
    $returnSta=isset($returnSta)?$returnSta:"编辑";//回滚单据审批状态
    $flag=isset($flag)?$flag:"json";//回滚单据审批状态
    //撤销SQL
	$delSql = " update cost_summary_list c SET c.Status='编辑' where c.id='$billId' ";
