...
$sql= "select * from cost_summary_list where 1 and isProject='0' and Status in('部门审批','财务审核','出纳付款','完成') and BillNo = '$QR_BillNo'" ;
$fsql->query($sql);
...
while($fsql->next_record()){
    ...
    $msql->query("select l.* , d.costflag , d.code ,us.dept_id, u.account , u.usercard  from cost_summary_list l
            left join department d on (l.CostBelongtoDeptIds=d.dept_name)
            left join user us on (us.user_id = l.costman )
            left join hrms u on (u.user_id = l.costman )
            where BillNo='".$fsql->f("BillNo")."'");
    if($msql->next_record())
    {
        $costflag=$msql->f('costflag');
        $code=$msql->f('code');
    	if($msql->f("CostBelongTo")==2)
    		$checkPrintName=true;
    	else
    		$checkPrintName=false;
        $subDept=$msql->f("SubDept");
        $clientType=$msql->f("CostClientType");
        $clientArea=$msql->f("CostClientArea");
        $clientName=$msql->f("CostClientName");
...
<div style="text-align:right;vertical-align: top;">
    ...
    <table class="small tabp" border="1"  bordercolor="black" cellspacing="0" cellpadding="0" width="100%" align="center"  >
            <tr class="TableLine1  WriteBg tabtr">
                <td nowrap="" colspan="2" width="15%">员工：<?php echo $costman_name;?></td>
                <td width="15%" nowrap>部门：<?php echo $costdepart_name;?></td>
                <td width="40%" >时间：<font size="1"><?php echo $CostDates;?></font></td>
                <td width="30%">区域：<?php
                if (in_array($CostDeptId, $NetAppLimitDeptI) && $_SESSION['COM_BRN_PT'] != 'bx') {

                 	$sql="select a.id,a.officeName, a.managerCode  from  oa_esm_office_baseinfo a where ID='$appArea' ";
					$msql->query($sql);
					$msql->next_record();
                    echo $msql->f('officeName');
                    if(!$msql->f('officeName')){
...
?>
            ...
            <tr class="TableLine1 tabtr WriteBg">
                <td  colspan="5" nowrap>
                <?php
                if($costflag=='spe'){
                    ?>
                    客户类型：<?php echo $clientType;?>&nbsp;&nbsp;
                    <?php
                    if($clientArea!="其他") echo "客户区域：".$clientArea."&nbsp;&nbsp;";
                    if($clientName!="其他") echo "客户名字：".$clientName;
                }elseif($costflag=='abr'){
                    ?>
                    海外国家名称：<?php echo $clientType;?>&nbsp;&nbsp;
                    客户名称：<?php echo $clientName;?>
                    <?php
                }else{
                    ?>
                    事由：<?php echo $clientType;?>
                    <?php
                }
                ?>
                </td>
...
<?php
    ...
    if($msql->num_rows()>0){
        ?>
<table class="small extab" bordercolor="black" bgcolor="black" border="1" cellspacing="0" cellpadding="0" width="100%" align="center" >
