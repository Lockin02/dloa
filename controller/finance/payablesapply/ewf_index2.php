...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_finance_payablesapply_changeaudit";//审批数据表
    ...
    //变量定义
    $formName=isset($formName)?$formName:"变更审批付款日期";//工作流表单名称
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=finance_payablesapply_payablesapplychange&action=toAuditView&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=finance_payablesapply_payablesapplychange&action=auditundo";//处理审批后跳转的页面
