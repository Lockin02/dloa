...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_sale_rdproject set ExaStatus='完成',state = '2',ExaDT = now(),ExaDTOne = now() where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_sale_rdproject set ExaStatus = '打回',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_sale_rdproject SET ExaStatus='部门审批',state='1' where id='$billId' ");//提交审批后改变审批状态
    $sendToURL="../../../index1.php?model=rdproject_yxrdproject_rdproject&action=toAddResearch";//生成工作流后跳转的页面
