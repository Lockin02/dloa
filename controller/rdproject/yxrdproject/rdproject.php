...
/**
 * @description: 研发合同Controller类
 * @date 2010-12-1 下午04:22:23 for LiuB
 */
...
//		}
		...
		if($id && $_GET['act'] == "app"){
			...
			msgGo('添加成功！','?model=rdproject_yxrdproject_rdproject&action=toAddResearch');
		}
//		$this->listDataDict();
	}
...
//   	  $this->permCheck();
		...
		if ($perm == 'view') {
            ...
            if($rows['areaPrincipalId'] != $_SESSION['USER_ID']&&$rows['orderPrincipalId'] != $_SESSION['USER_ID']&&$rows['createId'] != $_SESSION['USER_ID']){
                $rows = $this->service->filterWithoutField('研发合同金额',$rows,'form',array('orderMoney','orderTempMoney'));
                $rows['rdprojectequ'] = $this->service->filterWithoutField('研发设备金额',$rows['rdprojectequ'],'list',array('price','money'));
            }

            $rows['file'] = $this->service->getFilesByObjId($rows['id'],false);
            //合同文本权限
            if($rows['areaPrincipalId'] != $_SESSION['USER_ID']&&$rows['orderPrincipalId'] != $_SESSION['USER_ID']&&$rows['createId'] != $_SESSION['USER_ID']){
	             if(!empty($this->service->this_limit ['研发合同金额'])){
...
/**********************************************************************************************************/
	...
	function c_jsonCloseAuditNo() {
		...
		$service->searchArr['workFlow'] = '研发合同异常关闭审批';
...
//        $this->permCheck ();//安全校验
   ...
   function c_toShipView(){
    ...
    function c_toInvoiceTab(){
        ...
        if($this->service->isKeyMan_d($obj['objId'])||isset($this->service->this_limit['研发合同开票tab'])){
    ...
    function c_toIncomeTab(){
        $obj = $_GET['obj'];
    	$this->permCheck($obj['objId']);
        if($this->service->isKeyMan_d($obj['objId'])||isset($this->service->this_limit['研发合同到款tab'])){
	...
	function c_toInvoiceApplyTab() {
		$obj = $_GET ['obj'];
		if ($this->service->isKeyMan_d ( $obj ['objId'] ) || ! empty ( $this->service->this_limit ['研发合同开票tab'] )) {
