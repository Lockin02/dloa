...
class controller_rdproject_role_rdrole extends controller_base_action {
	...
	function c_rolePageInOption() {
		...
		$this->assign('proCenter',$proCenter);
	...
	function c_toAdd() {
		...
		if (! empty ( $projectId )&&$proCenter!=1) {
			//权限判断
			$this->isCurUserPerm ( $projectId, 'project_teamMember_admin' );
		}
		if($projectId){
			$rdprojectDao=new model_rdproject_project_rdproject();
			$rows=$rdprojectDao->get_d($projectId);
			$this->showDatadicts ( array('projectType' => 'YFXMGL'), $rows ['projectType'] );
		}else{
