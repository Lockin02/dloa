...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_rd_project SET status=6,ExaStatus='完成',actBeginDate='".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_rd_project SET status=4,ExaStatus='打回' where id='$billId' ";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_rd_project c SET c.status=2 where c.id='$billId' ");
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=rdproject_project_rdproject&action=rpRead&pjId=$billId&readType=exam";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=rdproject_project_rdproject&action=rpApprovalNo";//处理审批后跳转的页面
