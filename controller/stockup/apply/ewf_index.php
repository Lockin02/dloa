...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_stockup_apply";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_stockup_apply set status=3,ExaStatus = '完成',ExaDT = now() where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_stockup_apply set status=1,ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"产品备货销售申请审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
   ...
   // $msql->query2("update oa_stockup_apply set status=3 where id='$billId'");
