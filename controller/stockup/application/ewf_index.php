...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_stockup_application";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_stockup_application set status=4,ExaStatus = '完成',ExaDT = now() where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:"update oa_stockup_application set status=1,ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    $proSid=isset($proSid)?$proSid:$isto;//项目任务书ID  --项目经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"产品备货交付申请审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    if($proSid==2){
    	...
    	$sendToURL="../../../index1.php?model=stockup_apply_apply&action=appList";//生成工作流后跳转的页面
    }
	$proSid=0;
...
if($actTo=="delWork"){
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $examCode=isset($examCode)?$examCode:"oa_stockup_application";//审批数据表
    $formName=isset($formName)?$formName:"订票需求";//工作流表单名称
