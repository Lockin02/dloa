...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_asset_allocation";//审批数据表
    ...
    //变量定义
    $formName=isset($formName)?$formName:"资产调拨审批";//工作流表单名称
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=asset_daily_allocation&action=init&id=$billId&skey=$skey&perm=view&btn=1";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=asset_daily_allocation&action=page";//处理审批后跳转的页面
...
if($actTo=="delWork"){
	...
	//撤销SQL
	$delSql = " update oa_asset_allocation c SET c.ExaStatus='待提交' where c.id='$billId' ";
