...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_asset_scrap";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_asset_scrap set ExaStatus = '完成',ExaDT = '".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_asset_scrap set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"资产报废申请审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    //$sendToURL=$baseDir."?actTo=swfList";
     $msql->query(" update oa_asset_scrap SET ExaStatus='部门审批' where id='$billId' ");
...
if($actTo=="delWork"){
	...
	//撤销SQL
	$delSql = " update oa_asset_scrap c SET c.ExaStatus='待提交' where c.id='$billId' ";
