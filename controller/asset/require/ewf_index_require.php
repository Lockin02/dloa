...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_asset_requirement";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_asset_requirement set isRecognize='1',ExaStatus = '完成',ExaDT = '".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_asset_requirement set ExaStatus = '打回' where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"资产需求申请审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    //$sendToURL=$baseDir."?actTo=swfList";
     $msql->query(" update oa_asset_requirement SET isRecognize='3',ExaStatus='部门审批' where id='$billId' ");
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../index1.php?model=asset_require_requirement&action=toView&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=asset_require_requirement";//处理审批后跳转的页面，跳转到待审批页面
...
if($actTo=="delWork"){
	...
	$examCode=isset($examCode)?$examCode:"oa_asset_requirement";//审批数据表
	$formName=isset($formName)?$formName:"资产需求申请审批";//工作流表单名称
	$returnSta=isset($returnSta)?$returnSta:"";//回滚单据审批状态
	$flag=isset($flag)?$flag:"json";//回滚单据审批状态
	//撤销SQL
	$delSql = " update oa_asset_requirement c SET c.ExaStatus = '',c.isRecognize = 0 where c.id='$billId' ";
