...
//生成工作流
if($actTo=="ewfBuild"){
   $sendToURL="../../../index1.php?model=supplierManage_temporary_temporary&action=myloglist";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=supplierManage_temporary_temporary&action=init&perm=view&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=supplierManage_temporary_temporary&action=rpApprovalNo";//处理审批后跳转的页面
