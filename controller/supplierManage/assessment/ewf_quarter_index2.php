...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_supp_suppasses";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_supp_suppasses set ExaStatus = '完成',ExaDT = '".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_supp_suppasses_detail d left join oa_supp_suppasses s on s.id = d.parentId set d.affstate = 0,s.ExaStatus = '打回' where s.id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"供应商季度考核审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
    //$sendToURL=$baseDir."?actTo=swfList";
     $msql->query(" update oa_supp_suppasses_detail d left join oa_supp_suppasses s on s.id = d.parentId set d.affstate = 1,s.ExaStatus = '部门审批' where s.id ='$billId' ");
    $sendToURL="../../../index1.php?model=supplierManage_assessment_supasses&action=toAssesList";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../index1.php?model=supplierManage_assessment_supasses&action=toView&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=supplierManage_assessment_supasses&action=toMyList";//处理审批后跳转的页面，跳转到待审批页面
