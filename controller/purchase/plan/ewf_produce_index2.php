...
//	$msql->query(" update oa_purch_plan_basic c SET  c.ExaStatus='部门审批' where c.id='$billId' ");
   $sendToURL="../../../index1.php?model=purchase_external_external&action=toProduceAdd";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=purchase_plan_basic&action=read&purchType=produce&actType=audit&id=$billId";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=purchase_plan_basic&action=toProduceAuditTab";//处理审批后跳转的页面
