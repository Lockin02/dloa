...
if ($actTo == "ewfSelect") {
	...
	$passSqlCode = isset ( $passSqlCode ) ? $passSqlCode : " update oa_purch_apply_basic SET signState=0, ExaStatus='完成',ExaDT='".date("Y-m-d")."' where id='$billId' "; //审批完成后更新语句
...
if ($actTo == "ewfBuild") {
	...
	$sendToURL = "../../../index1.php?model=purchase_contract_purchasecontract&action=myPurchaseContractTab#tab1"; //生成工作流后跳转的页面
...
if ($actTo == "ewfExam") {
	...
	$billId = isset ( $billId ) ? $billId : ""; //审批表单数据ID
	$detailUrl = isset ( $detailUrl ) ? $detailUrl : "../../../index1.php?model=purchase_contract_purchasecontract&action=toTabView&id=$billId&readType=exam&skey=$skey"; //审批查看内容
