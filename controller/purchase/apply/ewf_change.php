...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_purch_apply_change SET state=3,ExaStatus='完成' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_purch_apply_change SET state=2,ExaStatus='打回' where id='$billId' ";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"采购申请单变更";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    //$sendToURL="../../../index1.php?model=purchase_task_basic&action=taskMyList&clickNumb=2";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=purchase_apply_applychange&action=readmain&id=$billId&goBack=no";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
	//TODO:
    $sendToURL="../../../index1.php?model=purchase_apply_applychange&action=myApproval";//处理审批后跳转的页面
