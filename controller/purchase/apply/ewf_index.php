...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_purch_apply_basic SET state=7,ExaStatus='待执行' where id='$billId' ";//审批完成后更新语句
...
if($actTo=="ewfBuild"){
	...
	$msql->query(" update oa_purch_apply_basic SET state=1 where id='$billId' ");

    $sendToURL="../../../index1.php?model=purchase_contract_purchasecontract&action=toMyApprovalTab";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=purchase_contract_purchasecontract&action=toViewVontract&id=$billId&goBack=no";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=purchase_apply_applybasic&action=myApproval";//处理审批后跳转的页面
