...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_purch_inquiry c SET c.state=1, c.ExaStatus='部门审批' where c.id='$billId' ");
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=purchase_inquiry_inquirysheet&action=toAssignSupp&actType=audit&id=$billId";//审批查看内容
...
if($actTo=="delWork"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_purch_inquiry";//审批数据表
    $formName=isset($formName)?$formName:"采购询价单审批";//工作流表单名称
