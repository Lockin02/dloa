...
if($actTo=="ewfSelect"){
    ...
    //变量定义
    $formName=isset($formName)?$formName:"采购退料审批";//工作流表单名称
...
//	$msql->query(" update oa_purch_plan_basic c SET  c.ExaStatus='部门审批' where c.id='$billId' ");
   $sendToURL="../../../index1.php?model=purchase_delivered_delivered&action=toMyList";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=purchase_delivered_delivered&action=init&perm=view&id=$billId&skey=$skey";//审批查看内容
