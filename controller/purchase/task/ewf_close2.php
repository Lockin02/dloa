...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_purch_task_basic SET state=4,dateFact='".date("Y-m-d")."', ExaStatus='完成',ExaDT='".date("Y-m-d")."' where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_purch_task_basic SET state=1, ExaStatus='打回',ExaDT='".date("Y-m-d")."' where id='$billId' ";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"采购任务关闭审批";//工作流表单名称
...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_purch_task_basic  SET ExaStatus='部门审批' where id='$billId' ");
...
if($actTo=="ewfExam"){
    ...
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=purchase_task_basic&action=toCloseRead&id=$billId&skey=$skey";//审批查看内容
