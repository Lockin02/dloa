...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_esm_resource_apply";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_esm_resource_apply set ExaStatus = '完成',confirmStatus = 3,ExaDT = now() where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_esm_resource_apply set ExaStatus = '打回',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"设备申请(项目)";//工作流表单名称
...
if($actTo=="ewfBuild"){
    //$sendToURL=$baseDir."?actTo=swfList";
     $msql->query(" update oa_esm_resource_apply SET ExaStatus='部门审批' where id='$billId' ");
