...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_esm_project_statusreport";//审批数据表
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_esm_project_statusreport set ExaStatus = '完成',status = 'XMZTBG03',ExaDT = now() where id='$billId' ";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_esm_project_statusreport set ExaStatus = '打回',status = 'XMZTBG01',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //变量定义
    $formName=isset($formName)?$formName:"工程项目周报审批";//工作流表单名称
...
if($actTo=="ewfBuild"){
	...
	$msql->query(" update oa_esm_project_statusreport SET status='XMZTBG02' where id='$billId' ");
