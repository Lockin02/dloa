...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:" update oa_sale_service set ExaStatus='完成',state = '2',ExaDT = now() where id='$billId'";//审批完成后更新语句
    $disPassSqlCode=isset($disPassSqlCode)?$disPassSqlCode:" update oa_sale_service set ExaStatus = '打回',ExaDT = now() where id='$billId'";//审批打回更新语句
    //特殊审批角色： 项目经理 -- 区域经理
    ...
    //客户信息
    $sql="select customertype from oa_sale_service  where id='$billId' ";
    $msql->query($sql);
    $msql->next_record();
...
//生成工作流
if($actTo=="ewfBuild"){
	$msql->query(" update oa_sale_service SET ExaStatus='部门审批',state='1' where id='$billId' ");//提交审批后改变审批状态
   $sendToURL="../../../index1.php?model=engineering_serviceContract_serviceContract&action=toAddContract";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=engineering_serviceContract_serviceContract&action=init&perm=view&id=$billId&skey=$skey";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=engineering_serviceContract_serviceContract&action=toApprovalNo";//处理审批后跳转的页面
