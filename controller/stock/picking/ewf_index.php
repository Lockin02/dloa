...
if($actTo=="ewfSelect"){
    ...
    $examCode=isset($examCode)?$examCode:"oa_stock_pickingapply";//审批数据表
    ...
    //变量定义
    $formName=isset($formName)?$formName:"发货申请单";//工作流表单名称
...
if($actTo=="ewfBuild"){
    ...
    $sendToURL="../../../index1.php?model=stock_picking_pickingapply&action=myApply";//生成工作流后跳转的页面
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
    $detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=stock_picking_pickingapply&action=auditing&id=$billId";//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=stock_picking_pickingapply&action=approvalNo";//处理审批后跳转的页面
