...
if($actTo=="ewfSelect"){
    ...
    $passSqlCode=isset($passSqlCode)?$passSqlCode:"update oa_stock_fillup set ExaStatus ='完成' where id=".$billId;//审批完成后更新语句
...
if($actTo=="ewfExam"){
    ...
    $billId=isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl=isset($detailUrl)?$detailUrl:"../../../index1.php?model=stock_fillup_fillup&action=init&actType=audit&id=$billId&skey=".$_GET['skey'];//审批查看内容
    $ewf->examWorkFlow($taskId,$spid,$detailUrl);
}
//提交审批工作流
if($actTo=="ewfExamSub"){
    $sendToURL="../../../index1.php?model=stock_fillup_fillup&action=toAuditNo";//处理审批后跳转的页面
