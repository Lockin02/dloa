...
if ($actTo == "ewfSelect") {
	...
	$examCode = isset ( $examCode ) ? $examCode : "oa_service_reduce_apply"; //审批数据表
	$passSqlCode = isset ( $passSqlCode ) ? $passSqlCode : "update oa_service_reduce_apply c  set c.ExaStatus = '完成',c.ExaDT = now() where c.id='$billId'"; //审批完成后更新语句
	$disPassSqlCode = isset ( $disPassSqlCode ) ? $disPassSqlCode : "update oa_service_reduce_apply set ExaStatus = '打回' where id='$billId'"; //审批打回更新语句
	//特殊审批角色： 项目经理 -- 区域经理
	...
	//变量定义
	$formName = isset ( $formName ) ? $formName : "费用减免申请单审批"; //工作流表单名称
...
if ($actTo == "ewfExam") {
	...
	$billId = isset ( $billId ) ? $billId : ""; //审批表单数据ID
	$detailUrl = isset ( $detailUrl ) ? $detailUrl : "../../../index1.php?model=service_reduce_reduceapply&action=toView&id=$billId&skey=$skey"; //审批查看内容
	$ewf->examWorkFlow ( $taskId, $spid, $detailUrl );
}
//提交审批工作流
if ($actTo == "ewfExamSub") {
	$sendToURL = "../../../index1.php?model=service_reduce_reduceapply"; //处理审批后跳转的页面
