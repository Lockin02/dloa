...
if($actTo == "ewfSelect") {
	...
	$passSqlCode = isset($passSqlCode)?$passSqlCode:" update oa_produce_produceapply c set c.ExaStatus = '完成',c.ExaDT = now() ,c.docStatus=0 where c.id='$billId' ";//审批完成后更新语句
	$disPassSqlCode = isset($disPassSqlCode)?$disPassSqlCode:" update oa_produce_produceapply c set c.ExaStatus = '打回' ,c.docStatus=7 where c.id='$billId'";//审批打回更新语句
	//特殊审批角色： 项目经理 -- 区域经理
	...
	//变量定义
	$formName = isset($formName)?$formName:"生产申请审批";//工作流表单名称
...
if($actTo == "ewfBuild"){
	...
	$msql->query(" update oa_produce_produceapply c SET c.ExaStatus='部门审批',c.docStatus=6 where c.id='$billId' ");
...
if($actTo=="ewfExam"){
	...
	$billId = isset($billId)?$billId:"";//审批表单数据ID
	$detailUrl = isset($detailUrl)?$detailUrl:"../../index1.php?model=produce_apply_produceapply&action=toView&id=$billId&skey=$skey";//审批查看内容
	$ewf->examWorkFlow($taskId,$spid,$detailUrl);
}

//提交审批工作流
if($actTo == "ewfExamSub"){
	$sendToURL = "../../../index1.php?model=produce_apply_produceapply&action=toAssetAuditTab";//处理审批后跳转的页面，跳转到待审批页面
