...
if($actTo == "ewfSelect") {
	...
	$examCode = isset($examCode)?$examCode:"oa_produce_produceapply";//审批数据表
	$passSqlCode = isset($passSqlCode)?$passSqlCode:"update oa_produce_produceapply set ExaStatus = '完成' ,ExaDT = now() ,docStatus = 1 where id='$billId' ";//审批完成后更新语句
	$disPassSqlCode = isset($disPassSqlCode)?$disPassSqlCode:"update oa_produce_produceapply set ExaStatus = '打回' ,ExaDT = now() where id='$billId'";//审批打回更新语句
	//特殊审批角色： 项目经理 -- 区域经理
	...
	//变量定义
	$formName = isset($formName)?$formName:"生产申请变更审批";//工作流表单名称
...
//生成工作流
if($actTo == "ewfBuild") {
	$msql->query(" update oa_produce_produceapply o1 left join oa_produce_produceapply o2 on o1.id=o2.originalId SET o1.ExaStatus='变更审批中',o1.docStatus = 8 where o2.id ='$billId'");//更改原来单据为变更审批中状态
