...
if($actTo == "ewfSelect") {
	...
	$examCode = isset($examCode)?$examCode:"oa_produce_produceplan";//审批数据表
	$passSqlCode = isset($passSqlCode)?$passSqlCode:"update oa_produce_produceplan set ExaStatus = '完成' ,ExaDT = now() ,docStatus = 6 where id='$billId' ";//审批完成后更新语句
	$disPassSqlCode = isset($disPassSqlCode)?$disPassSqlCode:"update oa_produce_produceplan set ExaStatus = '打回' ,ExaDT = now() where id='$billId'";//审批打回更新语句
	//特殊审批角色： 项目经理 -- 区域经理
	...
	//变量定义
	$formName = isset($formName)?$formName:"生产计划关闭审批";//工作流表单名称
...
//生成工作流
if($actTo == "ewfBuild") {
	$msql->query("update oa_produce_produceplan c SET c.ExaStatus='部门审批' where c.id='$billId' ");//更改原来单据为变更审批中状态
