...
//
//	this.location = "?model=engineering_task_rdtask&action=toPlanTaskPage&pnId="+planId+"&pnName="+planName+"&pjId="+projectId+"&pjName="+projectName;
}
...
function newTkNodeGridTree() {
	...
	var content = {
		...
		dataUrl : "?model=engineering_template_rdtnode&action=getParentRowsByTemplateId&templateId="+$("#templateId").val(),
		lazyLoadUrl : "?model=engineering_template_rdtnode&action=getRowsByTemplateId&templateId="+$("#templateId").val(),
		...
		contextMenu : {
			items : [{
				...
				action : function(row) {
					if(isNode(row)){
						showThickboxWin('?model=engineering_template_rdtnode&action=init&perm=view&id='
							+ row.pid
							+ '&placeValuesBefore&TB_iframe=true&modal=false&height=300&width=600');
					}else{
						showThickboxWin('?model=engineering_template_rdttask&action=init&perm=view&id='
							+ row.pid
				...
				action : function(row) {
					if(isNode(row)){
						showThickboxWin('?model=engineering_template_rdtnode&action=init&id='
							+ row.pid
							+ '&placeValuesBefore&TB_iframe=true&modal=false&height=300&width=600');
					}else{
						showThickboxWin('?model=engineering_template_rdttask&action=init&id='
							+ row.pid
							+ '&placeValuesBefore&TB_iframe=true&modal=false&height=300&width=800');
					}
				}
                  ...
                  text: "删除",
                    ...
                    action: function(row){
                    if(isNode(row)){
						showThickboxWin('?model=engineering_template_rdtnode&action=toDelete&id='
							+ row.pid
							+ '&placeValuesBefore&TB_iframe=true&modal=false&height=150&width=300');
					}else{
						showThickboxWin('?model=engineering_template_rdttask&action=toDelete&id='
							+ row.pid
							+ '&placeValuesBefore&TB_iframe=true&modal=false&height=150&width=300');
					}                }
			}]
