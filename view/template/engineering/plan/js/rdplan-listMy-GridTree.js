...
function newProjectTreeGrid() {
	...
	var content = {
		...
		dataUrl : "?model=engineering_project_engineering&action=rpAjaxMyPlan",
		lazyLoadUrl : "?model=engineering_plan_rdplan&action=rpAjaxMyPlan",
			...
			items : [{
				...
				action : function(row) {
					if(showMenuFnR(row)){
						...
						location='?model=engineering_task_rdtask&action=toPlanTaskPage&viewTask=1&pnId='
							+ row.pid
							+ "&pnName="
							+ row.planName
							+ "&pjId="
