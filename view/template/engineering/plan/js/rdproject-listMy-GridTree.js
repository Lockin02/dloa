function show_page(page) {
	this.location = "?model=engineering_group_rdgroup&action=page";
}
		...
		contextMenu : {
			...
			items : [{
				...
				action : function(row) {
					//根据是否有项目名称判断是项目还是组合
					if ( showMenuFnP(row) ) {
						showThickboxWin('?model=engineering_group_rdgroup&action=rgRead&gpId='
								+ row.pid
								+ '&placeValuesBefore&TB_iframe=true&modal=false&height=450&width=950');
					} else {
						showThickboxWin('?model=engineering_project_engineering&action=rpRead&pjId='
								+ row.pid
								+ '&placeValuesBefore&TB_iframe=true&modal=false&height=450&width=750');
					}
					// alert(t.pid)
				...
				action : function(row) {
					if ( showMenuFnP(row) ) {
						showThickboxWin('?model=engineering_group_rdgroup&action=rgUpdateTo&gpId='
								+ row.pid
								+ '&placeValuesBefore&TB_iframe=true&modal=false&height=450&width=950');
					} else {
						showThickboxWin('?model=engineering_project_engineering&action=rpUpdateTo&pjId='
								+ row.pid
								+ '&placeValuesBefore&TB_iframe=true&modal=false&height=450&width=750');
					}
				}
