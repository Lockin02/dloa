...
function mydel(obj, mytable,orderArray) {
	if (confirm('确定要删除该行？')) {
		...
		$row.append("<input type='text' name='serviceContract["+ orderArray +"]["+ rowNo +"][isDel]' value='1'>");
		$row.hide();
...
function dynamic_add(packinglist, countNumP) {
	...
	oTL2.innerHTML = "<input type='hidden' id='productId" + mycount + "'  name='serviceContract[serviceequ][" + mycount + "][productId]'/>" +
			         "<input id='productName" + mycount + "' type='text' class='txtmiddle' name='serviceContract[serviceequ][" + mycount + "][productName]'/>";
	...
	oTL10.innerHTML = "<img src='images/closeDiv.gif' onclick='mydel(this,\"" + packinglist.id + "\")' title='删除行'>";
