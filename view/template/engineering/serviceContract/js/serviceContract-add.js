...
//					$('#orderTempCode').val(data.orderTempCode);
					...
					// 提取从表数据
					      $.post(
									"?model=engineering_serviceContract_serviceContract&action=ajaxList",
									{
										"id" : data.orgid
									}, function(data) {
										 var obj = eval("(" + data + ")");
                                         ...
                                         if(index != -1){
                                            //如果有，则删除数组中的元素
                                         	...
                                         	$("#tr[name^='ListTab_"+obj.id+"']").remove();
                                         	recount('List');
                                         	$("tr[name^='equTab_" + obj.id + "']").remove();
                                         	recount('invbody');
                                         	$("#tr[name^='mycustomTab_"+obj.id+"']").remove();
                                         	recount('mycustom');
                                         	$("#tr[name^='mytraTab_"+obj.id+"']").remove();
                                         	recount('mytra');
                                         }else{
                                            // 如果没有,把id放入数组中
											...
											//配置清单
											if (obj.servicelist){
											   List(obj.servicelist, 'List' ,obj.id);
											}
											// 设备
											if (obj.serviceequ) {
												// $("#myequ").html("");
												initEqu(obj.serviceequ, 'invbody', obj.id);
											}
//											//自定义清单
//											if (obj.customizelist) {
//												// $("#myinv").html("");
//												invlist(obj.customizelist, 'mycustom', obj.id);
//											}
											//培训计划
											if (obj.trainingplan) {
												// $("#mypay").html("");
												mypaylist(obj.trainingplan, 'mytra', obj.id);
											}
                                         }
									});
				}
...
//grid初始化 ---配置清单
function List(thisObj,tableId,orderId){
   var tableObj = $('#' + tableId);
   var objLength = thisObj.length;
   var productNumber = $("#productNum");
   var inStr = "";
   //获取当前行
   var rowNum = $("tr[name^='ListTab_']").length * 1 + $("tr[id^='ListTab_']").length * 1;
   // 当前计数值
	...
	for (var k = 0; k < objLength; k++){
	    rowNum++;
		i++;
		inStr = "<tr align='center' id='ListTab_1"+orderId+"'><td>"+rowNum+"</td>"
		   +"<td><input type='text' size='55' name='serviceContract[servicelist]["+i+"][serviceItem]' id='serviceItem"+i+"' value='"+thisObj[j].serviceItem+"'></td>"
		   +"<td><input type='text' class='txt' name='serviceContract[servicelist]["+i+"][serviceNo]' id='serviceNo"+i+"' value='"+thisObj[j].serviceNo+"'></td>"
           +"<td><input type='text' size='45' name='serviceContract[servicelist]["+i+"][serviceRemark]' id='serviceRemark"+i+"' value='"+thisObj[j].serviceRemark+"'></td>"
           + "<td><img src='images/closeDiv.gif' onclick='mydel(this,\"List\")' title='删除行'></td>"
		   + "</tr>";
		// 插入元素
	...
	for (var k = 0; k < objLength; k++) {
	   rowNum++;
		...
		inStr = "<tr name='equTab_" + orderId + "'><td>" + rowNum + "</td>"
		   +"<td><input type='text' class='readOnlyTxtItem' name='serviceContract[serviceequ]["+i+"][productNo]' id='productNo"+i+"' value='"+thisObj[j].productNo+"'></td>"
           +"<input type='hidden' name='serviceContract[serviceequ]["+i+"][unitName]' id='unitName"+i+"' value='"+thisObj[j].unitName+"'>"
           +"<input type='hidden' name='serviceContract[serviceequ]["+i+"][productId]' id='productId"+i+"' value='"+thisObj[j].productId+"'></td>"
           +"<td><input type='text' class='readOnlyTxtItem' name='serviceContract[serviceequ]["+i+"][productName]' id='productName"+i+"' value='"+thisObj[j].productName+"'></td>"
           +"<td><input type='text' class='readOnlyTxtItem' name='serviceContract[serviceqeu]["+i+"][productModel]' id='prodcutModel"+i+"' value='"+thisObj[j].productModel+"'></td>"
           +"<td><input type='text' class='readOnlyTxtItem' name='serviceContract[serviceequ]["+i+"][number]' id='number"+i+"' value='"+thisObj[j].number+"'></td>"
           +"<td><input type='text' class='readOnlyTxtItem' name='serviceContract[serviceequ]["+i+"][price]' id='price"+i+"' value='"+thisObj[j].price+"'/></td>"
           +"<td><input type='text' class='readOnlyTxtItem' name='serviceContract[serviceequ]["+i+"][money]' id='money"+i+"' value='"+thisObj[j].money+"'/></td>"
           +"<td><input type='text' class='readOnlyTxtItem' name='serviceContract[serviceequ]["+i+"][warrantyPeriod]' id='warrantyPeriod"+i+"' value='"+thisObj[j].warrantyPeriod+"'/></td>"
...
function traininglist(thisObj, tableId, orderId) {
	...
	// 获取当前行数
	var rowNum = $("tr[name^='mytraTab_']").length * 1
			+ $("tr[id^='mytraTab_']").length * 1;
	// 当前计数值
	...
	for (var k = 0; k < objLength; k++) {
		...
		inStr = "<tr name='trainingTab_" + orderId + "'><td>" + i + "</td>"
				+ "<td><input class='txtshort' type='text' name='serviceContract[trainingplan]["+ i + "][beginDT]' id='TraDT"+ i + "' onfocus='WdatePicker()' value='"+ thisObj[j].beginDT+ "'></td>"
				+ "<td><input class='txtshort' type='text' name='serviceContract[trainingplan]["+ i + "][endDT]' id='TraEndDT"+ i + "' onfocus='WdatePicker()' value='"+ thisObj[j].endDT+ "'></td>"
				+ "<td><input class='txtshort' type='text' name='serviceContract[trainingplan]["+ i + "][traNum]' value='" + thisObj[j].traNum + "'/></td>"
				+ "<td><textarea name='serviceContract[trainingplan][" +i+ "][adress]' rows='3' cols='15' style='width: 100%'>"+ thisObj[j].adress + "</textarea></td>"
				+ "<td><textarea name='serviceContract[trainingplan][" +i+ "][content]' rows='3' cols='15' style='width: 100%'>"+ thisObj[j].content + "</textarea></td>"
				+ "<td><textarea name='serviceContract[trainingplan][" +i+ "][trainer]' rows='3' cols='15' style='width: 100%'>"+ thisObj[j].trainer + "</textarea></td>"
...
$(function(){
   ...
   //开票类型
    ...
    //合同属性
	orderNatureCodeArr = getData('FWHTSX');
...
/**
 * 正则表达式 验证合同号由英文字母与数字组成
 */
function orderCodeFormV(obj){
	var O = /^[A-Za-z0-9]+$/;
	var P = O.test(obj);
	if(P == false){
	   alert ("请正确填写合同号!")
