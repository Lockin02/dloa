...
var show_page = function(page) {
	$(".myorderZzxGrid").yxgrid("reload");
};
$(function() {
	$(".myorderZzxGrid").yxgrid({
		...
		menusEx : [{
			...
			action : function(row) {
				showOpenWin('?model=engineering_serviceContract_serviceContract&action=toBecomeContract&id='
						+ row.id
						+ '&perm=edit'
						+ "&skey="
						+ row['skey_']
...
//			},
			action : function(row) {
				if (row.orderCode != "") {
					showModalWin('?model=finance_invoiceapply_invoiceapply&action=toAdd&invoiceapply[objId]='
							...
							+ row.orderCode + '&invoiceapply[objType]=KPRK-03');
				} else {
					showModalWin('?model=finance_invoiceapply_invoiceapply&action=toAdd&invoiceapply[objId]='
							+ row.id
							+ '&invoiceapply[objCode]='
			...
			action : function(row) {
				location = '?model=engineering_serviceContract_serviceContract&action=toChange&changer=changer&id='
						+ row.id + "&skey=" + row['skey_'];
			}
		}, {
...
//				},
			...
			action : function(row) {
				showOpenWin('?model=projectmanagent_return_return&action=toAdd&orderId='
						+ row.id + "&orderType=oa_sale_service");
			}
		},{
