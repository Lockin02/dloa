...
function initWorklog() {
    ...
    if (objGrid.children().length == 0) {
        objGrid.yxeditgrid({
            url: '?model=engineering_worklog_esmworklog&action=warnSummary',
            type: 'view',
            param: paramObj,
            event: {
                ...
                },
                {
                    ...
                    process: function (v, row) {
                        return "<a href='javascript:void(0)' " +
                            "onclick='window.open(\"?model=engineering_worklog_esmworklog&action=toWarnView&t=4&year=" + year + "&month="
                            + month + "&feeDeptId=" + feeDeptId + "&createId=" + row.createId + "\")'>" + v + "</a>";
                    }
                }
            ]
        });
