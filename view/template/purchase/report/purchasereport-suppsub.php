...
select
	...
	if( gs.arrivalPay > gs.prePay , '货到付款','预付款' ) as paymentCondition
from
	(
	select
		sum(ae.moneyAll) as moneyAll,date_format(ab.createTime,'%Y%m') as thisYearMonth,$year as thisYear,
		ab.suppId,ab.suppName,
		sum(if(ab.paymentConditionName = '货到付款',ab.allMoney,0 )) as arrivalPay,
		sum(if(ab.paymentConditionName = '预付款',ab.allMoney,0 )) as prePay
	from
