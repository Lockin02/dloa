...
function search(){
	...
	location.href = "?model=asset_report_assetreport&action=toAgencySummary"
...
function OnContentCellDblClick(Sender){
	...
	if(useStatusName == "资产状态" || useStatusName == "总计" || useStatusName == ""){
		useStatusName = "总计(区域汇总)";
	}
	...
	if(assetName == ""){
		if($("#agencyCodeHidden").val() != 'all'){
			...
			url = "?model=asset_report_assetreport&action=toDetail"
				...
				+ "&deptId=" +  $("#deptId").val()
				+ "&deptName=" + $("#deptName").val()
				+ "&agencyCodeStr=" + $("#agencyCodeStr").val()
				+ "&useStatusName=" + useStatusName;
		}
	}else{
		url = "?model=asset_report_assetreport&action=toDetail"
