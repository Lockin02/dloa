...
(function($) {
	...
	function init(target) {
		...
		//add by chengl 2012-05-08
		var colArr = $("#portal").data("colArr");
		var table = $('<table border="0" cellspacing="0" cellpadding="0"><tr></tr></table>')
				.appendTo(target);
		...
		$(target).find('>div').each(function() { // each column panel
					...
					var td = $('<td class="portal-column-td"></td>')
							.appendTo(tr)
		...
		for (var i = 0; i < columnWidths.length; i++) {
			//columnWidths[i] /= (totalWidth* colArr[i]);
			columnWidths[i] = colArr[i];
		}
	...
	function makeDraggable(target, panel) {
		...
		panel.panel('panel').draggable({
			...
			proxy : function(source) {
				var p = $('<div class="portal-proxy">proxy</div>')
						.insertAfter(source);
		 ...
		 */
		 function change(_158,_159){
			...
			if(opts.fit==true){
			var p=_15a.parent().parent().parent().parent();
			opts.width=p.width();
			opts.height=p.height();
			}
			...
			if ($.boxModel == true) {
				...
				if (height) {
					spacer.height(height
							- (spacer.outerHeight() - spacer.height()));
				}
			} else {
				spacer.width(width);
				if (height) {
	...
	$.fn.portal.methods = {
		...
		add : function(jq, param) { // param: {panel,columnIndex}
			return jq.each(function() {
						var c = $(this).find('div.portal-column:eq('
								+ param.columnIndex + ')');
