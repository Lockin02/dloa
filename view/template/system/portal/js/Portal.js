...
mini.extend(mini.ux.Portal, mini.Control, {
	...
	destroy : function() {
		...
		mini.ux.Portal.superclass.destroy.call(this);
	},
	doLayout : function() {
		if (!this.canLayout())
			return;
	...
	setColumns : function(columns) {
		...
		for (var i = 0, l = columns.length; i < l; i++) {
			...
			if (mini.isNumber(c))
				c += "px";
			sb += '<td id="' + i + '" class="mini-portal-column" style="width:'
					+ c + '"></td>';
		}
		sb += '</tr></table>';
	...
	/*
	 * column, id title, iconCls, showCloseButton, showMaxButton, showMinButton
	 * width, height allowDrag
	 */
	...
	addPanel : function(panel) {
		if (!panel)
			return;
		var dpanel = panel;
		if (mini.isNumber(panel.column) == false)
			panel.column = 0;
		...
		// add by chengl
		for (var n in dpanel) {
			// alert(dpanel['id'] + ":"+n+"==>" + dpanel[n]);
			$("#"+dpanel['id']).data(n, dpanel[n]);
		}
		return panel;
	},
	// /////////////////////////////////////////
	getColumnIndexByXY : function(x, y) {
		...
		for (var i = 0, l = columnsBox.length; i < l; i++) {
			...
			if (elbox.x <= x && x <= elbox.right && elbox.y <= y
					&& y <= elbox.bottom) {
	...
	_getPanelByY : function(y, column, noPanel) {
		for (var i = 0, l = this.panels.length; i < l; i++) {
			...
			if (panel.column != column || panel == noPanel)
				continue;
	...
	__OnMouseDown : function(e) {
		...
		if (t) {
			...
			var sf = this;
			if (this.allowDrag && panel.allowDrag
					&& mini.isAncestor(panel.headerEl, e.target)
				...
				var drag = new mini.Drag({
					...
					onStart : function() {
						...
						jQuery(panel.el)
								.before('<div class="mini-portal-proxy"></div>')[0];
					...
					onMove : function(drag) {
						...
						var x = drag.now[0] - drag.init[0], y = drag.now[1]
								- drag.init[1];
						...
						if (column != -1) {
							...
							// if (tp) document.title = tp.title + ":" +
							// tp.__moveAction + ":" + column + ":" + new
							// Date().getTime();
							// else {
							// tp = null;
							// }
					...
					onStop : function() {
						...
						// add by chengl 2012-05-04 触发拖动释放事件
						$(panel.el)
								.trigger('stop', [panel, targetPanel, index]);
