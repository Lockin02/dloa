        ...
        colModel: [{
            ...
            process: function (v, row) {
                switch (v) {
                    ...
                    case '2' :
                        return '<img title="已生效" src="images/icon/ok2.png" style="width:15px;height:15px;">';
                    case '3' :
...
//用于列表进度显示
function formatProgress(v, row) {
    if (v) {
        var rate = accMul(accDiv(row.hookMoney, v, 4), 100, 2);
        return '<div style="width:auto;height:auto;border:1px solid #ccc;padding:0px;" title="'
        + "分摊金额：【"
        + moneyFormat2(row.costMoney)
        + "】，已勾稽金额：【"
        + moneyFormat2(row.hookMoney)
        + "】，未勾稽金额：【"
        + moneyFormat2(row.unHookMoney)
        + "】"
        + '">'
        + '<div style="width:'
        + rate
