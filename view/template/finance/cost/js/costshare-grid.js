...
$(function () {
    ...
    $("#costshareGrid").yxgrid({
        ...
        param: {auditStatusArr: '1,2,3', auditStatusNo: 0},
        title: '分摊明细列表',
        isOpButton: false,
        isAddAction: false,
        isEditAction: false,
        ...
        buttonsEx: [{
            ...
            action: function (row, rows) {
                if (row) {
                    var canAuditArr = [];
                    var hasOverPeriod = false;
                    for (var i = 0; i < rows.length; i++) {
                        if (checkPeriodLarge(rows[i].inPeriod) == false) {
                            hasOverPeriod = true;
                        }
                    ...
                    if (canAuditArr.length > 0) {
                        var confirmRel = hasOverPeriod? '含有大于当前周期分摊明细，是否继续进行审核操作?' : '确认将选中的记录审核吗？';
                        if (confirm(confirmRel)) {
                            $.ajax({
            ...
            action: function (row, rows) {
                if (row) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        if (rows[i].auditStatus == "2") {
                            var confirmRel = false;
                            if(hasOverPeriod){
                                confirmRel = confirm('含有大于当前周期分摊明细，是否继续进行审核操作?');
                            }else{
                                var chkRsl = checkHasOverPeriod(rows[i].objId);
                                confirmRel = (chkRsl)? confirm('['+rows[i].objCode+'] 中含有大于当前周期分摊明细，是否继续进行审核操作?') : true;
                            }
...
function checkPeriod(period) {
    ...
    var thisPeriodYear = parseInt(periodArr[0]);
    var thisPeriodMonth = parseInt(periodArr[1]);
    ...
    if (!periodYear) {
        $.ajax({
            ...
            success: function (data) {
                data = eval("(" + data + ")");
                periodYear = parseInt(data.thisYear);
                periodMonth = parseInt(data.thisMonth);
            }
        });
    }
    return periodYear < thisPeriodYear ||
        (periodYear == thisPeriodYear && periodMonth <= thisPeriodMonth);
}

function checkPeriodLarge(period) {
    var periodArr = period.split('.');
    var thisPeriodYear = parseInt(periodArr[0]);
    var thisPeriodMonth = parseInt(periodArr[1]);
...
function checkHasOverPeriod(objId){
    var responseText = $.ajax({
        url:'index1.php?model=finance_cost_costshare&action=checkHasOverPeriod',
        data : {"objId" : objId},
        type : "POST",
        async : false
    }).responseText;
    if(responseText != "no"){
        var periodsObj = responseText.split(",");
        var chkRel = false;
        $.each(periodsObj,function(i,period){
            if(checkPeriodLarge(period) == false && !chkRel){
                chkRel = true;
            };
        });
        return chkRel;
