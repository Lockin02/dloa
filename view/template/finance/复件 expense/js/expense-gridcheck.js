...
//						case '0' : return '旧报销单';break;
		...
		toEditConfig : {
			showMenuFn : function(row){
				if( row.isNew == '1' && row.Status == '部门检查' && row.ExaStatus == '编辑' && checkLimit == "1"){
					return true;
				}
				return false;
			},
		...
		menusEx : [{
				...
				icon: 'edit',
				showMenuFn : function(row){
					if( row.isNotReced == '1' && row.Status != "编辑" && checkLimit == "1"){
						return true;
					}
					return false;
				},
				...
				icon: 'edit',
				showMenuFn : function(row){
					if( row.isNotReced == '0' && row.isHandUp == "0" && checkLimit == "1"){
						return true;
					}
					return false;
				},
				...
				icon: 'edit',
				showMenuFn : function(row){
				if( row.isNew == '1' && row.Status == '部门检查' && row.ExaStatus == '打回' && checkLimit == "1"){
						return true;
					}
					return false;
				},
				...
				icon: 'edit',
				showMenuFn : function(row){
					if( row.isNew == '1' && row.Status == '部门检查' && checkLimit == "1"){
						return true;
					}
					return false;
				},
				...
				icon: 'delete',
				showMenuFn : function(row){
					if((row.Status == '打回' || row.Status == '部门检查') &&　checkLimit == "1"){
						return true;
					}
					return false;
				},
				...
				icon: 'print',
				showMenuFn : function(row){
					if(row.Status != '编辑' && row.Status != '打回' && row.Status != '部门检查'　&& checkLimit == "1"){
						return true;
					}
					return false;
				},
