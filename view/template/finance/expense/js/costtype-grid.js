$(function() {
    ...
    // 数据字典获取
    var dataDict = getData('YSLX');
    var dataHash = {};
    for (var i = 0; i < dataDict.length; i++) {
        dataHash[dataDict[i].dataCode] = dataDict[i].dataName;
    }
...
function append() {
    if ($("#isNewHidden").val() == "0") {
        ...
        var node = $('#costTypeGrid').treegrid('getSelected');
        if (node) {
            showThickboxWin("?model=finance_expense_costtype&action=toAdd&ParentCostTypeID=" + node.id +
                "&ParentCostType=" + node.CostTypeName + "&placeValuesBeforeTB_=savedValues&TB_iframe=true&modal=false&height=400&width=800");
        } else {
