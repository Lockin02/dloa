...
$(function() {
	...
	var thisAction; //数据源
	if (checkLimit == "1") {//如果有权限，则进去检查数据源
		thisAction = "pageJson";
	} else {
...
//						case '0' : return '旧报销单';break;
		...
		toEditConfig: {
			showMenuFn: function(row) {
				return row.isNew == '1' && row.Status == '部门检查' && row.ExaStatus == '编辑' && checkLimit == "1";
			},
		...
		menusEx: [{
			...
			icon: 'edit',
			showMenuFn: function(row) {
				return row.isNotReced == '1' && row.Status != "编辑" && checkLimit == "1";
			},
			action: function(row) {
				if (confirm('确认收单吗？')) {
					$.ajax({
			...
			icon: 'edit',
			showMenuFn: function(row) {
				return row.isNew == '1' && row.Status == '部门检查' && row.ExaStatus == '打回' && checkLimit == "1";
			},
			action: function(row) {
			...
			icon: 'edit',
			showMenuFn: function(row) {
				return row.isNew == '1' && row.Status == '部门检查' && checkLimit == "1";
			},
			action: function(row) {
				if (row.CheckAmount != row.Amount) {
					alert('单据金额发生变更，检查前单据金额为: ' + row.Amount + ',检查后单据金额为:' + row.CheckAmount + ',请重新确认此报销单');
					return false;
				}
			...
			icon: 'delete',
			showMenuFn: function(row) {
				return (row.Status == '打回' || row.Status == '部门检查') && checkLimit == "1";
			},
			action: function(row) {
			...
			icon: 'print',
			showMenuFn: function(row) {
				return row.Status != '编辑' && row.Status != '打回' && row.Status != '部门检查' && checkLimit == "1";
			},
			action: function(row) {
