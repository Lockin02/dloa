...
function newProjectTreeGrid() {
			 ...
			 */
	{
		...
		render:function(v){
			if(v!=undefined){
				...
				return "<img src='images/icon/ico6.gif' title='里程碑计划'/>里程碑计划";
			}
		}
	}, {
		header : "计划开始日期",
	...
	var content = {
		...
		hidddenProperties : ['name', 'projectId', 'projectName', 'planName','status','purviewKey'],// 用于隐藏在一行中的属性,适合传递值的一种方式.
		iconShowIndex : 3,
		contextMenu : {
			// width : 150,
			// alias:"1-1",
			items : [{
				...
				, {
					...
					action : function(row) {
						if (showMenuFnB(row)) {
							showThickboxWin('?model=rdproject_plan_rdplan&action=toImport&planId='
								+ row.pid
								+ "&planName="
								+ row.planName
								+ "&projectId="
				...
				action : function(row) {
						showThickboxWin('?model=rdproject_milestone_rdmilestone&action=rmListCenter&pjId='
								+ row.projectId
				...
				action : function(row) {
					if(showMenuFnR(row)){
						location='?model=rdproject_task_rdtask&action=toPlanTaskPage&pnId='
							+ row.pid
							+ "&pnName="
							+ row.planName
							+ "&pjId="
						...
						location='?model=rdproject_task_rdtask&action=toPlanTaskPage&viewTask=1&pnId='
							+ row.pid
							+ "&pnName="
							+ row.planName
							+ "&pjId="
			...
			, {
				...
				action : function(row) {
					if(!showMenuFnB(row)){
						showOpenWin('?model=rdproject_project_rdproject&action=rpRead'
							+ "&pjId="
							+ row.projectId
							+ '&placeValuesBefore&TB_iframe=true&modal=false&height=450&width=950');
					}else{
						showThickboxWin('?model=rdproject_plan_rdplan&action=view'
							+ "&pnId="
							+ row.pid
							+ "&pnName="
							+ row.planName
				...
				action : function(row) {
					showThickboxWin('?model=rdproject_plan_rdplan&action=toEdit&pnId='
						+ row.pid
						+ "&pjId="
						+ row.projectId
						+ '&type=myPlan&placeValuesBefore&TB_iframe=true&modal=false&height=450&width=750');
				...
				showMenuFn : showMenuFnA,
				action : function(row) {
					showThickboxWin('?model=rdproject_plan_rdplan&action=delectPlan&pnId='
						+ row.pid
						+ '&type=myPlan&placeValuesBefore&TB_iframe=true&modal=false&height=150&width=300');
				}
					// TODO:
				}, {
				text : "保存为模板",
				...
				showMenuFn : showMenuFnA,
				action : function(row){
					showThickboxWin('?model=rdproject_template_rdtplan&action=toAdd&pnId='
						+ row.pid
						+ "&pnName="
						+ row.planName
						+ "&pjId="
