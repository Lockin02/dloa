		...
		var $row=$(obj.parentNode.parentNode);
			rowLength = $("tr[parentRowId="+$row.find("input[id^='isCon']").val()+"]").size();
			  for(rowL=rowNo+1;rowL<=rowNo+rowLength;rowL++){
			     $("tr[parentRowId="+$row.find("input[id^='isCon']").val()+"]").append("<input type='text' name='rdproject[rdprojectequ]["+ rowL +"][isDel]' value='1'>");
...
function mydel(obj, mytable) {
	if (confirm('确定要删除该行？')) {
		...
		if($row.find("input[id^='isConfig']").val() == ''){
			rowLength = $("tr[parentRowId="+$row.find("input[id^='isCon']").val()+"]").size();
			  for(rowL=rowNo+1;rowL<=rowNo+rowLength;rowL++){
			     $("tr[parentRowId="+$row.find("input[id^='isCon']").val()+"]").append("<input type='hidden' name='rdproject[rdprojectequ]["+ rowL +"][isDel]' value='1'>");
		         $("tr[parentRowId="+$row.find("input[id^='isCon']").val()+"]").hide();
			  }
			}
		$row.append("<input type='text' name='rdproject[rdprojectequ]["+ rowNo +"][isDel]' value='1'>");
		$row.hide();
					...
					'row_dblclick' : function(i){
						return function(e, row, data) {
							...
							if(isDelId != productId){
	                            $("#invbody").append("<input type='hidden' name='rdproject[rdprojectequ]["+ i +"][isEdit]' value='1'>");
							}
							$("#number" + i).val(1);
...
function dynamic_add(packinglist, countNumP) {
	...
	oTL1.innerHTML = "<input type='text' id='productNo" + mycount + "' class='txtmiddle' name='rdproject[rdprojectequ][" + mycount + "][productNo]' readonly>"+
	                 ...
	                 "<input type='hidden' id='isAdd"+mycount+"' name='rdproject[rdprojectequ]["+mycount+"][isAdd]' value='1'>";
      // 单选产品
	...
	oTL3.innerHTML = "<input id='productModel" + mycount + "' type='text' class='txtmiddle' name='rdproject[rdprojectequ][" + mycount + "][productModel]' readonly>";
	...
	oTL10.innerHTML = "<img src='images/closeDiv.gif' onclick='mydelT(this,\"" + packinglist.id + "\")' title='删除行' id='Del"+mycount+"'>";
