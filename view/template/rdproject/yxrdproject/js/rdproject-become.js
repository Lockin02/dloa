...
function myedit(obj, mytable) {
	...
	rowLength = $("tr[parentRowId=" + $row.find("input[id^='isCon']").val()
			+ "]").size();
	for (rowL = rowNo + 1; rowL <= rowNo + rowLength; rowL++) {
		$("tr[parentRowId=" + $row.find("input[id^='isCon']").val() + "]")
				.append("<input type='text' name='rdproject[rdprojectequ]["
						+ rowL + "][isDel]' value='1'>");
		$("tr[parentRowId=" + $row.find("input[id^='isCon']").val() + "]")
				.hide();
	}
...
function mydelC(obj, mytable) {
	if (confirm('确定要删除该行？')) {
		...
		if ($row.find("input[id^='isConfig']").val() == '') {
			rowLength = $("tr[parentRowId="
					+ $row.find("input[id^='isCon']").val() + "]").size();
			for (rowL = rowNo + 1; rowL <= rowNo + rowLength; rowL++) {
				$("tr[parentRowId=" + $row.find("input[id^='isCon']").val()
						+ "]")
						.append("<input type='hidden' name='rdproject[rdprojectequ]["
								+ rowL + "][isDel]' value='1'>");
				$("tr[parentRowId=" + $row.find("input[id^='isCon']").val()
						+ "]").hide();
			}
		}
		$row.append("<input type='text' name='rdproject[rdprojectequ][" + rowNo
				+ "][isDel]' value='1'>");
		$row.hide();
...
function dynamic_add(packinglist, countNumP) {
	...
	oTL1.innerHTML = "<input type='text' id='productNo" + mycount
			...
			+ "][unitName]'>" + "<input type='hidden' id='isAdd" + mycount
			+ "' name='rdproject[rdprojectequ][" + mycount
			+ "][isAdd]' value='1'>";
	// 单选产品
		...
		gridOptions : {
			...
			event : {
				'row_dblclick' : function(mycount) {
					return function(e, row, data) {
						myeditT(document.getElementById("Del" + mycount),
								"invbody");
	...
	oTL10.innerHTML = "<img src='images/closeDiv.gif' onclick='mydelT(this,\""
