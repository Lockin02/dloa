...
$(function() {
		...
		buttonsEx : [{
			...
			icon : 'excel',
			action : function(row) {
				showThickboxWin("?model=stockup_application_application&action=toExportList"
		...
		menusEx: [/*{
			...
			action: function(row) {
				if(row.productNum > 100){
					showThickboxWin('controller/stockup/application/ewf_index.php?actTo=ewfSelect&billId=' + row.id + "&flowMoney=6&billDept="+ row.appDeptId+ "&placeValuesBeforeTB_=savedValues&TB_iframe=true&modal=false&height=500&width=750");
				}else{
					showThickboxWin('controller/stockup/application/ewf_index.php?actTo=ewfSelect&billId=' + row.id + "&flowMoney=3&billDept="+ row.appDeptId+ "&placeValuesBeforeTB_=savedValues&TB_iframe=true&modal=false&height=500&width=750");
				}
			}
        },{
            text: '删除',
