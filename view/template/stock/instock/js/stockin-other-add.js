...
function confirmAudit(act) {// 审核确认
    if(act == "save"){
        if(checkForm() && chkAvailableEquNum()){
            $("#form1").attr("action","?model=stock_instock_stockin&action=add");
            $("#form1").submit();
        }
    }else{
...
function chkAvailableEquNum(){
    ...
    if(resultObj.result != "ok" && resultObj.msg != ""){
        var alertMsg = "下推的物料: "+resultObj.msg+" 已超出可下推数量,请先检查并处理未审核的入库单。";
        alert(alertMsg);
        return false;
    }else{
        return true;
...
function checkSerioNum() {
    ...
    var itemscount = $("#itemscount").val();
    for (var i = 0; i < itemscount; i++) {
        var serialSequenceObj = $("#serialSequence" + i);
        if (serialSequenceObj.length == 0) {
            continue;
        }

        var serialnoNameStr = serialSequenceObj.val();
...
function checkRelDocType() {// 根据红篮色单控制源单类型
    ...
    if ($("#isRed").val() == 0) {
        ...
        $(".main_head_title").html('<font color="blue">其他入库单</font>');
        var slOption = false;
        ...
        $(".main_head_title").html('<font color="red">其他入库单</font>');
        var tlOption = false;
        $("#relDocType option").each(function () {
            if ($(this).val() == "RQTRK") {
                tlOption = true;
        ...
        if (!tlOption) {
            $("#relDocType").append("<option value='RQTRK'>其他入库</option>");
        }
    }
}
/**
    ...
    if (itemscount < 1) {
        ...
        for (var i = 0; i < itemscount; i++) {
            if ($("#isDelTag" + i).val() != 1 && $("#productId" + i).length == 1) {
                ...
                if ($("#relDocType").val() == "RHHRK" || $("#relDocType").val() == "RZCRK" && $("#isRed").val() == "0") {//用于换货蓝色入库通知或资产蓝色入库
                    if (actNum.val()*1 > $("#storageNum" + i).val()*1) {
                        alert("实收数量不能大于应收数量");
                        actNum.focus();
                        return false;
                    }
                    if($("#relDocType").val() == "RZCRK" && ($("#productCode" + i).val() == '' 
                    	|| $("#productName" + i).val() == '')){//资产入库时，验证是否有未确认的物料
                        alert("存在未确认的物料，请确认后再进行入库");
                        return false; 	
                    }
                }
            }
