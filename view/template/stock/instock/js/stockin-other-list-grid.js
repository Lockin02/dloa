    ...
    $("#purhasestorageGrid").yxsubgrid({
            ...
            },
            {
                ...
                showMenuFn: function (row) {
                    if (row.docStatus == "YSH" && row.isRed == "0") {
                        //禁用资产入库蓝字单的下推红色单, （禁用换货入库通知单蓝字单的下推红色单, 系统没对换货入库通知单关联的红单处理,下推后会导致物料执行数量叠加,暂时禁掉先）
                        if (row.relDocType == "RZCRK" || row.relDocType == "RHHRK")
                            return false;
                        return true;
                    } else {
                        return false;
            ...
            },
            {
                ...
                showMenuFn: function (row) {
                    if (row.docStatus == "YSH") {
                        //禁用资产入库红字单的反审核
                        if (row.relDocType == "RZCRK" && row.isRed == "1")
                            return false;

                        if (row.relDocType == "RXSTH")
                            return true;
        ...
        colModel: [
            ...
            },
            {
                ...
                display: '源单编号',
                process: function (v, row) {
                    if (row.relDocType == 'RQTCK') {
                        return "<a href='javascript:void(0)' onclick='showThickboxWin(\"?model=stock_withdraw_innotice&action=toView&id" + row.relDocId
                            + "&placeValuesBeforeTB_=savedValues&TB_iframe=true&modal=false&height=500&width=850" + "\")'>" + v + "</a>";
                    } else if (row.relDocType == 'RQTRK') {
                        return "<a href='javascript:void(0)' onclick='showThickboxWin(\"?model=stock_withdraw_innotice&action=toView&id" + row.relDocId
                            + "&placeValuesBeforeTB_=savedValues&TB_iframe=true&modal=false&height=500&width=850" + "\")'>" + v + "</a>";
                    } else if (row.relDocType == 'RZCRK') {
                        return "<a href='javascript:void(0)' onclick='showThickboxWin(\"?model=asset_require_requireout&action=toView&id=" + row.relDocId
                            + "&placeValuesBeforeTB_=savedValues&TB_iframe=true&modal=false&height=500&width=850" + "\")'>" + v + "</a>";
                    } else if (row.relDocType == 'RHHRK') {
                        return "<a href='javascript:void(0)' onclick='showThickboxWin(\"?model=stock_withdraw_innotice&action=toView&id=" + row.relDocId
                            + "&placeValuesBeforeTB_=savedValues&TB_iframe=true&modal=false&height=500&width=850" + "\")'>" + v + "</a>";
                    }
                },
