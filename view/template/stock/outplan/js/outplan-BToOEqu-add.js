	...
	if( in_array(docType,arrayObj) ){
		$.post(
			"?model=common_contract_allsource&action=getBorrwoToOrderequ",
			{
				...
				if(!strTrim(data)){ // 没有物料时,strTrim(去掉空格)
					$("#BToOEqu").append("");
					$("#borrowTr").hide();
				}else {
	               var $html = $("<table class='form_main_table' id='borrowTable'>" +
	    		               "<thead><tr align='left'><td colspan='9'>借试用转销售产品清单   <img src='images/verpic_open.gif' onclick=\"dis('borrowbody')\" title='缩放' /></td></tr></thead>" +
