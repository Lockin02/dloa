...
//		}, {
		...
		menusEx : [{
			...
			action : function(row) {
				showThickboxWin('?model=stock_outplan_outplan&action=toUploadFile&id='
					+ row.id
					+ '&type=stock_outplan_outplan_filing'// 发货计划归档文件
			...
			icon : 'delete',
			showMenuFn : function(row) {
				if (row.changeTips == 1 && row.isNeedConfirm == 0) {
					return true;
				}
				return false;
			},
			...
			showMenuFn : function(row) {
				if (row.docStatus != 'YGB' && row.docStatus != 'YWC'
						&& row.issuedStatus == 0 && row.isNeedConfirm == 0) {
					return true;
				}
				return false;
			},
			...
			icon : 'add',
			showMenuFn : function(row) {
				if (row.issuedStatus != 1 && row.docStatus != 'YGB' && row.isNeedConfirm == 0) {
					return true;
				}
				return false;
			},
			...
			showMenuFn : function(row) {
				if (row.issuedStatus == 1 && row.docStatus != 'YGB'
						&& row.status != 'YZX' && row.isNeedConfirm == 0) {
					return true;
				}
				return false;
			},
			action : function(row) {
				var tip = DateDiff(row.shipPlanDate,formatDate(new Date()));
				if(tip > 0){
				   alert("当前已比计划发货日期【"+row.shipPlanDate+"】晚 "+tip+" 天，请先填写计划反馈单")
			...
			showMenuFn : function(row) {
				if (row.issuedStatus == '1' && row.isNeedConfirm == 0) {
					return true;
				}
				return false;
			},
			...
			icon : 'edit',
			showMenuFn : function(row) {
				if (row.dongleRate == 0 && row.docStatus != 'YGB' && row.isNeedConfirm == 0) {
					return true;
				}
				return false;
			},
			...
			icon : 'add',
			showMenuFn : function(row) {
				if (row.dongleRate == 1 && row.isNeedConfirm == 0) {
					return true;
				}
				return false;
			},
