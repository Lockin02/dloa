...
$(function() {
	$("#withdrawGrid").yxsubgrid({
		...
		colModel : [{
			...
			process : function(v) {
				switch(v){
					...
					case '2' : return '<img src="images/icon/cicle_green.png" title="已完成赔偿单"/>';break;
				}
			},
			width : 50
		}, {
			...
			process : function(v) {
				switch(v){
					case 'YZX' : return '已处理';break;
					case 'BFZX' : return '正在处理';break;
					case 'WZX' : return '待处理';break;
					default : return '待处理';
				}
			},
			width : 80,
		...
		menusEx : [{
			...
			icon : 'add',
			showMenuFn : function(row) {
				if (row.docStatus == "1") {//如果是设备遗失，直接确认
					return true;
				}
				return false;
			},
			...
			icon : 'delete',
			showMenuFn : function(row) {
				if (row.docStatus == "1") {//设备遗失不能进行此操作
					return true;
				}
				return false;
			},
