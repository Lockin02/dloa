...
$(function() {
	$("#innoticeGrid").yxsubgrid({
		...
		param : {docType : 'oa_contract_exchange'},
		showcheckbox : false,
		isAddAction : false,
		isEditAction : false,
		isViewAction : true,
		...
		menusEx : [{
			...
			action : function(row, rows, grid) {
				if (row) {
					// alert()
					showModalWin("index1.php?model=stock_instock_stockin&action=toBluePush&docType=RKOTHER&relDocType=RHHRK&relDocId="
						+ row.id + "&relDocCode=" + row.noticeCode,1,row.id);
				} else {
					alert("请选中一条数据");
				}
			}
