    ...
    $("#stockoutGrid").yxsubgrid({
        ...
        menusEx: [{
            ...
            action: function (row, rows, grid) {
                //用于资产出库验证
                if (row.relDocType == 'QTCKZCCK' && isNum(row.relDocId) && isCardExist(row.relDocId)) {
                    alert("存在已生成的卡片，不允许反审核！");
                    return false;
                } else if (row.docType == 'CKOTHER' && row.toUse == 'CHUKUDLBF' && row.stockName == '呆料仓库' && row.isRed == 0) {
                    alert("呆料报废入库单已审核，不允许反审核！");
                    return false;
                }
...
//用于资产出库-验证该出库单对应的需求下是否存在已生成的资产卡片
