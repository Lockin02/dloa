		...
		for (var i = 0; i < itemscount; i++) {
			if ($("#isDelTag" + i).val() != 1) {
                ...
                if (!isNum(actOutNum.val())) {// 判断数量
                    alert("数量" + "<" + actOutNum.val() + ">" + "填写有误!");
                    actOutNum.focus();
                    return false;
                }
                if($("#relDocType").val() == "LLCKSCRWD" && $("#relDocId" + i).val() != '' && $("#isRed").val() == "0"){//用于生产任务单蓝色出库
                	var num = 0;
                    $.ajax({//获取该物料实际可出库数量
                        type: "POST",
                        async: false,
                        url: "?model=produce_plan_pickingitem&action=getNumAtOutStock",
                        data: {
                            "id": $("#relDocId" + i).val()
                        },
                    ...
                    if(num <= 0){
                        alert("名称为" + "<" + $("#productName" + i).val() + ">" + "的物料已出库完毕，无须再次提交");
                        return false;
                    }
                    if(actOutNum.val()*1 > num){
                        alert("名称为" + "<" + $("#productName" + i).val() + ">" + "的物料的数量不能超过" + num);
                        actOutNum.focus();
                        return false;
                    }
                }
		...
		$.ajax({// 缓存序列号
			...
			success : function(result) {
				...
				for(var i=0;i<result.length;i++){
					...
					if(stockName == result[i]['stockName']){
