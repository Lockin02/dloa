...
function checkRelDocType() {// 根据红篮色单控制源单类型
    ...
    if ($("#isRed").val() == 0) {
        ...
        $(".main_head_title").html('<font color="blue">销售出库单</font>');
        $("#packbody").css("color", "blue");
        var slOption = false;
        $("#relDocType option").each(function () {
            if ($(this).val() == "XSCKXSCK"
                || $(this).val() == "XSCKTHTZ") {
                $(this).remove();
            }
            if ($(this).val() == "XSCKFHJH") {
                slOption = true;
            }
        });
        if (!slOption) {
            $("#relDocType").append("<option value='XSCKFHD'>发货单</option>")
            $("#relDocType").append("<option value='XSCKFHJH'>发货计划</option>")
        }

    } else {
        $("#itembody").css("color", "red");
        $(".main_head_title").html('<font color="red">销售出库单</font>');
        $("#packbody").css("color", "red");
        var tlOption = false;
        $("#relDocType option").each(function () {
            if ($(this).val() == "XSCKFHD") {
                $(this).remove();
        ...
        if (!tlOption) {
            $("#relDocType").append("<option value='XSCKXSCK'>销售出库</option>");
            $("#relDocType").append("<option value='XSCKTHTZ'>退货通知</option>");
        }
    }
}
...
function addItems() {
    ...
    oTL12.innerHTML = '******<input type="hidden" name="stockout[items][' + mycount
    ...
    oTL15.innerHTML = '******<input type="hidden" name="stockout[items][' + mycount
    ...
    if (itemscount < 1) {
        ...
        for (var i = 0; i < itemscount; i++) {
            if ($("#isDelTag" + i).val() != 1) {
                ...
                if ($("#stockId" + i).val() == "" || parseInt($("#stockId" + i).val()) == 0) {
                    alert("发货仓库不能为空，请选择...");
                    return false;
                }
                if (audit) {
                    ...
                    if (!isNum(actOutNum.val())) {// 判断数量
                        alert("实发数量" + "<" + actOutNum.val() + ">" + "填写有误!");
                        actOutNum.focus();
                        return false;
                    }
                    if ($("#relDocType").val() == "XSCKTHTZ") {//用于退货通知
                        if (actOutNum.val() > $("#shouldOutNum" + i).val() || actOutNum.val() <= 0) {
                            alert("出库数量不能大于应发数量或小于等于0");
                            actOutNum.focus();
                            return false;
                        }
                    }
