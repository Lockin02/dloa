...
//							alert($(this).html());
		...
		menusEx : [{
			...
			action : function(row, rows, grid) {
				showThickboxWin("?model=stock_productinfo_productinfoAdd&action=init&methodType=confirm&id="
						+ row.id
						+ "&placeValuesBeforeTB_=savedValues&TB_iframe=true&modal=false&height=500&width=900");		
			/*	$.ajax({
					url:"?model=stock_productinfo_productinfoAdd&action=changeConfirm&id="+ row.id,
					type:'get',
					dataType:'json',
			...
			action : function(row, rows, grid) {
				$.ajax({
					type : "POST",
					async : false,
					url : "?model=stock_productinfo_productinfoAdd&action=ajaxBack",
					data : {
						id : row.id
					},
