$(function() {
	...
	//单价锁定设置
	if ($("#priceLock1").val() == "1") {
		$("#priceLock").attr("checked", 'checked');
	}
	if ($("#isRelated").val() == 1) {
	...
	//是否加密锁
	if ($("#encryptionLock1").val() == "1") {
		$("#encryptionLock").attr("checked", 'checked');
	}
	/**
	 * /** 验证信息
	 */
	...
	// 产品选择
	var relGoodsNameObj = $("#relGoodsName");
	var relGoodsProNameObj = $("#relGoodsProName");
	relGoodsNameObj.yxcombogrid_goods({
		hiddenId: "relGoodsId",
		gridOptions: {
			showcheckbox: false,
			event: {
	...
	// 如果有关联的产品属性，则按此初始化
	if (relGoodsNameObj.val() != "") {
		relGoodsProNameObj.yxcombogrid_goodsproperties({
			hiddenId: "relGoodsPro",
			gridOptions: {
				param: {mainId: $("#relGoodsId").val()}
			}
		});
	}
...
function checkSubmit() {
    ...
    var priCost = $.trim(priCostObj.val()) * 1;
    if (priCost == '') {
        alert('物料成本不能为空');
        return false;
    }
