...
function dynamic_add(packinglist, countNumP) {
	...
	oTL3.innerHTML = "<input id='productModel" + mycount
			+ "' type='text' class='readOnlyTxtItem' name='rentalcontract[rentalcontractequ]["
	...
	oTL8.innerHTML = "<input type='hidden' id='LicenseId" + mycount + "' name='rentalcontract[rentalcontractequ][" + mycount + "][license]'/>" +
...
function mydel(obj, mytable, orderArray) {
	if (confirm('确定要删除该行？')) {
		...
		$row.append("<input type='hiden' name='rentalcontract["+ orderArray +"]["+ rowNo +"][isDel]' value='1'>");
		$row.hide();
