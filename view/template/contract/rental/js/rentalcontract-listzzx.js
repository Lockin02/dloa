...
var show_page = function(page) {
	$(".rentalcontractZzxGrid").yxgrid("reload");
};
$(function() {
	$(".rentalcontractZzxGrid").yxgrid({
		...
		menusEx : [{
			...
			action : function(row) {
				showOpenWin('?model=contract_rental_rentalcontract&action=toBecomeContract&id='
						+ row.id
						+ '&perm=edit'
						+ "&skey="
						+ row['skey_']
...
//			},
			action : function(row) {
				if (row.orderCode != "") {
					showModalWin('?model=finance_invoiceapply_invoiceapply&action=toAdd&invoiceapply[objId]='
							...
							+ row.orderCode + '&invoiceapply[objType]=KPRK-05');
				} else {
					showModalWin('?model=finance_invoiceapply_invoiceapply&action=toAdd&invoiceapply[objId]='
							+ row.id
							+ '&invoiceapply[objCode]='
			...
			action : function(row) {
				location = '?model=contract_rental_rentalcontract&action=toChange&changer=changer&id='
						+ row.id + "&skey=" + row['skey_'];
			}
		}, {
...
//				},
			...
			action : function(row) {
				showOpenWin('?model=projectmanagent_return_return&action=toAdd&orderId='
						+ row.id + "&orderType=oa_sale_lease");
			}
		},{
