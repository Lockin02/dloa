...
$(function () {
	$("#entryNoticeGrid").yxgrid({
		...
		colModel : [{
			...
			sortable : true,
			process : function(v,row){
					return "<a href='#' onclick='showOpenWin(\"?model=hr_recruitment_apply&action=toView&id=" + row.applyId +"\",1)'>" + v + "</a>";
			}
		}, {
			name : 'email',
			display : '电子邮箱',
		...
		buttonsEx : [ {
							...
							icon : 'excel',
							action : function(row) {
								showThickboxWin("?model=model_hr_recruitment_entryNotice&action=toExport&docType=RKPURCHASE"
		...
		toViewConfig : {
			toViewFn : function (p, g) {
				...
				if (rowObj) {
					...
					showOpenWin("?model=hr_recruitment_entryNotice&action=toView&id=" + rowData[p.keyField],'1');
				} else {
					alert('请选择一行记录！');
				}
			}
