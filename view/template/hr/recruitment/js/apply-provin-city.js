$(document).ready(function() {
	...
	$.ajax({
		...
		success : function(data){
			//初始化省份信息
			getProvinces(data);
			var provinceIdObj = $('#province');
			if (provinceIdObj.attr('val')) {
				provinceIdObj.val(provinceIdObj.attr('val'));
				provinceIdObj.trigger('change');
			}
			//初始化城市信息
			var cityIdObj = $('#city');
			if (cityIdObj.attr('val')) {
				cityIdObj.val(cityIdObj.attr('val'));
			}
		}
	});
});