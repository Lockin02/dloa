...
function toApp() {
	document.getElementById('form1').action = "index1.php?model=projectmanagent_order_order&action=chanceAdd&act=app";

}
function toSave() {
	document.getElementById('form1').action = "index1.php?model=projectmanagent_order_order&action=chanceAdd";
...
function initEqu(thisObj, tableId, orderId) {
	...
	for (var k = 0; k < objLength; k++) {
		rowNum++;
		i++;
		inStr = "<tr name='equTab_"
				+ orderId
				+ "'><td>"
				+ rowNum
				+ "</td>"
				+ "<td><input class='txtshort' type='text' name='order[orderequ]["
				+ i
				+ "][productNo]' id='EquId"
				+ i
				+ "' value='"
				+ thisObj[j].productNo
				+ "'/></td>"
				+ "<td><input class='txt' type='text' name='order[orderequ]["
				+ i
				...
				+ "'>"
				+ "<input type='hidden' name='order[orderequ]["
				+ i
				...
				+ i
				+ "' value='"
				+ thisObj[j].projArraDate
				+ "' onfocus='WdatePicker()'></td>"
				+ "<td><select class='txtshort'  name='order[orderequ]["
				+ i
				...
				+ "<td><input type='hidden' id='licenseId"
				+ i
				+ "]' name='order[orderequ]["
				+ i
				+ "]][license]' value='"
				+ thisObj[j].license
				+ "'/>"
				+ "<input type='button' class='txt_btn_a' value='配置' onclick='License(\"licenseId"
				+ i
				+ "\");'/>"
				+ "</td>"
				+ "<td><input type='checkbox'  name='order[orderequ]["
				+ i
				+ "][isSell]' checked='checked'></td>"
...
function link_add(mypay, countNum) {
	...
	oTL2.innerHTML = "<input class='txt' type='text' name='order[linkman]["
			+ mycount + "][telephone]' id='telephone" + mycount + "' onchange='tel(" + mycount + ")'/>";
	oTL3 = oTR.insertCell([3]);
	oTL3.innerHTML = "<input class='txt' type='text' name='order[linkman]["
			+ mycount + "][Email]' id='Email" + mycount + "' onchange='Email(" + mycount + ")'/>";
...
function dynamic_add(packinglist, countNumP) {
	...
	oTL1.innerHTML = "<input type='text' id='productNo" + mycount + "' class='txtshort' name='order[orderequ][" + mycount + "][productNo]' >" + "<input type='hidden' id='unitName" + mycount + "' name='order[ordereequ][" + mycount + "][unitName]'>";
	// 单选产品
