					...
					'row_dblclick' : function(i){
						return function(e, row, data) {
							...
							if(isDelId != productId){
	                            $("#invbody").append("<input type='hidden' name='borrow[borrowequ]["+ i +"][isEdit]' value='1'>");
							}
							$("#number" + i).val(1);
		...
		var $row=$(obj.parentNode.parentNode);
			rowLength = $("tr[parentRowId="+$row.find("input[id^='isCon']").val()+"]").size();
			  for(rowL=rowNo+1;rowL<=rowNo+rowLength;rowL++){
			     $("tr[parentRowId="+$row.find("input[id^='isCon']").val()+"]").append("<input type='text' name='borrow[borrowequ]["+ rowL +"][isDel]' value='1'>");
