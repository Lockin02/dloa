					...
					'row_dblclick' : function(i){
						return function(e, row, data) {
							...
							if(isDelId != productId){
	                            $("#invbody").append("<input type='text' name='borrow[borrowequ]["+ i +"][isEdit]' value='1'>");
							}
						}
					}(i)
				}
...
function dynamic_add(packinglist, countNumP) {
	...
	oTL1.innerHTML = "<input type='text' id='productNo" + mycount + "' class='txtmiddle' name='borrow[borrowequ][" + mycount + "][productNo]' >"+
	                 "<input type='hidden' id='isAdd"+mycount+"' name='borrow[borrowequ]["+mycount+"][isAdd]' value='1'>";
		// 单选产品
	...
	oTL4.innerHTML = "<input class='txtshort' type='text' name='borrow[borrowequ][" + mycount
	...
	oTL6.innerHTML = "<input class='txtshort' type='text' name='borrow[borrowequ][" + mycount
			+ "][price]' id='price" + mycount
			+ "' />";

	var oTL7 = oTR.insertCell([7]);
	oTL7.innerHTML = "<input class='txtshort' type='text' name='borrow[borrowequ][" + mycount
...
function mydel(obj, mytable) {
	if (confirm('确定要删除该行？')) {
		...
		$row.append("<input type='text' name='borrow[borrowequ]["+ rowNo +"][isDel]' value='1'>");
		$row.hide();
