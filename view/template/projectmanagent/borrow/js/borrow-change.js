...
$(function(){
	 ...
	 switch(SingleTypeT){
	     case "" :
		     ...
		     document.getElementById("SingleType").options.add(new Option("无源单","无源单"));
			 document.getElementById("SingleType").options.add(new Option("商机","商机"));
			 document.getElementById("SingleType").options.add(new Option("合同","合同"));
			 singleSelect();
			 break;
	     case "chance" :
	         document.getElementById("SingleType").options.length=0;
...
function singleSelect(){
    ...
    switch(SingleType){
         case "无源单" : $("#single").html("<input type='text' class='readOnlyText' readonly='readonly'>");
                        $("#SingleTypeT").val("");break;
         case "商机" : $("#single").html("<input type='text' class='txt' name='borrow[chanceCode]' id='chanceCode' >" +
         		                        "<input type='hidden' class='txt' name='borrow[chanceId]' id='chanceId'>");
         		              if(chanceCode != ''){$("#chanceCode").val(chanceCode);}
         		              if(chanceId != ''  ){$("#chanceId").val(chanceId);}
...
//		 case "合同" : $("#single").html("<input type='text' class='txt' name='borrow[contractNum]' id='contractNum'>" +
...
//         		           $("#contractNum").yxcombogrid_allorder({
//							hiddenId : 'id',
...
//									'row_dblclick' : function(e, row, data) {
//										if(data.orderCode == ''){
//											$("#contractNum").val(data.orderTempCode);
//										}else{
//											$("#contractNum").val(data.orderCode);
//										}
//										$("#contractName").val(data.orderName);
//										$("#contractId").val(data.orgid);
//										$("#customerName").val(data.customerName);
//										$("#customerId").val(data.customerId);
...
function dynamic_add(packinglist, countNumP) {
	...
	oTL1.innerHTML = "<input type='text' id='productNo" + mycount + "' class='txtmiddle' name='borrow[borrowequTemp][" + mycount + "][productNo]' >";
		// 单选产品
	...
	oTL2.innerHTML = "<input type='hidden' id='productId" + mycount + "'  name='borrow[borrowequTemp][" + mycount + "][productId]'/>" +
			"<input id='productName" + mycount + "' type='text' class='txt' name='borrow[borrowequTemp][" + mycount + "][productName]' />";
	...
	oTL3.innerHTML = "<input id='productModel" + mycount + "' type='text' class='txtmiddle' name='borrow[borrowequTemp][" + mycount + "][productModel]' readonly>";
    var oTL4 = oTR.insertCell([4]);
	oTL4.innerHTML = "<input class='txtshort' type='text' name='borrow[borrowequTemp][" + mycount + "][number]' id='number" + mycount + "'  />";
	var oTL5 = oTR.insertCell([5]);
	oTL5.innerHTML = "<input class='txtshort' type='text' name='borrow[borrowequTemp][" + mycount +"][unitName]' id='unitName"+ mycount +"' >"
	var oTL6 = oTR.insertCell([6]);
	oTL6.innerHTML = "<input class='txtshort' type='text' name='borrow[borrowequTemp][" + mycount + "][price]' id='price" + mycount + "' />";
	var oTL7 = oTR.insertCell([7]);
	oTL7.innerHTML = "<input class='txtshort' type='text' name='borrow[borrowequTemp][" + mycount + "][money]' id='money" + mycount + "' />";
	var oTL8 = oTR.insertCell([8]);
	oTL8.innerHTML = "<input type='text' class='txtshort' name='borrow[borrowequTemp][" + mycount + "][warrantyPeriod]' id='warrantyPeriod" + mycount + "'>" ;
	var oTL9 = oTR.insertCell([9]);
	oTL9.innerHTML = "<input type='hidden' id='licenseId" + mycount + "' name='borrow[borrowequTemp][" + mycount + "][License]'/>" +
			         "<input type='button' name='' class='txt_btn_a' value='配置' onclick='License(\"licenseId" + mycount + "\");'>"+
			         "<input type='hidden' name='borrow[borrowequTemp]["+ mycount +"][isCon]' id='isCon"+mycount+"'>"+
			         "<input type='hidden' name='borrow[borrowequTemp]["+ mycount +"][isConfig]' id='isConfig"+mycount+"'>";
	var oTL10 = oTR.insertCell([10]);
	oTL10.innerHTML = "<img src='images/closeDiv.gif' onclick='mydelC(this,\"" + packinglist.id + "\")' title='删除行' id='Del"+mycount+"'>";
...
function mydel(obj, mytable, array) {
	if (confirm('确定要删除该行？')) {
		...
		$row.append("<input type='hiden' name='borrow["+ array +"]["+ rowNo +"][isDel]' value='1'>");
		$row.hide();
