...
$(function() {
	...
	$("#rdprojectName").yxcombogrid_rdProject({// 研发项目combogrid
		hiddenId: 'rdprojectId',
		width: 600,
		event: {
		...
		gridOptions: {
			...
			event: {
				'row_dblclick': function(e, row, data) {
					$("#rdprojectCode").val(data.projectCode);
				}
			}
		}
	});
...
//判断是否显示 走发货计划按钮
$(function() {
	var isShipTip = $("#isShipTip").val();
	if (isShipTip == "1") {
		$("#isShipTipTR").html("<td class='form_text_left'><span class='red'>是否进入执行部走发货计划</span></td>" +

		"<td class='form_text_right' >" +

		"&nbsp&nbsp&nbsp<input type='radio' name='borrow[isShipTip]'  value='是' />是" +

		"&nbsp&nbsp&nbsp<input type='radio' name='borrow[isShipTip]'  value='否'  checked/>否" +
