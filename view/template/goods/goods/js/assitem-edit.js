...
function addItems() {
	...
	oTL2.innerHTML = '<select name="assitem[items][' + mycount
			+ '][columnName]" id="columnName' + mycount
			+ '" class="select" ></select>';
	var oTL3 = oTR.insertCell(3);
	oTL3.innerHTML = '<input type="text" name="assitem[items][' + mycount
			+ '][columnText]" id="columnText' + mycount + '" class="txt" />';
	var oTL4 = oTR.insertCell(4);
	oTL4.innerHTML = '<input type="text" name="assitem[items][' + mycount
...
function delItem(obj) {
	if (confirm('确定要删除该行？')) {
		...
		$(obj).parent().append('<input type="hidden" name="assitem[items]['
