...
function addItems() {
	...
	oTL3.innerHTML = '<input type="text" name="reduceapply[items][' + mycount
	...
	oTL5.innerHTML = '<input type="text" name="reduceapply[items][' + mycount
			+ '][pattern]" id="pattern' + mycount
			+ '" class="readOnlyTxtShort" readOnly/>';
	var oTL6 = oTR.insertCell(6);
	oTL6.innerHTML = '<input type="text" name="reduceapply[items][' + mycount
			+ '][unitName]" id="unitName' + mycount
			+ '" class="readOnlyTxtShort" readOnly/>';
	var oTL7 = oTR.insertCell(7);
	oTL7.innerHTML = '<input type="text" name="reduceapply[items][' + mycount
...
function confirmAudit() {
	if (confirm("你确定要提交审核吗?")) {
		if (checkForm()) {
			$("#form1").attr("action","?model=service_reduce_reduceapply&action=edit&actType=audit");
			$("#form1").submit();
		}
