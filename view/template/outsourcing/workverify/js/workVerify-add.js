...
//					type : 'txt'
 		...
 		function initTemplate(beginDate,endDate){
			$("#rentListInfo").yxeditgrid({
					objName : 'workVerify[rentList]',
					url : '?model=outsourcing_workverify_workVerify&action=worklogListJson',
					param : {
						beginDate :beginDate,
						endDate :endDate,
						dir : 'ASC'
		...
		function getworkInfo(){
			...
			if(endDate!=''){
				...
				if(s < 0) {
		         ...
		         //直接提交
function toSubmit(){
	document.getElementById('form1').action="?model=outsourcing_workverify_workVerify&action=add&addType=submit";
}