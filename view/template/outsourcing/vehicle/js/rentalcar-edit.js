$(document).ready(function() {
	...
	//有无测试人员跟车
	$("#isTestEngineer option").each(function() {
		if( $(this).val() == $("#isTestEngineerVal").val() ){
			$(this).attr("selected","selected'");
		}
	});

	//是否申请油卡
	$("#isApplyOilCard option").each(function() {
		if( $(this).val() == $("#isApplyOilCardVal").val() ){
			$(this).attr("selected","selected'");
		}
	});

	//总费用有效事件
	$("#allFee_v").bind('blur.checkVal' ,checkAllFee);
	...
	$("#suppInfo").yxeditgrid({
		...
		colModel : [{
			...
			process : function($input, rowData) {
				var rowNum = $input.data("rowNum");
				$("#suppInfo_cmp_isProvideInvoice" + rowNum).trigger('change');
			}
		},{
