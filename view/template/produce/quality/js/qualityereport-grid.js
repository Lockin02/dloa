	...
	$("#qualityereportGrid").yxgrid({
		...
		menusEx : [{
			...
			showMenuFn : function(row) {
				if(row.ExaStatus == '待提交' && row.auditStatus == 'BHG' && row.relDocTypeCode == 'ZJSQDLBF'){// 呆料报废的不合格待提交不可提交审核 PMS2386
					return false;
				}else if (row.ExaStatus == '待提交' && row.auditStatus != 'BH') {
					return true;
				} else {
					return false;
				}
			...
			action : function(row) {
				var canConfirm = $.ajax({
					url: '?model=produce_quality_qualityereport&action=checkAllPass',
					data: 'applyId=' + row.applyId,
					dataType: 'html',
					type: 'post',
					async: false
				}).responseText;
				if(row.relDocTypeCode == 'ZJSQDLBF' && canConfirm != 1) {// 该呆料报废单还存在部分物料为通过质检处理,不能提交审核
					alert("该呆料报废单还存在部分物料为通过质检处理,不能进行审核。");
				}else{
