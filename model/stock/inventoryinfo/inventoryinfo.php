...
EOT;
	 ...
	 * 根据库存信息id 锁定库存信息
	 */
	function lockExeNum($id, $lockNum,$productName) {
		...
		if ($inventoryinfo ['exeNum'] >= 0) {
			if ($lockNum > $inventoryinfo ['exeNum']) {
				if(!empty($productName)){
					throw new Exception ( $productName."锁定数量有误！" );
				}
			}
		}
	 ...
	 * 根据产品Id和仓库类型获取库存数量
	 */
	function getExeNumsByStockType($productId, $stockType = 'salesStockCode',$rType = 0) {
		$systeminfoDao = new model_stock_stockinfo_systeminfo ();
		...
		if($rType == '0'){
			...
			return $this->get_table_fields ( $this->tbl_name, $conditions, 'lockedNum' );
		}
	}
	...
	/**
	 *
	 * 根据物料id获取物料的即时库存（所有仓库）
	 */
	function getProActNum($productId) {

		$sql = "select ifnull(sum(actNum),0) as actNum from oa_stock_inventory_info where stockCode<>'OUTSTOCK' and productId='$productId'";
		$resultArr = $this->findSql ( $sql );
		return $resultArr ['0'] ['actNum'];
	}
	 ...
	 * @param $productId
	 */
	function getStockNum($productId){
		$sql = "select productName,productCode,productId,stockId,stockName,actNum
				from oa_stock_inventory_info
				where stockName in ('库存商品','旧设备仓','借出仓') and productId = '$productId'";
