...
//引入接口
include_once WEB_TOR . 'model/stock/withdraw/strategy/drawStrategy.php';
/**
 * @author zengzx
 ...
 */
class model_stock_withdraw_strategy_exchangedraw  implements drawStrategy {
...
//		echo "<pre>";
//		print_R($paramArr);
		$sourceDao = new model_stock_withdraw_withcommon();
	 ...
	 */
	  function updateContNumAsOutRed($contProInfo,$rows){
	  	...
	  	$changeDao = new model_projectmanagent_exchange_exchange();
	  	//更新合同已执行数量
		$contSql = " update oa_contract_exchange_equ set backNum=backNum-".$rows['outNum']." where borrowId= " . $contProInfo['docId'] . " and id= " . $rows['contEquId'];
