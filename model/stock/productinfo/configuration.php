 ...
 */
class model_stock_productinfo_configuration extends model_base {
	public $db;

	function __construct() {
		$this->tbl_name = "oa_stock_product_configuration";
		$this->sql_map = "stock/productinfo/configurationSql.php";
		parent::__construct ();
	}
...
EOT;
	 ...
	 * @param $proTypeId
	 */
	function findArrByPro($hardWareId) {
		$sql = "select c.* from oa_stock_product_configuration c where c.configType='proaccess' and c.hardWareId='$hardWareId'";
	 ...
	 * @param $proTypeId
	 */
	function findMianArr($hardWareId) {
		$sql = "select c.* from oa_stock_product_configuration c where c.configType='typeaccess' and c.hardWareId='$hardWareId'";
		return $this->findSql($sql);
	}
	 ...
	 * @return mixed
	 */
	function findProductConfig($productIdArr) {
		if (!empty($productIdArr)) {
			$sql = "select c.* from oa_stock_product_configuration c where c.configType='proconfig' and c.hardWareId IN(" .
				implode(",", $productIdArr).")";
			return $this->findSql($sql);
		} else {
			return false;
		}
