 ...
 */
$sql_arr = array (
	 "select_default"=>"select c.id ,c.proTypeId ,c.proType ,c.productCode ,c.productName ,c.pattern ,c.priCost ,c.unitName ,c.aidUnit ,c.converRate ,c.warranty ,c.arrivalPeriod ,c.accountingCode ,c.checkType ,c.properties ,c.supplier ,c.stockId ,c.stockName ,c.stockCode ,c.remark ,c.ext1 ,c.ext2 ,c.ext3 ,c.statType ,c.statTypeName ,c.encrypt ,c.allocation ,c.createName ,c.createId ,c.createTime ,c.updateName ,c.updateId ,c.updateTime ,c.changeProductId ,c.changeProductCode ,c.changeProductName ,c.closeReson ,c.leastPackNum ,c.leastOrderNum ,c.material ,c.brand ,c.color ,c.purchUserCode ,c.purchUserName ,c.purchPeriod,c.packageInfo,c.depreciation,c.esmCanUse,c.status,c.state from oa_stock_product_info_temp c where 1=1 ",
	 "select_productandconf" => "select
			c.id ,c.proTypeId ,c.proType ,c.productCode ,c.productName ,c.pattern ,c.priCost ,c.unitName ,c.aidUnit ,c.converRate,c.allocation,c.warranty ,c.arrivalPeriod ,c.accountingCode ,c.checkType ,c.properties ,c.supplier ,c.stockId ,c.stockName,
			...
			co.configId,co.hardWareId ,co.configType,co.configName ,co.configPattern,co.configCode ,co.configNum ,co.explains,c.status,c.state 
		from
			oa_stock_product_info_temp  c
			left join
			(
			select
	...
	"select_esm"=>"select c.id ,c.proTypeId ,c.proType ,c.productCode ,c.productName ,c.pattern ,c.unitName ,c.esmCanUse,
		...
		where 1 ",
	"select_esmcount"=>"select
			sum(IFNULL(a.inventoryAsset,0 )) as inventoryAsset,
			sum(IFNULL(s.actNum,0 )) as inventoryStock
		from
			oa_stock_product_info_temp c
			left join
			(select productId,sum(actNum) as actNum from oa_stock_inventory_info GROUP BY productId) s on c.id = s.productId
