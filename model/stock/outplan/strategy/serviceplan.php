 ...
 */
class model_stock_outplan_strategy_serviceplan  implements planStrategy {
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				...
				if( $rows[$key]['contEquId'] ){
					$contNumSql = " select number as contNum,issuedShipNum,isSell from oa_service_equ where orderId=" . $rows[$key]['docId']
...
EOT;
	...
	function showItemView($rows) {
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				if( $rows[$key]['contEquId'] ){
					$isSellSql = " select isSell from oa_service_equ where orderId=" . $rows[$key]['docId']
						. " and id=" . $rows[$key]['contEquId'];
				}else{
					$isSellSql = " select isSell from oa_service_equ where orderId=" . $rows[$key]['docId']
						. " and productId=" . $rows[$key]['productId'];
				...
				$str .= <<<EOT
					<tr><td>$j</td>
						...
						<td>$val[productModel]
							<input type="hidden" id="productModel_$j" name="outplan[bItem][$j][productModel]" value="$val[productModel]" class='readOnlyTxtItem' readonly/></td>
						<td>$val[unitName]
							<input type="hidden" name="outplan[bItem][$j][unitName]" id="unitName$j" value="$val[unitName]" class='readOnlyTxtShort' readonly/>
							<input type="hidden" id="planNum_$j"  name="outplan[bItem][$j][issuedShipNum]" value='$planNum' class="txtmiddle"/>
						</td>
						<td>$val[number]
							<input type="hidden" name="outplan[bItem][$j][number]" id="number_$j" value="$val[number]" class='txtshort' onblur="checkThis($j)"/>
...
/*************************************************************************************************************************************************************/
	...
	function viewRelInfo( $paramArr,$skey ) {
		$str .= ' <img src="images\icon\view.gif" onclick="showOpenWin(\'?model=engineering_serviceContract_serviceContract&action=toViewTab&id='.$paramArr['docId'].'&perm=view&skey='.$skey.'\')">';
		return $str;
	}
	...
	function dealRelInfoAtAdd($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			$sql = " update oa_service_equ set issuedShipNum = issuedShipNum+" . $relItemArr[$key]['number'] . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['docId'];
	...
	function dealRelInfoAtDel($id) {
		$outplanDao = new model_stock_outplan_outplan();
		$sql = "update oa_service_equ oe left join oa_stock_outplan_product op on oe.id = op.contEquId ".
	...
	function dealRelInfoAtChange($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			if( $vqal['isDel']==1 ){
				$sql = " update oa_service_equ set issuedShipNum = " . $relItemArr[$key]['issuedShipNum'] . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['docId'];
				$outplanDao->_db->query( $sql );
			}else{
				$issuedShipNum = $relItemArr[$key]['issuedShipNum'] + $relItemArr[$key]['number'];
				$sql = " update oa_service_equ set issuedShipNum = " . $issuedShipNum . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['docId'];
				$outplanDao->_db->query( $sql );
			}
		}
	}
	 ...
	 */
	 function getDocInfo( $id ){
	 	$docDao = new model_engineering_serviceContract_serviceContract();
	 	$rows = $docDao -> getShip_d( $id );
		if( trim($rows['orderCode']) == '' ){
			$rows['planDocCode'] = $rows['orderTempCode'];
			$rows['orderequ'] = $rows['serviceequ'];
			unset( $rows['serviceequ'] );
		}else{
			$rows['planDocCode'] = $rows['orderCode'];
			$rows['orderequ'] = $rows['serviceequ'];
			unset( $rows['serviceequ'] );
		}
		if(isset($rows['cusName'])){
			$rows['customerName'] = $rows['cusName'];
			$rows['customerId'] = $rows['cusNameId'];
			unset( $rows['cusName'] );
			unset( $rows['cusNameId'] );
		}
		$rows['docApplicant'] = $rows['orderPrincipal'];
		$rows['docApplicantId'] = $rows['orderPrincipalId'];
	 ...
	 */
	 function getSaleman( $id ){
	 	...
	 	$condition="id=".$id;
	 	$salemanKey = "orderPrincipal as responsible,orderPrincipalId as responsibleId";
	  ...
	  */
	  ...
	  function updateContNumAsOut($planInfo,$rows){
	  	...
	  	$contDao = new model_engineering_serviceContract_serviceContract();
	  	//更新合同已执行数量
		$contSql = " update oa_service_equ set executedNum = executedNum+" . $rows['outNum'] . " where orderId= " . $planInfo['docId'] . " and id= " . $rows['contEquId'];
	 ...
	 */
	  function updateContNumAsOutRed($contProInfo,$rows){
	  	$contDao = new model_engineering_serviceContract_serviceContract();
	  	//更新合同已执行数量
		$contSql = " update oa_service_equ set executedNum = executedNum+" . $rows['outNum'] . " ,backNum=backNum-".$rows['outNum']." where orderId= " . $contProInfo['docId'] . " and id= " . $rows['contEquId'];
		$contDao->_db->query( $contSql );
		// 根据合同Id修改合同状态。
