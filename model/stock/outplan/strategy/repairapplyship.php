 ...
 * @author zengzx
 * @since 1.0 - 2011-12-1
 * @description:工程项目发货单策略(维修申请单策略)
 */
class model_stock_outplan_strategy_repairapplyship implements shipStrategy {
				...
				if ($val ['isShip'] == "0") {
					...
					<tr>
						...
						<td>
							...
							<!--
							<input type="hidden" id="docType$j" name="ship[productsdetail][$j][docType]" value="oa_service_repair_apply" />
						...
						<td>
							<input type="text" name="ship[productsdetail][$j][contNum]" id="contNum$j" value="1" class='readOnlyTxtItem'/>

						</td>
						<td>
							<input type="text" name="ship[productsdetail][$j][number]" id="number$j" value="1" class='readOnlyTxtItem'  />
						</td>
						<td>
			...
			foreach ( $rows as $key => $val ) {
				...
				$str .= <<<EOT
					<tr>
						...
						<td>
							<input type="text" name="ship[productsdetail][$j][number]" id="number$j" value="$val[number]" class='readOnlyTxtItem'  />
						</td>
						<td>
...
EOT;
	...
	function dealRelInfoAtAdd($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key => $value ) {
			array_push ( $planEquIdArr, $value ['planEquId'] );
		}
		if (count ( $planEquIdArr ) > 0) {
			$repairApplyItemDao = new model_service_repair_applyitem ();
			$planEquIdStr = implode ( ",", $planEquIdArr );
			$repairApplyItemDao->query ( "update  oa_service_repair_applyitem set isShip=1 where id in($planEquIdStr)" );
		}
	}
	/**
	 * 修改发货时处理相关业务
	...
	function dealRelInfoAtDel($paramArr = false) {
		$planEquIdArr = array ();
		foreach ( $paramArr ['details'] as $key => $value ) {
			array_push ( $planEquIdArr, $value ['planEquId'] );
		}
		if (count ( $planEquIdArr ) > 0) {
			$repairApplyItemDao = new model_service_repair_applyitem ();
			$planEquIdStr = implode ( ",", $planEquIdArr );
			$repairApplyItemDao->query ( "update  oa_service_repair_applyitem set isShip=0 where id in($planEquIdStr)" );
		}
	}
