 ...
 */
class model_stock_outplan_strategy_salesplan  implements planStrategy {
	...
	function showItemView($rows) {
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				//合同设备是否赠送
				if( $rows[$key]['contEquId'] ){
					$isSellSql = " select isSell from oa_sale_order_equ where orderId=" . $rows[$key]['docId']
						. " and id=" . $rows[$key]['contEquId'];
				}else{
					$isSellSql = " select isSell from oa_sale_order_equ where orderId=" . $rows[$key]['docId']
						. " and productId=" . $rows[$key]['productId'];
...
/*************************************************************************************************************************************************************/
	...
	function viewRelInfo( $paramArr,$skey ) {
		$str .= ' <img src="images\icon\view.gif" onclick="showOpenWin(\'?model=projectmanagent_order_order&action=toViewTab&id='.$paramArr['docId'].'&perm=view&skey='.$skey.'\')">';
		return $str;
	}
	...
	function dealRelInfoAtAdd($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			$sql = " update oa_sale_order_equ set issuedShipNum = issuedShipNum+" . $relItemArr[$key]['number'] . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['docId'];
	...
	function dealRelInfoAtDel($id) {
		$outplanDao = new model_stock_outplan_outplan();
		$sql = "update oa_sale_order_equ oe left join oa_stock_outplan_product op on oe.id = op.contEquId ".
	...
	function dealRelInfoAtChange($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			if( $vqal['isDel']==1 ){
				$sql = " update oa_sale_order_equ set issuedShipNum = " . $relItemArr[$key]['issuedShipNum'] . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['docId'];
				$outplanDao->_db->query( $sql );
			}else{
				$issuedShipNum = $relItemArr[$key]['issuedShipNum'] + $relItemArr[$key]['number'];
				$sql = " update oa_sale_order_equ set issuedShipNum = " . $issuedShipNum . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['docId'];
				$outplanDao->_db->query( $sql );
			}
		}
	}
	 ...
	 */
	  function updateContNumAsOut($planInfo,$rows){
	  	...
	  	//更新合同已执行数量
		$contSql = " update oa_sale_order_equ set executedNum = executedNum+" . $rows['outNum'] . " where orderId= " . $planInfo['docId'] . " and id= " . $rows['contEquId'];
	 ...
	 */
	  function updateContNumAsOutRed($contProInfo,$rows){
	  	$contDao = new model_projectmanagent_order_order();
	  	//更新合同已执行数量
		$contSql = " update oa_sale_order_equ set executedNum = executedNum+" . $rows['outNum'] . " ,backNum=backNum-".$rows['outNum']." where orderId= " . $contProInfo['docId'] . " and id= " . $rows['contEquId'];
		$contDao->_db->query( $contSql );
		// 根据合同Id修改合同状态。
