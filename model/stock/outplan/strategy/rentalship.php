 ...
 * @description:销售发货单策略
 */
class model_stock_outplan_strategy_rentalship implements shipStrategy
{
    ...
    function showItemAddByPlan($rows)
    {
        $j = 0;
        if (is_array($rows)) {
            $contProDao = new model_contract_rental_tentalcontractequ();
            $i = 0; //列表记录序号
            $str = ""; //返回的模板字符串
...
//			//更新合同设备数量
//			$contEquSql = "update oa_lease_equ set executedNum=executedNum+" . $val['number']
