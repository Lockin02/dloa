 ...
 * @description:借用发货单策略
 */
class model_stock_outplan_strategy_borrowship implements shipStrategy
{
    ...
    function showItemAddByPlan($rows)
    {
        $j = 0;
        if (is_array($rows)) {
            $contProDao = new model_common_contract_allsource();
            $i = 0; //列表记录序号
            $str = ""; //返回的模板字符串
            foreach ($rows as $key => $val) {
                $contNumArr = $contProDao->getEquInfo($rows[$key]['contEquId'], 'oa_borrow_borrow');
            ...
            foreach ($rows as $key => $val) {
                ...
                $str .= <<<EOT
					<tr><td>$j</td>
						...
						<td>
							<input type="text" name="ship[productsdetail][$j][number]" id="number$j" value="$val[number]" class='txtshort Num' onblur="checkThis($j)"/>
						</td>
						<!--td width="8%"><font>否</font></td-->
						<td>
...
EOT;
    ...
    function getDocInfo($id)
    {
        ...
        $borrowObj = $borrowDao->get_d($rows['docId']);
...
//			//更新合同设备数量
//			$contEquSql = "update oa_borrow_equ set executedNum=executedNum+" . $val['number']
//			  . " where borrowId = " . $rows['docId'] . " and productId = " . $val['productId'];
