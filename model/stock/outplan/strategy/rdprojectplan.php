 ...
 */
class model_stock_outplan_strategy_rdprojectplan  implements planStrategy {
...
EOT;
			...
			if( $i == 0 ){
				$str = '<tr><td colspan="132"><input type="hidden" id="noRows" value="1"/>暂无相关信息</td></tr>';
			}
		}
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				...
				if( $rows[$key]['contEquId'] ){
					$contNumSql = " select number as contNum,issuedShipNum,isSell from oa_rdproject_equ where orderId=" . $rows[$key]['docId']
...
EOT;
	...
	function showItemView($rows) {
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				if( $rows[$key]['contEquId'] ){
					$isSellSql = " select isSell from oa_rdproject_equ where orderId=" . $rows[$key]['docId']
						. " and id=" . $rows[$key]['contEquId'];
				}else{
					$isSellSql = " select isSell from oa_rdproject_equ where orderId=" . $rows[$key]['docId']
						. " and productId=" . $rows[$key]['productId'];
...
/*************************************************************************************************************************************************************/
	...
	function viewRelInfo( $paramArr,$skey ) {
		$str .= ' <img src="images\icon\view.gif" onclick="showOpenWin(\'?model=rdproject_yxrdproject_rdproject&action=toViewTab&id='.$paramArr['docId'].'&perm=view&skey='.$skey.'\')">';
		return $str;
	}
	...
	function dealRelInfoAtAdd($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			$sql = " update oa_rdproject_equ set issuedShipNum = issuedShipNum+" . $relItemArr[$key]['number'] . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['docId'];
	...
	function dealRelInfoAtDel($id) {
		$outplanDao = new model_stock_outplan_outplan();
		$sql = "update oa_rdproject_equ oe left join oa_stock_outplan_product op on oe.id = op.contEquId ".
	...
	function dealRelInfoAtChange($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			if( $vqal['isDel']==1 ){
				$sql = " update oa_rdproject_equ set issuedShipNum = " . $relItemArr[$key]['issuedShipNum'] . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['docId'];
				$outplanDao->_db->query( $sql );
			}else{
				$issuedShipNum = $relItemArr[$key]['issuedShipNum'] + $relItemArr[$key]['number'];
				$sql = " update oa_rdproject_equ set issuedShipNum = " . $issuedShipNum . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['docId'];
				$outplanDao->_db->query( $sql );
			}
		}
	}
	 ...
	 */
	 function getDocInfo( $id ){
	 	$docDao = new model_rdproject_yxrdproject_rdproject();
		...
		if( trim($rows['orderCode']) == '' ){
			...
			$rows['orderequ'] = $rows['rdprojectequ'];
			unset( $rows['rdprojectequ'] );
		}else{
	 ...
	 */
	  ...
	  */
	  ...
	  function updateContNumAsOut($planInfo,$rows){
	  	...
	  	$contDao = new model_rdproject_yxrdproject_rdproject();
	  	//更新合同已执行数量
		$contSql = " update oa_rdproject_equ set executedNum = executedNum+" . $rows['outNum'] . " where orderId= " . $planInfo['docId'] . " and id= " . $rows['contEquId'];
	 ...
	 */
	  function updateContNumAsOutRed($contProInfo,$rows){
	  	$contDao = new model_rdproject_yxrdproject_rdproject();
	  	//更新合同已执行数量
		$contSql = " update oa_rdproject_equ set executedNum = executedNum+" . $rows['outNum'] . " ,backNum=backNum-".$rows['outNum']." where orderId= " . $contProInfo['docId'] . " and id= " . $rows['contEquId'];
		$contDao->_db->query( $contSql );
		// 根据合同Id修改合同状态。
