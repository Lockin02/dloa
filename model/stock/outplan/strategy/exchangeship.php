 ...
 * @description:销售发货单策略
 */
class model_stock_outplan_strategy_exchangeship implements shipStrategy
{
    ...
    function showItemAddByPlan($rows)
    {
        ...
        if (is_array($rows)) {
            ...
            foreach ($rows as $key => $val) {
                $contNumArr = $contProDao->getEquInfo($rows[$key]['contEquId'], 'oa_contract_exchangeapply');
...
//			//更新合同设备数量
//			$contEquSql = "update oa_rdproject_equ set executedNum=executedNum+" . $val['number']
//			  . " where presentId = " . $rows['docId'] . " and productId = " . $val['productId'];
