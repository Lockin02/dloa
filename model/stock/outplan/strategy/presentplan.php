 ...
 */
class model_stock_outplan_strategy_presentplan  implements planStrategy {
...
-->						<td>
						...
						<!--td width="8%"><font color='red'>是</font></td-->
						<!--td>
	...
	function showItemEdit($rows) {
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				$j = $i + 1;
				//合同数量 及 合同已下达发货数量
				$contNumArr = $docDao->getEquInfo($rows[$key]['contEquId'],'oa_present_present');
...
<!--						<td>
							<input type="hidden" id="productLine$j" name="outplan[productsdetail][$j][productLine]" value="$val[productLine]" class='readOnlyTxtItem' readonly/>
							<input type="text" id="productLineName$j" name="outplan[productsdetail][$j][productLineName]" value="$val[productLineName]" class='readOnlyTxtNormal' readonly/></td>
-->						<td>
							<input type="text" id="productNo$j" readonly="true" name="outplan[productsdetail][$j][productNo]" value="$val[productNo]" class='readOnlyTxtNormal'/></td>
						<td>
...
/*************************************************************************************************************************************************************/
	...
	function viewRelInfo( $paramArr,$skey ) {
		$str .= ' <img src="images\icon\view.gif" onclick="showOpenWin(\'?model=projectmanagent_present_present&action=init&id='.$paramArr['docId'].'&perm=view&skey='.$skey.'\')">';
		return $str;
	}
	...
	function dealRelInfoAtDel($id) {
		...
		$sql = "update oa_present_equ oe left join oa_stock_outplan_product op on oe.id = op.contEquId ".
	...
	function dealRelInfoAtChange($paramArr = false, $relItemArr = false) {
		$outplanDao = new model_stock_outplan_outplan();
        $conSql = "select max(id) as contractTmpId,ExaStatus from oa_present_present where originalId = '{$paramArr['docId']}' and ExaStatus <> '完成';";//查看变更中的数据
        $tmpConData = $outplanDao->_db->get_one($conSql);
        $contractIdSql = " and presentId=" . $paramArr['docId'];
        if(isset($tmpConData['contractTmpId']) && $tmpConData['contractTmpId'] != ""){// 如有相关数据,则更新条件内需加上临时记录ID
            $contractIdSql = " and presentId in (" . $paramArr['docId'] . ",". $tmpConData['contractTmpId'] . ")";
        }
		foreach ( $relItemArr as $key=>$vqal ){
            ...
            if(isset($tmpConData['contractTmpId']) && $tmpConData['contractTmpId'] != ""){
                $equSql = "select id from oa_present_equ where presentId = {$tmpConData['contractTmpId']} and originalId = {$relItemArr[$key]['contEquId']}";
			...
			if( $vqal['isDel']==1 ){
				$sql = " update oa_present_equ set issuedShipNum = " . $relItemArr[$key]['issuedShipNum'] . " where ". $equIdSql . $contractIdSql;
				$outplanDao->_db->query( $sql );
			}else{
				$issuedShipNum = $relItemArr[$key]['issuedShipNum'] + $relItemArr[$key]['number'];
				$sql = " update oa_present_equ set issuedShipNum = " . $issuedShipNum . " where ". $equIdSql . $contractIdSql;
				$outplanDao->_db->query( $sql );
			}
		}
	}
	 ...
	 */
	 function getDocInfo( $id ){
	 	$docDao = new model_projectmanagent_present_present();
...
//	 	print_R($rows);
		...
		$rows['equ'] = $rows['presentequ'];
		unset( $rows['presentequ'] );
		$rows['docApplicant'] = $rows['createName'];
		$rows['docApplicantId'] = $rows['createId'];
		$rows['planDocName'] = $rows['orderName'];
		$rows['customerId'] = $rows['customerNameId'];
	 	$rows['docType'] = "oa_present_present";
	 	$rows['contractType'] = "oa_present_present";
	 	$rows['contractTypeName'] = "赠送发货";
	 ...
	 */
	 function getSaleman( $id ){
	 	...
	 	$condition="id=".$id;
	 	$salemanKey = "salesName as responsible,salesNameId as responsibleId";
	 ...
	 */
	  ...
	  */
	  function dealupdateOrderShipStatus_d( $id ){
	  	$docDao = new model_projectmanagent_present_present();
	 	$orderRemainSql = " select (sum(number)-sum(executedNum)) as remainNum,sum(number) as allNum from oa_present_equ where presentId=" . $id;
	 ...
	 */
	  function updateContNumAsOutRed($contProInfo,$rows){
	  	$contDao = new model_common_contract_allsource();
	  	//更新合同已执行数量
		$contSql = " update oa_present_equ set backNum=backNum-".$rows['outNum']." where presentId= " . $contProInfo['docId'] . " and id= " . $rows['contEquId'];
