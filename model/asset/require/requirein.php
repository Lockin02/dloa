 ...
 class model_asset_require_requirein extends model_base {
   ...
   /*--------------------------------------------业务操作--------------------------------------------*/
	...
	function updateAsOut($rows) {
		// 如果是数字，那么是旧OA，否则是新OA
		if(is_numeric($rows['relDocItemId']) && strlen($rows['relDocItemId']) < 32){
			...
			// 接入aws
			$result = util_curlUtil::getDataFromAWS('asset', 'updateProductTransferDetail', array(
				"id" => $rows['relDocItemId'],
				"outId" => $rows['id'],
				"outStorageNum" => $rows['outNum'],
				"outPrice" => $rows['outPrice']
			));
