	 		...
	 		foreach ( $rows as $key => $val ) {
				...
				<tr align="center">
                  ...
                  <td>
                   ...
                   <td>
                     <input type="text" name="stockout[items][$i][productCode]" id="productCode$i" class="readOnlyTxtShort" value="$val[productCode]" readonly/>
					...
					<td>
					   <input type="text" name="stockout[items][$i][shouldOutNum]" id="shouldOutNum$i" class="readOnlyTxtShort" value="$val[shouldOutNum]" readonly/>
					</td>
					<td>
					...
					<td>
                     ...
                     <td>
                        <input type="text" name="stockout[items][$i][subCost]" id="subCost$i" class="readOnlyTxtShort formatMoney" value="$subCost" readonly/>
					</td>
                     <td>
    				...
    				<td>
    					<input type="text" name="stockout[items][$i][saleSubCost]" id="saleSubCost$i" class="readOnlyTxtShort formatMoney" readonly/>
					</td>
...
EOT;
	  ...
	  * 资产出库-产品列表
	  */
	 function showProAtStockOutNew_d($rows) {
	 	$str = "";
	 	if ($rows) {
	 		$i = 0;
	 		foreach ( $rows as $key => $val ) {
	 			// 获取已经出库的明细,并减掉相应的数量
	 			$sql = "SELECT SUM(actOutNum) AS actOutNum FROM oa_stock_outstock_item WHERE relDocId = '{$key}';";
	 			$rs = $this->_db->getArray($sql);
	 			$outStorageNum = empty($rs) ? 0 : $rs[0]['actOutNum'];
	 			$sNum = $i + 1;
	 			$storageNum = $val['applyNum'] - $outStorageNum;
	 			if ($storageNum == 0)
	 				continue;
				...
				<tr align="center">
                  ...
                  <td>
                   ...
                   <td>
					...
					<td>
					   <input type="text" name="stockout[items][$i][shouldOutNum]" id="shouldOutNum$i" class="readOnlyTxtItem" value="$storageNum" readonly/>
					</td>
					<td>
						<input type="text" name="stockout[items][$i][actOutNum]" id="actOutNum$i" class="txtshort" value="$storageNum" onfocus="exploreProTipInfo('$i')" ondblclick="chooseSerialNo($i)"  onchange="subTotalPrice1(this)" onblur="FloatMul('cost$i','actOutNum$i','subCost$i')"/>
					</td>
					<td>
						<input type="text" name="stockout[items][$i][stockName]" id="stockName$i" class="txtshort"   />
						<input type="hidden" name="stockout[items][$i][stockId]" id="stockId$i"   />
						<input type="hidden" name="stockout[items][$i][stockCode]" id="stockCode$i"   />
						<input type="hidden" name="stockout[items][$i][relDocId]" id="relDocId$i" value="$key"  />
						<input type="hidden" name="stockout[items][$i][relDocName]" id="relDocName$i" value=""  />
						<input type="hidden" name="stockout[items][$i][relDocCode]" id="relDocCode$i" value=""  />
					</td>
					<td>
					...
					<td>
						<input type="text" name="stockout[items][$i][cost]" id="cost$i" class="txtshort formatMoney" onblur="FloatMul('cost$i','actOutNum$i','subCost$i')" value="$val[price]"  />
					</td>
                     <td>
