...
EOT;
				...
				if($isDelivery){//由交付部执行采购
					foreach($object['receiveItem'] as $val){
						if($val['isDelTag']!="1"){
							$rs = $applyDao->get_d($applyArr[$val['contractId']]);
							$requirementId = $rs['relDocId'];//获取采购需求对应的资产需求申请id
							$requirementCode = $rs['relDocCode'];//获取采购需求对应的资产需求申请编号
							break;
						}
					}
				}else{//由行政部执行采购
					$rs = $applyDao->get_d($object['applyId']);
					$requirementId = $rs['relDocId'];//获取采购需求对应的资产需求申请id
					$requirementCode = $rs['relDocCode'];//获取采购需求对应的资产需求申请编号
				}
				$obj['requiredNo'] = $requirementCode;//资产需求申请编号
