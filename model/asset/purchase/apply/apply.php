	...
	function edit_d($object) {
		try {
			...
			if (is_array($object ['applyItem'])) {
                ...
                // 同步主子表的采购部门
                if(isset($object['purchaseDept'])){
                    $applyItemDao->update($mainArr,array("purchDept" => $object['purchaseDept']));
                }
...
EOT;
	...
	function confirmProduct_d($object) {
		try {
			...
			$id = null;
            //保存新增物料
            if(isset($object['equipment'])&&is_array($object['equipment'])){
                $applyItemDao = new model_asset_purchase_apply_applyItem();
                $row=$this->get_d($object['id']);
                foreach ($object['equipment'] as $key => $val) {
                        $itemRow=$applyItemDao->get_d($val['itemId']);
                        $val['applyId'] = $object['id'];
                        $val['applyCode'] = $row['formCode'];
                        $val['purchDept'] = $row['purchaseDept'];
                        $val['productCode'] = $val ['productNumb'];
                        $val['purchDept'] = $itemRow ['purchDept'];
                    $val['productCategoryCode'] = $itemRow ['productCategoryCode'];
                    $val['requireItemId'] = $itemRow ['requireItemId'];
                        $applyItemDao->add_d($val);
                }
				...
				if (is_array($equ) && count($equ) > 0 && isset($equ['productId'])) {
					...
					if ($equ ['productId'] > 0) {
						$sql = "update oa_asset_purchase_apply_item set productId=$productId,productName='$productName'," .
							"productCode='$productCode',pattem='$pattem',unitName='$unitName',applyAmount=$applyAmount where id=$equId";
						$this->query($sql);
					}
					$set = true;
				}
			...
			foreach ($listEqu as $key => $val) {
				if ($val['isBack'] == 0) {
					...
					<tr class="$iClass">
						...
						<td>
						<input type="hidden" class="txtshort" id="applyAmount$val[id]"  value="$val[applyAmount]"  />
						<input type="text" class="amount txtshort" id="applyAmount$i" idatr="$val[id]" name="apply[$i][applyAmount]" value="$val[applyAmount]"  onblur="checkNumForNew(this);" /></td>
						<td>
						$val[dateHope]</td>
						<td>
							$val[remark]
