...
class model_contract_common_contExcelUtil {
...
//        		$stime=microtime(true); //获取程序开始执行的时间
        ...
        foreach ($thArr as $key => $val) {
            if ($key == 'customerName' || $key == 'contractCode' || $key == 'contractName') {
                model_contract_common_contExcelUtil :: tableTrowLong($theadColumn, 1, $val, $objPhpExcelFile);
            } else
                model_contract_common_contExcelUtil :: tableTrow($theadColumn, 1, $val, $objPhpExcelFile);
            $theadColumn++;
        }
        $i = 2;
        ...
        if (!count(array_filter($rowdatas)) == 0) {
            ...
            foreach ($rowdatas as $n => $v) {
                $m = 0;
                foreach ($thArr as $field => $value) {
                    // 排除非法日期格式
                    if ($v[$field] != '0000-00-00') {
                        if (is_numeric($v[$field])) {
                            //解决显示为科学计数法的问题
                            $objPhpExcelFile->getActiveSheet()->setCellValueExplicitByColumnAndRow($m, $row + $n,
                                $v[$field], PHPExcel_Cell_DataType::TYPE_NUMERIC);
                        } else {
                            $objPhpExcelFile->getActiveSheet()->setCellValueByColumnAndRow($m, $row + $n, mb_convert_encoding($v[$field],"utf-8","gbk,big5"));
                        }
                    }

                    if ($field == "surincomeMoney" || $field == "surOrderMoney" || $field == "surOrderMoney") {
                        $objPhpExcelFile->getActiveSheet()->setCellValueExplicitByColumnAndRow($m, $row + $n, iconv("GBK", "utf-8", $v[$field]), PHPExcel_Cell_DataType :: TYPE_NUMERIC);
                    }
...
//        echo "<br />{$total} times";
        ...
        if (!count(array_filter($rowdatas)) == 0) {
            ...
            for ($n = 0; $n < count($rowdatas); $n++) {
                ...
                foreach ($rowdatas[$n] as $field => $value) {
                    ...
                    $m++;
                    //					model_contract_common_contExcelUtil :: toCecter($i, $objPHPExcel);
                }
                $i++;
            }
        } else {
    ...
    public static function exportContShipInfo() {
        ...
        foreach ($productColumn as $key => $val) {
            model_contract_common_contExcelUtil :: tableColumn($theadColumn, 1, $val, $objPhpExcelFile);
            model_contract_common_contExcelUtil :: tableColumn($theadColumn, 2, $exeNumArr[$key], $objPhpExcelFile);
            model_contract_common_contExcelUtil :: tableColumn($theadColumn, 3, $remainNumArr[$key], $objPhpExcelFile);
            $theadColumn++;
        }
        $i = 4;
     ...
     * 显示生产能力表
     */
    public static function produceViewList() {
        //		$stime=microtime(true); //获取程序开始执行的时间
        PHPExcel_CachedObjectStorageFactory :: cache_in_memory_serialized;
        //		//创建一个Excel工作流
        //		$objPhpExcelFile = new PHPExcel();
        ...
        $objReader = PHPExcel_IOFactory :: createReader('Excel2007'); //use excel2007 for 2007 format
        $objPhpExcelFile = $objReader->load("upfile/生产能力表.xlsx"); //读取模板
        //Excel2003及以前的格式
        $objWriter = new PHPExcel_Writer_Excel2007($objPhpExcelFile);

        $objWriteHTML = new PHPExcel_Writer_HTML($objPhpExcelFile); //输出网页格式的对象
...
//		header("Pragma: no-cache");
     ...
     * 导出商机列表
     */
    public static function exportChanceRecord($thArr, $rowdatas,$fileName){
