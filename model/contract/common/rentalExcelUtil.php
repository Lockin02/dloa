...
class model_contract_common_rentalExcelUtil{
	...
	public static function exporTemplate($dataArr, $rowdatas,$imageName){
		...
		$objPHPExcel = $objReader->load ( "upfile/rentalExport.xls" ); //读取模板
		...
		//设置表头及样式 设置
		for($m = 0; $m < 12; $m ++) {
			model_contract_common_rentalExcelUtil::tableThead($m,1,'合同信息',$objPHPExcel);
		}
		...
		//产品清单
		$tempSequence = 1;
		foreach( $dataArr['rentalcontractequ'] as $sequence => $equ ){
			$rentalcontractequ[$sequence]['sequence']=$tempSequence;
//			$rentalcontractequ[$sequence]['productLine']=$dataArr['rentalcontractequ'][$sequence]['productLine'];
			$rentalcontractequ[$sequence]['productNo']=$dataArr['rentalcontractequ'][$sequence]['productNo'];
			$rentalcontractequ[$sequence]['productName']=$dataArr['rentalcontractequ'][$sequence]['productName'];
			$rentalcontractequ[$sequence]['productModel']=$dataArr['rentalcontractequ'][$sequence]['productModel'];
			$rentalcontractequ[$sequence]['number']=$dataArr['rentalcontractequ'][$sequence]['number'];
			$rentalcontractequ[$sequence]['price']=$dataArr['rentalcontractequ'][$sequence]['price'];
			$rentalcontractequ[$sequence]['money']=$dataArr['rentalcontractequ'][$sequence]['money'];
			$rentalcontractequ[$sequence]['warrantyPeriod']=$dataArr['rentalcontractequ'][$sequence]['warrantyPeriod'];
			if($dataArr['rentalcontractequ'][$sequence]['isSell']==null){
				$rentalcontractequ[$sequence]['isSell']='否';
			}else{
				$rentalcontractequ[$sequence]['isSell']='是';
			}
			$tempSequence++;
		}
		//自定义产品清单
...
//			$customizelist[$sequence]['productLine']=$dataArr['customizelist'][$sequence]['productLine'];
		...
		/*************************************产品清单信息*****************************************/
			...
			$objPHPExcel->getActiveSheet ()->mergeCells('A' . ++$i . ':L' . $i);
			for($m = 0; $m < 12; $m ++) {
				model_contract_common_rentalExcelUtil::tableThead($m,$i,'产品清单',$objPHPExcel);
			}
			...
			$orderequTr = array('产品型号','数量','单价','金额','保修期(月)','合同内');
			model_contract_common_rentalExcelUtil::tableTrow(0,$i,'序号',$objPHPExcel);
			for($m = 1; $m < 3; $m ++) {
				model_contract_common_rentalExcelUtil::tableTrow($m,$i,'产品编号',$objPHPExcel);
			}
			for($m = 3; $m < 6; $m ++) {
				model_contract_common_rentalExcelUtil::tableTrow($m,$i,'产品名称',$objPHPExcel);
			}
			for($m = 6; $m < 12; $m ++) {
				model_contract_common_rentalExcelUtil::tableTrow($m,$i,$orderequTr[$m-6],$objPHPExcel);
			}
			//数据输出
			if( !count(array_filter($dataArr['rentalcontractequ']))==0 ){
				$i++;
				$row = $i;
				$tempArr = array(0,1,2,3,1,1,1,1,1,1);
				for($n=0;$n<count($rentalcontractequ);$n++){
					$m=0;$sum=0;
					$objPHPExcel->getActiveSheet ()->mergeCells('B' . $i . ':C' . $i);
					$objPHPExcel->getActiveSheet ()->mergeCells('D' . $i . ':F' . $i);
					foreach($rentalcontractequ[$n] as $field =>$value){
						$sum += $tempArr[$m];
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0+$sum,$row+$n,iconv("GBK","utf-8",$value));
						$m++;
						model_contract_common_rentalExcelUtil::toCecter($i,$objPHPExcel);
					}
					$i++;
				}
			}else{
		...
		$objPHPExcel->getActiveSheet ()->mergeCells('A' . ++$i . ':L' . $i);
		for($m = 0; $m < 12; $m ++) {
			model_contract_common_rentalExcelUtil::tableThead($m,$i,'自定义产品清单',$objPHPExcel);
		}
		...
		model_contract_common_rentalExcelUtil::tableTrow ( 0, $i, '序号', $objPHPExcel );
		model_contract_common_rentalExcelUtil::tableTrow ( 1, $i, '产品编号', $objPHPExcel );
		for($m = 2; $m < 4; $m ++) {
			model_contract_common_rentalExcelUtil::tableTrow ( $m, $i, '产品名称', $objPHPExcel );
		}
		...
		for($m = 4; $m < 9; $m ++) {
			model_contract_common_rentalExcelUtil::tableTrow ( $m, $i, $orderequTr [$m-4], $objPHPExcel );
		}
		//设置表头及样式 设置 -- 自定义产品清单 列名
		$objPHPExcel->getActiveSheet ()->mergeCells ( 'J' . $i . ':K' . $i );
		for($m = 9; $m < 11; $m ++) {
			model_contract_common_rentalExcelUtil::tableTrow ( $m, $i, '备注', $objPHPExcel );
		}
		model_contract_common_rentalExcelUtil::tableTrow ( 11, $i, '合同内', $objPHPExcel );
		...
		/*************************************培训计划信息*****************************************/
			...
			$objPHPExcel->getActiveSheet ()->mergeCells('A' . ++$i . ':L' . $i);
			for($m = 0; $m < 12; $m ++) {
				model_contract_common_rentalExcelUtil::tableThead($m,$i,'培训计划',$objPHPExcel);
			}
			...
			for($m = 0; $m < 4; $m ++) {
				model_contract_common_rentalExcelUtil::tableTrow($m,$i,$orderequTr[$m],$objPHPExcel);
			}
			for($m = 4; $m < 6; $m ++) {
				model_contract_common_rentalExcelUtil::tableTrow($m,$i,'培训地点',$objPHPExcel);
			}
			for($m = 6; $m < 9; $m ++) {
				model_contract_common_rentalExcelUtil::tableTrow($m,$i,'培训内容',$objPHPExcel);
			}
			for($m = 9; $m < 12; $m ++) {
				model_contract_common_rentalExcelUtil::tableTrow($m,$i,'培训工程师要求',$objPHPExcel);
			}
			//数据输出
