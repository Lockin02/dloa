...
class model_contract_common_serviceExcelUtil{
	...
	public static function exporTemplate($dataArr, $rowdatas,$imageName){
		...
		$objPHPExcel = $objReader->load ( "upfile/serviceExport.xls" ); //读取模板
		...
		//设置表头及样式 设置
		for($m = 0; $m < 12; $m ++) {
			model_contract_common_serviceExcelUtil::tableThead($m,1,'合同信息',$objPHPExcel);
		}
...
//			$serviceequ[$sequence]['productLine']=$dataArr['serviceequ'][$sequence]['productLine'];
			...
			if($dataArr['serviceequ'][$sequence]['isSell']==null){
				$serviceequ[$sequence]['isSell']='否';
			}else{
				$serviceequ[$sequence]['isSell']='是';
			}
			$tempSequence++;
		}
		//自定义产品清单
...
//			$customizelist[$sequence]['productLine']=$dataArr['customizelist'][$sequence]['productLine'];
		...
		/*************************************配置清单/服务内容*****************************************/
			$i=12;
			//设置表头及样式 设置 -- 配置清单/服务内容
			$objPHPExcel->getActiveSheet ()->mergeCells('A' . $i . ':L' . $i);
			$objPHPExcel->getActiveSheet ()->mergeCells('A' . ++$i . ':L' . $i);
			for($m = 0; $m < 12; $m ++) {
				model_contract_common_serviceExcelUtil::tableThead($m,$i,'配置清单/服务内容',$objPHPExcel);
			}

			//设置表头及样式 设置 --配置清单/服务内容 列名
			$i++;
			$objPHPExcel->getActiveSheet ()->mergeCells('B' . $i . ':F' . $i);
			$objPHPExcel->getActiveSheet ()->mergeCells('H' . $i . ':L' . $i);
			model_contract_common_serviceExcelUtil::tableTrow(0,$i,'序号',$objPHPExcel);
			for($m = 1; $m < 6; $m ++) {
				model_contract_common_serviceExcelUtil::tableTrow($m,$i,'内容',$objPHPExcel);
			}
			model_contract_common_serviceExcelUtil::tableTrow($m,$i,'数量/人数',$objPHPExcel);
			for($m = 7; $m < 12; $m ++) {
				model_contract_common_serviceExcelUtil::tableTrow($m,$i,'详细',$objPHPExcel);
			}
			//数据输出
		...
		/*************************************产品清单信息*****************************************/
			...
			$objPHPExcel->getActiveSheet ()->mergeCells('A' . ++$i . ':L' . $i);
			for($m = 0; $m < 12; $m ++) {
				model_contract_common_serviceExcelUtil::tableThead($m,$i,'产品清单',$objPHPExcel);
			}
			...
			model_contract_common_serviceExcelUtil::tableTrow(0,$i,'序号',$objPHPExcel);
			for($m = 1; $m < 3; $m ++) {
				model_contract_common_serviceExcelUtil::tableTrow($m,$i,'产品编号',$objPHPExcel);
			}
			for($m = 3; $m < 6; $m ++) {
				model_contract_common_serviceExcelUtil::tableTrow($m,$i,'产品名称',$objPHPExcel);
			}
			for($m = 6; $m < 12; $m ++) {
		...
		$objPHPExcel->getActiveSheet ()->mergeCells('A' . ++$i . ':L' . $i);
		for($m = 0; $m < 12; $m ++) {
			model_contract_common_serviceExcelUtil::tableThead($m,$i,'自定义产品清单',$objPHPExcel);
		}
		...
		model_contract_common_serviceExcelUtil::tableTrow ( 0, $i, '序号', $objPHPExcel );
		model_contract_common_serviceExcelUtil::tableTrow ( 1, $i, '产品编号', $objPHPExcel );
		for($m = 2; $m < 4; $m ++) {
			model_contract_common_serviceExcelUtil::tableTrow ( $m, $i, '产品名称', $objPHPExcel );
		}
		...
		for($m = 9; $m < 11; $m ++) {
			model_contract_common_serviceExcelUtil::tableTrow ( $m, $i, '备注', $objPHPExcel );
		}
		model_contract_common_serviceExcelUtil::tableTrow ( 11, $i, '合同内', $objPHPExcel );
		...
		/*************************************培训计划信息*****************************************/
			...
			$objPHPExcel->getActiveSheet ()->mergeCells('A' . ++$i . ':L' . $i);
			for($m = 0; $m < 12; $m ++) {
				model_contract_common_serviceExcelUtil::tableThead($m,$i,'培训计划',$objPHPExcel);
			}
			...
			for($m = 4; $m < 6; $m ++) {
				model_contract_common_serviceExcelUtil::tableTrow($m,$i,'培训地点',$objPHPExcel);
			}
			for($m = 6; $m < 9; $m ++) {
				model_contract_common_serviceExcelUtil::tableTrow($m,$i,'培训内容',$objPHPExcel);
			}
			for($m = 9; $m < 12; $m ++) {
				model_contract_common_serviceExcelUtil::tableTrow($m,$i,'培训工程师要求',$objPHPExcel);
			}
			//数据输出
