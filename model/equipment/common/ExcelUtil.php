...
class model_equipment_common_ExcelUtil {
	...
	public static function export2ExcelUtil($thArr, $rowdatas) {
		...
		$objPhpExcelFile = $objReader->load("upfile/equment.xls"); //读取模板
		...
		//			model_equipment_common_ExcelUtil :: tableThead(0, 1, '列表信息', $objPhpExcelFile);
		//			model_equipment_common_ExcelUtil :: toCecter(1, $objPhpExcelFile);
		//		}
		//设置表头及样式 设置
		$theadColumn = 0;
		foreach ($thArr as $key => $val) {
			model_equipment_common_ExcelUtil :: tableTrow($theadColumn, 1, $val, $objPhpExcelFile);
			$theadColumn++;
		}
		$i = 2;
		...
		if (!count(array_filter($rowdatas)) == 0) {
			...
			for ($n = 0; $n < count($rowdatas); $n++) {
				...
				foreach ($rowdatas[$n] as $field => $value) {
					...
					$m++;
					//model_equipment_common_ExcelUtil :: toCecter($i, $objPhpExcelFile);
				}
				$i++;
			}
		} else {
		...
		if (!count(array_filter($rowdatas)) == 0) {
			...
			for ($n = 0; $n < count($rowdatas); $n++) {
				...
				foreach ($rowdatas[$n] as $field => $value) {
					...
					$m++;
					//					model_equipment_common_ExcelUtil :: toCecter($i, $objPHPExcel);
				}
				$i++;
			}
		} else {
	...
	public static function exportContShipInfo() {
		...
		foreach ($productColumn as $key => $val) {
			model_equipment_common_ExcelUtil :: tableColumn($theadColumn, 1, $val, $objPhpExcelFile);
			model_equipment_common_ExcelUtil :: tableColumn($theadColumn, 2, $exeNumArr[$key], $objPhpExcelFile);
			model_equipment_common_ExcelUtil :: tableColumn($theadColumn, 3, $remainNumArr[$key], $objPhpExcelFile);
			$theadColumn++;
		}
		$i = 4;
    ...
    public static function exportChanceExcelUtil($thArr, $rowdatas) {
		...
		foreach ($thArr as $key => $val) {
			if ($key == 'orderCode' || $key == 'orderTempCode' || $key == 'orderName') {
				model_equipment_common_ExcelUtil :: tableTrowLong($theadColumn, 1, $val, $objPhpExcelFile);
			} else
				model_equipment_common_ExcelUtil :: tableTrow($theadColumn, 1, $val, $objPhpExcelFile);
			$theadColumn++;
		}
