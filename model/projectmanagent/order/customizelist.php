 ...
 */
 class model_projectmanagent_order_customizelist  extends model_base {

	function __construct() {
		$this->tbl_name = "oa_sale_order_customizelist";
		$this->sql_map = "projectmanagent/order/customizelistSql.php";
		parent::__construct ();
	}
			...
			$datadictArr = $this->getDatadicts ( "CPX" );
				...
				$str .=<<<EOT
					<tr align="center">
					 	...
					 	<td>
					 		<input type="text" class="txtshort" name="order[customizelist][$i][productCode]" id="PequID$i" size="10" value="$val[productCode]">
					 	</td>
					 	<td>
					        <input type="text" class="txtshort" name="order[customizelist][$i][productName]" id="PequName$i" size="15" value="$val[productName]">
					 	</td>
					 	<td>
					 		<input type="text" class="txtshort" name="order[customizelist][$i][productModel]" id="PreModel$i" size="10" value="$val[productModel]">
					 	</td>
					 	<td>
					 		<input type="text" class="txtshort" name="order[customizelist][$i][number]" onblur="FloatMul('PreAmount$i','PrePrice$i','CountMoney$i')" id="PreAmount$i" size="8" maxlength="10" value="$val[number]"/>
					 	</td>
					 	<td>
					 	...
					 	<td>
					 		<input type="text" class="txtshort" name="order[customizelist][$i][money]" id="CountMoney$i" size="8" maxlength="10" class="formatMoney"  value="$val[money]"/>
					 	</td>
					 	<td>
					 	...
					 	<td>
					 		<input type="text" class="txt" name="order[customizelist][$i][remark]" id="PRemark$i" size="18" maxlength="100" value="$val[remark]"/>
					 	</td>
					 	<td>
							<input type="hidden" id="cuslicenseId$i" name="order[customizelist][$i][license]" value="$val[license]"/>
			...
			$datadictArr = $this->getDatadicts ( "CPX" );
				...
				$str .=<<<EOT
					<tr align="center">
				 		...
				 		<td >
				        	<input type="checkbox" name="order[customizelist][$i][isSell]" checked="checked"/>
						</td>
					 	<td>
					 		<img src="images/closeDiv.gif" onClick="mydel(this,'mycustom')" title="删除行">
					 	</td>
