...
class model_projectmanagent_track_export2ExcelUtil extends model_base {
	...
	//以下是对工作表基本参数的设置
	//设置当前工作表的名称
	$objPhpExcelFile->getActiveSheet()->setTitle(iconv("gb2312", "utf-8", '空表头通用模板'));
	//设置表头及样式 设置
	...
	foreach ($thArr as $key => $val) {
		model_projectmanagent_track_export2ExcelUtil :: tableTrow($theadColumn, 1, $val, $objPhpExcelFile);
		$theadColumn++;
	...
	if (!count(array_filter($rowdatas)) == 0) {
		...
		for ($m = 0; $m < count($thArr); $m++) {
			...
			header('Content-Disposition:inline;filename="' . "空表头通用模板.xlsx" . '"');
