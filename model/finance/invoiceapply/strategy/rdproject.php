 ...
 * 服务合同开票申请策略
 */
class model_finance_invoiceapply_strategy_rdproject extends model_base implements iinvoiceapply{
...
EOT;
	...
	function getObjInfo_d($obj){
		$rdprojectOrderDao = new model_rdproject_yxrdproject_rdproject();
		$rdproject = $rdprojectOrderDao->get_d($obj['objId'],'none');


		//处理业务编号
		$rdproject['rObjCode'] = $rdproject['objCode'];

		//数组过滤,使数组能够通用载入
		//开票申请金额
		if($rdproject['orderMoney'] > 0){
			$rdproject['money'] = $rdproject['orderMoney'];
			unset($rdproject['orderMoney']);
		}else{
			$rdproject['money']  = $rdproject['orderTempMoney'];
			unset($rdproject['orderTempMoney']);
		}

		//数组过滤,使数组能够通用载入
		$rdproject['customerName'] = $rdproject['cusName'];
		unset($rdproject['cusName']);

		$rdproject['customerId'] = $rdproject['cusNameId'];
		unset($rdproject['cusNameId']);

		$rdproject['managerName'] = $rdproject['areaPrincipal'];
		$rdproject['managerId'] = $rdproject['areaPrincipalId'];
