 ...
 * 销售开票申请策略
 */
class model_finance_invoiceapply_strategy_salesorder extends model_base implements iinvoiceapply{

	/************************页面渲染**********************/
	private $thisClass = 'model_projectmanagent_order_order';
...
EOT;
	...
	function getObjInfo_d($obj){
		$saleOrderDao = new model_projectmanagent_order_order();
		$sales = $saleOrderDao->get_d($obj['objId'],'none');

		//处理业务编号
		$sales['rObjCode'] = $sales['objCode'];

		//数组过滤,使数组能够通用载入
		//开票申请金额
		if($sales['orderMoney'] > 0){
			$sales['money'] = $sales['orderMoney'];
			unset($sales['orderMoney']);
		}else{
			$sales['money']  = $sales['orderTempMoney'];
			unset($sales['orderTempMoney']);
		}

		$sales['managerName'] = $sales['areaPrincipal'];
		$sales['managerId'] = $sales['areaPrincipalId'];
