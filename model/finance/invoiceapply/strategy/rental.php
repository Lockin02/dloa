 ...
 * 服务合同开票申请策略
 */
class model_finance_invoiceapply_strategy_rental extends model_base implements iinvoiceapply{
...
EOT;
	...
	function getObjInfo_d($obj){
		$rentalOrderDao = new model_contract_rental_rentalcontract();
		$rental = $rentalOrderDao->get_d($obj['objId'],'none');

		//处理业务编号
		$rental['rObjCode'] = $rental['objCode'];

		//数组过滤,使数组能够通用载入
		//开票申请金额
		if($rental['orderMoney'] > 0){
			$rental['money'] = $rental['orderMoney'];
			unset($rental['orderMoney']);
		}else{
			$rental['money']  = $rental['orderTempMoney'];
			unset($rental['orderTempMoney']);
		}

		//数组过滤,使数组能够通用载入
		$rental['customerName'] = $rental['tenant'];
		unset($rental['tenant']);

		$rental['customerId'] = $rental['tenantId'];
		unset($rental['tenantId']);

		$rental['managerName'] = $rental['areaPrincipal'];
		$rental['managerId'] = $rental['areaPrincipalId'];
