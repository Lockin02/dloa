 ...
 */
class model_finance_invcost_invcostDetail extends model_base {

	function __construct() {
		$this->tbl_name = "oa_finance_invcost_detail";
		$this->sql_map = "finance/invcost/invcostDetailSql.php";
		parent :: __construct();
	}
...
//		return $str . '<tr><td colspan="50" style="text-align:center;">' . $showpage->show(6) . '</td></tr>';
	 ...
	 * 根据费用发票ID获取费用条目
	 */
	function getDetailByInvCostId($invCostId) {
	 ...
	 * 根据费用发票ID删除费用条目
	 */
	function deleteDetailByInCostId($invcostId) {
		$condition = array (
			"invCostId" => $invcostId
		);
