 ...
 * @version 1.0
 * @description:付款申请费用分摊明细表 Model层
 */
class model_finance_payablescost_payablescost extends model_base {

	function __construct() {
		$this->tbl_name = "oa_finance_payablesapply_cost";
		$this->sql_map = "finance/payablescost/payablescostSql.php";
		parent :: __construct();
	}
		...
		if(empty($object)){
			...
			try{
				...
				foreach($object['detail'] as $key => $val){
					...
					$object['detail'][$key]['shareMoney']=str_replace(',','',$object['detail'][$key]['shareMoney']); //去除千分位逗号(费用分摊)
...
//				print_r($payablesapplyIndexArr);
	...
	function getListViewCost_d($otherId){
		//先获取对应其他合同下的费用分摊
		$payablesapplydetailDao = new model_finance_payablesapply_detail();
		$payablesapplyArr = $payablesapplydetailDao->findAll(array('objId' => $otherId,'objType' => 'YFRK-02'),null,'payapplyId');
		$datas = array();
		if($payablesapplyArr){
			$payapplyIdArr = array();
			foreach($payablesapplyArr as $val){
				if(!in_array($val['payapplyId'],$payapplyIdArr)){
					array_push($payapplyIdArr,$val['payapplyId']);
				}
			}
			$this->searchArr = array('payapplyIdArr' => implode(',',$payapplyIdArr));
