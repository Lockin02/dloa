...
class model_finance_payablesapply_strategy_outaccount extends model_base implements ipayablesapply{

	//策略对应类
	private $thisClass = 'model_outsourcing_account_basic';
	...
	//对应编码
	private $thisCode = 'YFRK-05';

	//源单信息获取
	function getObjInfo_d($obj){
		$innerObjDao = new $this->thisClass();
//		$innerObj = $innerObjDao->get_d($obj['objId']);
		$innerObjDao->searchArr = array('id' => $obj['objId']);
		$innerObj = $innerObjDao -> list_d();
			...
			foreach($object['detail'] as $key => $val){
				...
				$str .=<<<EOT
					<tr>
						...
						<td>
							<input type="text" name="payablesapply[detail][$i][objCode]" id="objCode$i" value="$val[approvalCode]" class="readOnlyTxtNormal" readonly="readonly"/>
						...
						<td>
							<input type="text" name="payablesapply[detail][$i][expand1]" id="expand1$i" value="工程项目" class="readOnlyTxtMiddle" readonly="readonly"/>
						</td>
						<td>
