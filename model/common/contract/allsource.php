 ...
 */
class model_common_contract_allsource extends model_base {
	...
	"oa_contract_contract" => array (
				"baseModel" => "model_contract_contract_contract",
				"baseTable" => "oa_contract_contract",
				"equModel" => "model_contract_contract_equ",
				"equTable" => "oa_contract_equ",
				"mainField" => "contractId",
				"financeDatadict" => "'KPRK-02','KPRK-01'",
				"productModel" => "model_contract_contract_product"
					//"getEqus"=>"list2"
			)
				//			//租赁
		//			"oa_sale_lease" => array(
	...
	*/
	 ...
	 *获取源单某个产品设备清单
	 */
	function getProEqusNew($id, $docType) {
		// 获取源单(合同单，借试用单，赠送单)明细id，用于解决入库明细同个物料重复出现导致锁定库存报错的问题 add By weijb 2015.11.18
		$sql = "SELECT
				pe.applyEquId
			FROM
				oa_purchase_arrival_equ c
			LEFT JOIN oa_purch_apply_equ ae ON c.contractId = ae.id
			LEFT JOIN oa_purch_task_equ te ON ae.taskEquId = te.id
			LEFT JOIN oa_purch_plan_equ pe ON te.planEquId = pe.id
			WHERE
				c.id = ".$id;
	 ...
	 * 根据合同编号更新重复的业务编号
	 */
	function updateObjCodeByContractCode($table,$contractCode){
		$subsql="select id from $table where isTemp=0  and contractCode='$contractCode'";
		$srs=$this->_db->get_one($subsql);
		$contractId=$srs['id'];

		$subsql="select left(objCode,LENGTH(objCode)-4) as code from $table where isTemp=0  and contractCode='$contractCode'";
		$srs=$this->_db->get_one($subsql);
		$precode=$srs['code'];


		$sql="select max(right(objCode,4)) as code from $table where left(objCode,LENGTH(objCode)-4) =
(
$subsql
)";
		...
		switch(strlen($newCode)){
			case 1:$newCode="000".$newCode;break;
			case 2:$newCode="00".$newCode;break;
			case 3:$newCode="0".$newCode;break;
			case 4:$newCode=$newCode;break;
		}
		$newCode=$precode.$newCode;

		$sql="update $table set objCode='$newCode' where contractCode='$contractCode'";
		$this->query($sql);
		...
		//处理业务类型存表名的模块
			foreach ($this->relationTable as $tablename => $valArr) {
				...
				foreach ($this->sourceArr as $key => $val) {
					if ($tablename != $key) {
						$sql = "update $tablename left join $key on $tablename.$sourceId=$key.id set $tablename.rObjCode=$key.objCode " .
						"where $tablename.$sourceType='$key' and $key.id=$contractId";
						//echo $sql;
						$this->query($sql);
					}
			...
			foreach ($this->relationTableFinance as $tablename => $valArr) {
				...
				foreach ($this->sourceArr as $key => $val) {
					...
					if (!empty ($souceFinance)) {
						$sql = "update $tablename left join $key on $tablename.$sourceId=$key.id set $tablename.rObjCode=$key.objCode " .
						"where $tablename.$sourceType in ($souceFinance)  and $key.id=$contractId";
						$this->query($sql);
					}
				}
			}
	 ...
	 * 下达发货计划后，更新合同下达信息
	 */
	function updateIssuedInfo($docInfo, $rows) {
		...
		foreach ($rows as $key => $val) {
			$sql = " update " . $this->sourceArr[$docType]['equTable'] . " set issuedShipNum = issuedShipNum+" . $rows[$key]['number'] . " where id=" . $rows[$key]['contEquId'] . " and " . $this->sourceArr[$docType]['mainField'] . "=" . $docInfo['docId'];
	...
	function updateIssuedStatus($docInfo) {
		...
		$baseDao = new $baseModel ();
		$baseDao->updateShipStatus_d($docInfo['docId']);
		return 1;
	}
	/**************************************************下达信息 end************************************************/
	...
	function updateOutStockInfo($docInfo, $rows) {
		...
		if($docType == 'oa_contract_contract'){
			//更新合同项目执行进度
				...
				$conProDao->updateConProScheduleByCid($docInfo['docId']);
		}
	}
...
//			$this->query($rdprojectSql);
	...
	function getProductEqu_d($id,$sourceType) {
		...
		$equArr = $equDao->getConMat_d($productEqu[$mainIdFieldName],$productEqu['id']);
		if($sourceType == "contract" && !empty($equArr)){
			return $equArr;
		}else{
		$licenseIdArr=array();
		//添加一个缓存配置数量的数组
...
//		print_r($goodsInfoIdStr);
			$propertiesitemDao->searchArr['ids'] = $goodsInfoIdStr; //$goodsInfoIdStr
...
//print_R($cacheNumArr);
			foreach($licenseIdArr as $key=>$val){
				foreach($val as $k=>$v){
					...
					if(!empty($item['productId']) && !empty($product)){
						$product=$productArrNew[$item['productId']];
...
//		print_r($equArrReturn);
	 ...
	 * $sourceType 源单类型：合同，借试用，赠送，换货
	 */
	function newGetProductEqu_d($id,$sourceType) {
		switch($sourceType){
			case 'contract'://合同
				$dao= new model_contract_contract_product();
				break;
...
/*************************************合同导入************end***************************************************************/
	 ...
	 * 手工 定时保存 销售三张报表数据
	 */
	function saveTimingData_d($objType) {
		  $date = date("Y-m-d");
			//定时保存sql
	       $timingSql = array(
	          "oa_chance_view" => "
	          		Insert into oa_chance_view_timing (
					 year,type,areaPrincipal,areaName,customerTypeName,customerType,areaPrincipalTemp,areaNameTemp,customerTypeNameTemp,DEPT_ID,
					   Q1_100_01,Q1_100_02,Q1_100_03,Q1_100_all,Q1_50_01,Q1_50_02,Q1_50_03,Q1_50_all,Q1_25_01,Q1_25_02,Q1_25_03,Q1_25_all,Q1_0_00,
					   Q2_100_01,Q2_100_02,Q2_100_03,Q2_100_all,Q2_50_01,Q2_50_02,Q2_50_03,Q2_50_all,Q2_25_01,Q2_25_02,Q2_25_03,Q2_25_all,Q2_0_00,
					   Q3_100_01,Q3_100_02,Q3_100_03,Q3_100_all,Q3_50_01,Q3_50_02,Q3_50_03,Q3_50_all,Q3_25_01,Q3_25_02,Q3_25_03,Q3_25_all,Q3_0_00,
					   Q4_100_01,Q4_100_02,Q4_100_03,Q4_100_all,Q4_50_01,Q4_50_02,Q4_50_03,Q4_50_all,Q4_25_01,Q4_25_02,Q4_25_03,Q4_25_all,Q4_0_00,
					   all_100_01,all_100_02,all_100_03,all_100_all,all_50_01,all_50_02,all_50_03,all_50_all,all_25_01,all_25_02,all_25_03,all_25_all,all_0_00,
					   timingDate
					)
					select
					 year,type,areaPrincipal,areaName,customerTypeName,customerType,areaPrincipalTemp,areaNameTemp,customerTypeNameTemp,DEPT_ID,
					...
					from oa_chance_view",
			"oa_contract_view" => "
	          		Insert into oa_contract_view_timing (
					 year,type,areaPrincipal,areaName,customerTypeName,customerType,areaPrincipalTemp,areaNameTemp,customerTypeNameTemp,DEPT_ID,
					   Q1_100,Q1_50,Q1_0,Q1_c,
					   Q2_100,Q2_50,Q2_0,Q2_c,
					   Q3_100,Q3_50,Q3_0,Q3_c,
					   Q4_100,Q4_50,Q4_0,Q4_c,
					   all_100,all_50,all_0,all_c,
					   timingDate
					)
					select
					 year,type,areaPrincipal,areaName,customerTypeName,customerType,areaPrincipalTemp,areaNameTemp,customerTypeNameTemp,DEPT_ID,
					   ...
					   all_100,all_50,all_0,all_c,
					   '$date' as timingDate
					from oa_contract_view",
			"oa_saleall_view" => "
	          		Insert into oa_saleall_view_timing (
					 year,type,areaPrincipal,areaName,customerTypeName,customerType,areaPrincipalTemp,areaNameTemp,customerTypeNameTemp,DEPT_ID,
					   Q1_100,Q1_50,Q1_25,Q1_0,Q1_c,
					   Q2_100,Q2_50,Q2_25,Q2_0,Q2_c,
					   Q3_100,Q3_50,Q3_25,Q3_0,Q3_c,
					   Q4_100,Q4_50,Q4_25,Q4_0,Q4_c,
					   all_100,all_50,all_25,all_0,all_c,
					   timingDate
					)
					select
					 year,type,areaPrincipal,areaName,customerTypeName,customerType,areaPrincipalTemp,areaNameTemp,customerTypeNameTemp,DEPT_ID,
