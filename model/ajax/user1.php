...
class model_ajax_user extends includes_class_page
{
	...
	function depart_user_list()
	{
		...
		while (($rs = $this->db->fetch_array($query))!=false)
		{
			$str .='
				...
				$str .='<td align="center">
	        		...
	        		| <a href="?model=user&action=edit_func_list&userid='.$rs['USER_ID'].'&placeValuesBeforeTB_=savedValues&TB_iframe=true&modal=false&height=600&width=1000" title="修改《'.$rs['USER_ID'].'》用户权限" class="thickbox">权限设置</a>
		...
		$this->_set_url('num='.$num.'&model=user&action=depart_user_list&depart_id='.$depart_id.'&');
	...
	function jobs_user_list()
	{
		...
		$pageclass = $this->show_page(array('total'=>$num,'perpage'=>pagenum,'ajax'=>'show_page','page_name'=>'page'));
		$this->_set_url('num='.$num.'&model=user&action=jobs_user_list&jobs_id='.$jobs_id.'&');
		return $str.'<tr><td colspan="7" height="25">'.$this->show(6).'</td></tr>';
	}
	/**
	 * 搜索用户
	...
	function get_userid()
	{
		...
		if ($username)
		{
			$rs = $this->db->get_one("select * from user where has_left=0 and del=0 and USER_NAME='".$username."'");
			if ($rs)
			{
				return $rs['USER_ID'];
			}else{
	...
	function get_username(){
		$userid = $_GET['userid'] ? $_GET['userid'] : $_POST['userid'];
		$rs = $this->db->get_one("select user_name from user where has_left=0 and del=0 and user_id='".$userid."'");
	...
	function get_username_pinyin()
	{
		...
		if ($username)
		{
			...
			if ($word)
			{
				...
				return $pinyin->getFullSpell($name).'.'.$pinyin->getFullSpell($word[0]);
			}
		}
	}
	...
	function user_select()
	{
		...
		$user_id = is_array($user_id) ? $user_id :($user_id ? explode(',',$user_id) : null);
