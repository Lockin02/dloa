...
class model_loan_loan_loan extends model_base {
    ...
    function add_d($object) {
        try {
            ...
            // 如果是非项目借款的单据,清空项目ID字段
            if(isset($object['XmFlag']) && isset($object['projectId'])){
                $object['projectId'] = ($object['XmFlag'] == 1)? $object['projectId'] : '';
            }
   ...
   function getAppArea($proNo){
    ...
    function getReMoney_d($id,$Amount){
    	...
    	if(empty($reArr[0]['reMoney'])){
            ...
            return bcsub($Amount,$reArr[0]['reMoney'],2);
        }
...
//            echo"<pre>";print_r($deptIdsStr);exit();
    ...
    function change_d($object){
        try{
            ...
            // 如果存在项目号,重新匹配项目的ID信息
            if(isset($object['ProjectNo']) && $object['ProjectNo'] != ''){
                $esmProjectDao = new model_engineering_project_esmproject();
                $project = $esmProjectDao->find(array("projectCode" => $object['ProjectNo']));
                if($project){
    ...
    function dealAfterAuditChange_d($spid)
    {
        ...
        try{
            ...
            switch ($object['ExaStatus']) {
                case AUDITED: // 审批完成
                    // 因出现变更后项目ID更新漏了的情况,这里暂时将项目信息重新填入到原纪录中去
                    $this->update("ID in ({$originalId})",array("ProjectNo"=>$object['ProjectNo'],"projectId"=>$object['projectId']));
