...
class model_file_uploadfile_management extends model_base
{
	...
	function uploadFileAction($geyFile, $upload) {
		...
		$dateStr = date("Ymd").'/';
		$uploadPath1 = str_replace("\\", "/", UPLOADPATH) .$dateStr;
		if (!is_dir($uploadPath1)) {// 检查年月日文件夹层是否存在
			mkdir($uploadPath1);
		}

		$uploadPath = $uploadPath1. $upload['serviceType'] . '/';
		if (!is_dir($uploadPath)) {// 检查模块文件夹层是否存在
			mkdir($uploadPath);
		}

		include(WEB_TOR . 'includes/upload_class.php');
		$maxFileSize = 300;
		ini_set('upload_max_filesize', $maxFileSize . 'M');
		$upfile = new NdUpload();
		$upfile->nd_max_filesize = 1024 * 1024 * $maxFileSize;
		$upfile->nd_arr_ext_accepted = array(
			'.zip', '.rar', '.jpg', '.gif', '.bmp', '.doc', '.chm',
			'.docx', '.txt', '.xls', '.xlsx', '.pdf', '.psd', '.et',
			'.ppt', '.png', '.eml', '.JEPG', '.7z', '.tar', '.jpeg'
		);
		$upfile->setDir($uploadPath);
		$upfile->copyFile();
		$upfile->showError();
		if ($upfile->getErrCode() != 6) {
			return array('errorCode' => 'uploadError', 'errorMsg' => '【附件类内部错误】' . $upfile->showError());
		} else {
			/*******组合SQL数组***********/
	...
	function showFilelist($rows, $isShowDel) {
		if ($rows) {
			...
			foreach ($rows as $key => $val) {
				...
				$newFileTip = ($val['originalId'] > 0)? "" : "newFile";
				$str .= "
                    <div id='fileDiv$val[id]'>
                    <a class='{$newFileTip}' href='?model=file_uploadfile_management&action=toDownFileById&fileId=$val[id]&sessionId=" . session_id() . "' taget='_blank' title='$title'>$val[originalName]</a>
                        &nbsp;";
	 ...
	 * 根据文件名和文件夹名删除文件
	 */
	function delFile($serviceType, $filename, $createTimeStr = '') {
		$hasTimeDir = 0;
		// 如果有传创建时间,则先查年月日文件夹下是否含对应文件
		if($createTimeStr != ''){
			// 根据创建日期获取所属年月日文件夹名
			$regex="'\d{4}-\d{1,2}-\d{1,2}'is";
			preg_match_all($regex,$createTimeStr,$matches);
			$timePath = str_replace("-","",$matches[0][0]);
			$delpath = UPLOADPATH .$timePath.'/'.$serviceType . '/' . $filename;
			if (is_file($delpath)) {
				$hasTimeDir = 1;
				unlink($delpath);
			}
		}
		// 如果年月日文件夹下不含对应文件,则直接按模块名来查
		if(!$hasTimeDir){
	...
	function downFileByPath($path, $DOC_ID, $DOC_NAME) {
		...
		if (fopen($local, "r")) {
			...
			header('Content-Type: application/octet-stream');
			header('Content-Disposition: attachment; filename=' . $this->get_basename($DOC_NAME));
	...
	function downFile2($inDocument, $DOC_ID, $DOC_NAME) {
		/**   2010-10-07 修改 **/
		if ($DOC_ID) {
			$local = UPLOADPATH . $inDocument . '/' . $DOC_ID;
		} else {
			$local = UPLOADPATH . $inDocument;
		}
		/**  2010-10-07 修改结束 **/
	...
	function downFileById($id) {
		...
		// 根据创建日期获取所属年月日文件夹名
		$regex="'\d{4}-\d{1,2}-\d{1,2}'is";
		preg_match_all($regex,$file['createTime'],$matches);
		$timePath = str_replace("-","",$matches[0][0]);

		$fileLocal = UPLOADPATH . $file['serviceType'] . '/' . $file['newName'];// 不带年月日文件夹路径
		$fileLocal1 = UPLOADPATH . $timePath.'/' . $file['serviceType'] . '/'. $file['newName'];// 带年月日文件夹路径
		if(fopen($fileLocal1, "r")){// 先查带年月日文件夹路径
			$uploadPath = UPLOADPATH.$timePath.'/'. $file['serviceType'] . '/';
			$this->downFileByPath($uploadPath, $file['newName'], $file['originalName']);
		} else if (fopen($fileLocal, "r")) {// 再查不带年月日文件夹路径
			$this->downFileByPath(UPLOADPATH. $file['serviceType'] . '/', $file['newName'], $file['originalName']);
		} else {// 如果上面两路径都查不到对应文件,则在特定的路径地址查询相应文件
			$local1 = SPECIALPATH .$timePath.'/'. $file['serviceType'] . '/';
			if (fopen($local1.$file['newName'], "r")) {
				$local = $local1;
			}else{
	...
	function downAllFileByIds($ids, $type, $filename) {
		...
		foreach ($dataList as $val) {
			...
			if (file_exists($attachfile)) {
				$zip->addFile($attachfile, $this->get_basename($val['originalName'])); //第二个参数是放在压缩包中的文件名称
			} else {
				echo "<script>alert('下载的文件不存在');history.back();</script>";
			}
		}
		...
		header('Content-Description: File Transfer');
		header('Content-Type: application/octet-stream');
		header('Content-Disposition: attachment; filename=' . $this->get_basename($filename));
		header('Content-Transfer-Encoding: binary');
		header('Expires: 0');
		header('Cache-Control: must-revalidate, post-check=0, pre-check=0');
		header('Pragma: public');
