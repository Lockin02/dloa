 ...
 * @version V1.0
 */
class model_rdproject_plan_rdplan extends model_treeNode {
	...
	function __construct() {
		...
		$this->sql_map = "rdproject/plan/rdplanSql.php";
		parent::__construct ();
	}
...
//		$node['status'] = 'JHJXZ';
		...
		$purviewDao = new model_rdproject_plan_rdpurview();
		try {
			$this->start_d ();
			//调用树工具产生左右节点id
			...
			//添加权限
			$purviewDao->addGroup($node['projectId'],$newId,$node['parentId']);
...
//			$this->rollBack ();
	...
	function addByImport($node){
		$node['realBeginDate'] = $node['planBeginDate'];
		$purviewDao = new model_rdproject_plan_rdpurview();
		$templatePlanDao = new model_rdproject_template_rdtplan();
...
//		return $fusionCharts->mixCharts($outArr,'计划资源使用透视图',null,'h','450','300',array('估计工作量','已投入工作量'));
	 ...
	 * 根据项目ID及计划名称返回项目计划Id
	 */
	function getPlanIdByName($projectId,$planName){
		$searchArr = array(
			'projectId'=>$projectId,
			'planName'=>$planName
		);
		return $this->find( $searchArr,null,'id' );
	}
}
?>
