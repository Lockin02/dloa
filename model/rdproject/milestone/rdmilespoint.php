 ...
 * @version V1.0
 */
class model_rdproject_milestone_rdmilespoint extends model_base {
	...
	function __construct() {
		...
		$this->sql_map = "rdproject/milestone/rdmilespointSql.php";
		parent::__construct ();
...
EOT;
		...
		function rmPointU2_s ( $arr,$pjId ) {
			$str ='';
			$i = 0;
			$pjDao = new model_rdproject_project_rdproject();
			$pjArr = $pjDao->rpArrById_d($pjId);
			$milDao = new model_rdproject_milestone_rdmilestone();
...
EOT;
		...
		function rmList_s ( $arr ) {
			...
			foreach( $arr as $key =>$val ){
				...
				<td>
					<a href="?model=rdproject_milestone_rdmilespoint&action=toEdit&id=$val[id]&placeValuesBefore&TB_iframe=true&modal=false&height=350&width=550" class="thickbox">修改</a>
				</td>
			</tr>
EOT;
			}
...
//				print_r($objectinfo);
	...
	function rmArrList_d ($seachArr) {
		...
		foreach($pointArr as $key => $val){
            ...
            if(!empty($pointArr[$key]['warpRate'])){
                $pointArr[$key]['warpRate'] = $pointArr[$key]['warpRate'] ."%";
			}else{
	 ...
	 */
	function rmUpdateEffort_d ($val,$id,$warpRate = 0) {
		$objectinfo = array(
			"id"=>$id,
			"effortRate"=>$val,
            'warpRate' => $warpRate
     ...
     * 更新里程碑点的完成率
     */
    function updateMildstoneEff_d($id){
        $this->searchArr = array( 'stoneId'=> $id,'isStone' => 0,'status' => 'WQD,JXZ,DSH,TG,WTG,ZT' );
        $row = $this->list_d('sum_mildestoneEff');
        $thisEff = $row[0]['thisEff']/$row[0]['taskNum'];
        $thisWarp = $row[0]['thisWarp']/$row[0]['taskNum'];

        return $this->updateById(array('id'=> $id,'effortRate' => $thisEff,'warpRate' => $thisWarp ));
    }

    /**
     * 更新里程碑状态,如果里程碑任务全部完结,则推进到下一个阶段
     */
    function updateMilestonePeriod_d($id){
        try{
            //更新里程碑完成率
            $this->updateMildstoneEff_d($id);

            //查询当前里程被完成任务个数
            $this->searchArr = array( 'stoneId'=> $id,'isStone' => 0,'status' => 'WQD,JXZ,DSH,TG,WTG,ZT' );
            $row = $this->list_d('sum_mildestoneCom');
            $taskNum = $row[0]['taskNum'];
            $taskCompleted = $row[0]['taskCompleted'];
            //里程碑任务完成个数和里程碑任务个数相等时,将当前里程碑关闭,打开下一个里程碑点
            if($taskNum == $taskCompleted){
                $this->updateById(array('id'=> $id,"status"=> $this->statusDao->statusEtoK ( "end" ),"realEndDate"=>day_date));
                $this->update(array('parentId'=> $id) ,array( "status"=> $this->statusDao->statusEtoK ( "execute" ),"realBeginDate"=>day_date));
            }
            return true;
        }catch(exception $e){
            throw $e;
     ...
     * 新建里程碑任务时更新里程碑点
     */
    function updateMileInfo_d($id,$taskObj,$stoneDate){
        if(empty($stoneDate['planBeginDate'])){
            $planBeginDate = $taskObj['planBeginDate'];
            $planEndDate = $taskObj['planEndDate'];
        }else{
            $planBeginDate = strtotime($stoneDate['planBeginDate']) < strtotime($taskObj['planBeginDate']) ? $stoneDate['planBeginDate'] : $taskObj['planBeginDate'];
            $planEndDate = strtotime($stoneDate['planEndDate']) > strtotime($taskObj['planEndDate']) ? $stoneDate['planEndDate'] : $taskObj['planEndDate'];
        }
        return $this->updateById(array('id'=> $id,'planBeginDate'=> $planBeginDate,'planEndDate'=>$planEndDate));
    }
}
?>
