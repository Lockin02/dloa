...
$sql_arr = array(
    "select" => "select
        'dept' as icon,'dept' as type,c.Code,c.DEPT_ID as id,c.Depart_x,c.levelflag,c.PARENT_ID,
        c.DEPT_NAME,c.DEPT_NAME as name,c.Leader_id,c.Leader_name,c.TEL_NO,c.FAX_NO,c.DEPT_NO,c.DEPT_FUNC,c.Dflag,
        c.pdeptid,c.pdeptname,
        (select if(count(d.DEPT_ID)>1,1,0) from department d where d.Depart_x like CONCAT(c.Depart_x,'%')) as hasChildren
        from department c where 1=1 ",
    "selectForUser" => "select 'dept' as icon,'dept' as type,c.DEPT_ID as id,c.Code,c.Depart_x,c.levelflag,c.PARENT_ID,
        c.DEPT_NAME,c.DEPT_NAME as name,c.Leader_id,c.Leader_name,c.TEL_NO,c.FAX_NO,c.DEPT_NO,c.DEPT_FUNC,c.Dflag,
        (select if(count(d.DEPT_ID)>1,1,0) from department d where d.Depart_x like CONCAT(c.Depart_x,'%')) as hasChildren,
