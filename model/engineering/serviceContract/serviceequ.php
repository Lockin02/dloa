 ...
 */
class model_engineering_serviceContract_serviceequ extends model_base {

	function __construct() {
		$this->tbl_name = "oa_service_equ";
		$this->sql_map = "engineering/serviceContract/serviceequSql.php";
		parent :: __construct();
	}
/**
     * 蓝字出库 销售合同从表渲染
     */
     function showItemAtService($orderId) {
...
EOT;
	...
	function initTableView($object,$objId) {
         //获取最近一次变更审批的明细记录
			   ...
			   $changeInfo = $dao->getLastDetails("servicecontract",$objId);
			   ...
			   if(!empty($detailId)){
                 foreach($detailId as $k => $v){
	                $sql = "select * from oa_service_equ where id = '".$k."' ";
		...
		foreach ($object as $key => $val) {
            ...
            if(!empty($val[productNo])){
			 ...
			 //判断主表是否在变更提请中
              $Osql = "select isBecome from oa_sale_service where id= '".$val['orderId']."' ";
            ...
            if($val['changeTips'] != '0'){
                  if($val['isDel']  == '1' ){
					...
					<tr style="background:#C8C8C8">
						<td width="5%">$i</td>
						<td>$val[productNo]</td>
						<td><span class="red" onclick="beColor('service',$val[id],$val[orderId]);">$val[productName]</span>&nbsp<img src="images/icon/icon105.gif" onclick="conInfo($val[productId],$val[orderId]);" title="查看配置信息"/></td>
		...
		foreach ( $object as $key => $val ) {
			...
			$str .= <<<EOT
					<tr id="equTab_$i" parentRowId="$val[isCon]" ><td width="5%">$i
						...
						<td><input type="text" class="readOnlyTxtShort" readonly="readonly"  name="serviceContract[serviceequ][$i][productNo]" id="productNo$i"  value="$val[productCode]"/>
			                <input type="hidden" name="serviceContract[serviceequ][$i][unitName]" id="unitName$i" value="$val[unitName]">
			            </td>
			            <td><input type="hidden" id="productId$i" name="serviceContract[serviceequ][$i][productId]" value="$val[id]"/>
			                <input type="text" name="serviceContract[serviceequ][$i][productName]" id="productName$i" class="readOnlyTxtMiddle" readonly="readonly"  value="$val[productName]"/>
			                &nbsp<img src="images/icon/icon105.gif" onclick="conInfo('productId$i');" title="查看配置信息"/>
			            </td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][productModel]"  id="productModel$i" class="readOnlyTxtShort" readonly="readonly" value="$val[pattern]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][number]" id="number$i" class="txtshort" value="$val[configNum]" onblur="FloatMul('number$i','price$i','money$i')"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][price]" id="price$i" class="txtshort formatMoney" onblur="FloatMul('number$i','price$i','money$i')" value="$val[price]"/></td>
                        <td><input type="text" name="serviceContract[serviceequ][$i][money]" id="money$i" class="txtshort formatMoney" onblur="FloatMul('number$i','price$i','money$i')" value="$val[money]"/></td>
			        	<td nowrap width="8%"><input type="text" class="txtshort" name="serviceContract[serviceequ][$i][warrantyPeriod]" id="warrantyPeriod$i" value="$val[warranty]" /></td>
				        <td><input type="hidden" id="licenseId$i" name="serviceContract[serviceequ][$i][license]" value="$val[license]"/>
 			                <input type="button" class="txt_btn_a" value="配置" onclick="License('licenseId$i');" />
 			            </td>
				        <td width="4%"><input type="checkbox" name="serviceContract[serviceequ][$i][isSell]" id="isSell$i" checked="checked"/>
				            <input type="hidden" name="serviceContract[serviceequ][$i][isCon]" id="isCon" value="$val[isCon]">
				            <input type="hidden" name="serviceContract[serviceequ][$i][isConfig]" id="$val[isConfig]" value="$val[isCon]"></td>
		...
		foreach ( $object as $key => $val ) {
			...
			$str .= <<<EOT
					<tr id="equTab_$i" parentRowId="$val[isCon]" ><td width="5%">$i
						...
						<td><input type="text" class="readOnlyTxtShort" readonly="readonly"  name="serviceContract[serviceequ][$i][productNo]" id="productNo$i"  value="$val[productCode]"/>
			                <input type="hidden" name="serviceContract[serviceequ][$i][unitName]" id="unitName$i" value="$val[unitName]">
			                <input type="hidden" name="serviceContract[serviceequ][$i][isAdd]" id="isAdd$i" value="1" />
			            </td>
			            <td><input type="hidden" id="productId$i" name="serviceContract[serviceequ][$i][productId]" value="$val[id]"/>
			                <input type="text" name="serviceContract[serviceequ][$i][productName]" id="productName$i" class="readOnlyTxtMiddle" readonly="readonly"  value="$val[productName]"/>
			                &nbsp<img src="images/icon/icon105.gif" onclick="conInfo('productId$i');" title="查看配置信息"/>
		...
		foreach ($object as $key => $val) {
           ...
           if(!empty($val['isConfig'])){
					...
					<tr id="equTab_$i" parentRowId="$val[isCon]"><td width="5%">$i</td>
						<td><input type="text" name="serviceContract[serviceequ][$i][productNo]" id="productNo" class="readOnlyTxtShort" value="$val[productNo]"/>
			                <input type="hidden" name="serviceContract[serviceequ][$i][unitName]" id="unitName" value="$val[unitName]" /></td>
			            <td><input type="hidden" id="productId$i" name="serviceContract[serviceequ][$i][productId]" value="$val[productId]"/>
			                <input type="text" name="serviceContract[serviceequ][$i][productName]" id="productName" class="readOnlyTxtNormal"  value="$val[productName]"/>
			                &nbsp<img src="images/icon/icon105.gif" onclick="conInfo('productId$i',$i);" title="查看配置信息"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][productModel]"  id="productModel" class="readOnlyTxtShort" readonly="readonly" value="$val[productModel]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][number]" id="number$i" class="txtshort" value="$val[number]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][price]" id="price$i" class="txtshort formatMoney" onblur="FloatMul('number$i','price$i','money$i')" value="$val[price]"/></td>
                        <td><input type="text" name="serviceContract[serviceequ][$i][money]" id="money$i" class="txtshort formatMoney" value="$val[money]"/></td>
				        <td nowrap width="8%"><input type="text" class="txtshort" name="serviceContract[serviceequ][$i][warrantyPeriod]" id="warrantyPeriod$i" value="$val[warrantyPeriod]" /></td>
                        <td><input type="hidden" id="licenseId$i" name="serviceContract[serviceequ][$i][license]" value="$val[license]"/>
		 			        <input type="button" class="txt_btn_a" value="配置" onclick="License('licenseId$i','edit');" /></td>
		 			    <td width="4%"><input type="checkbox" name="serviceContract[serviceequ][$i][isSell]" id="isSell$i" $checked/>
		 			        <input type="hidden" name="serviceContract[serviceequ][$i][isCon]" id="isCon$i" value="$val[isCon]">
				            <input type="hidden" name="serviceContract[serviceequ][$i][isConfig]" id="isConfig$i" value="$val[isConfig]">
				            <input type="hidden" name="serviceContract[serviceequ][$i][remark]" id="remark$i" value="$val[remark]"></td>
			            <td><img src="images/closeDiv.gif" onclick="mydel(this,'invbody')" title="删除行" id="Del$i"/></td>
					</tr>
EOT;
           }else{
					...
					<tr id="equTab_$i" ><td width="5%">$i</td>
						<td><input type="text" name="serviceContract[serviceequ][$i][productNo]" id="productNo$i" class="txtmiddle" value="$val[productNo]"/>
			                <input type="hidden" name="serviceContract[serviceequ][$i][unitName]" id="unitName" value="$val[unitName]" /></td>
			            <td><input type="hidden" id="productId$i" name="serviceContract[serviceequ][$i][productId]" value="$val[productId]"/>
			                <input type="text" name="serviceContract[serviceequ][$i][productName]" id="productName$i" class="txt"  value="$val[productName]"/>
			                &nbsp<img src="images/icon/icon105.gif" onclick="conInfo('productId$i',$i);" title="查看配置信息"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][productModel]"  id="productModel$i" class="txtmiddle" readonly="readonly" value="$val[productModel]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][number]" id="number$i" class="txtshort" value="$val[number]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][price]" id="price$i" class="txtshort formatMoney" onblur="FloatMul('number$i','price$i','money$i')" value="$val[price]"/></td>
                        <td><input type="text" name="serviceContract[serviceequ][$i][money]" id="money$i" class="txtshort formatMoney" value="$val[money]"/></td>
				        <td nowrap width="8%"><input type="text" class="txtshort" name="serviceContract[serviceequ][$i][warrantyPeriod]" id="warrantyPeriod$i" value="$val[warrantyPeriod]" /></td>
		...
		foreach ($object as $key => $val) {
			...
			if(!empty($val['isConfig'])){
					...
					<tr id="equTab_$i" parentRowId="$val[isCon]"><td width="5%">$i</td>
						<td><input type="text" name="serviceContract[serviceequ][$i][productNo]" id="productNo" class="readOnlyTxtShort" value="$val[productNo]"/>
			                <input type="hidden" name="serviceContract[serviceequ][$i][unitName]" id="unitName" value="$val[unitName]" /></td>
			            <td><input type="hidden" id="isAdd$i" name="serviceContract[serviceequ][$i][proId]" value="$val[id]">
			                <input type="hidden" id="isDel$i" value="$val[productId]" />
			                <input type="hidden" id="productId$i" name="serviceContract[serviceequ][$i][productId]" value="$val[productId]"/>
			                <input type="text" name="serviceContract[serviceequ][$i][productName]" id="productName" class="readOnlyTxtNormal"  value="$val[productName]"/>
			                &nbsp<img src="images/icon/icon105.gif" onclick="conInfo('productId$i',$i);" title="查看配置信息"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][productModel]"  id="productModel" class="readOnlyTxtShort" readonly="readonly" value="$val[productModel]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][number]" id="number$i" class="txtshort" value="$val[number]"/></td>
...
EOT;
           				...
           				$str .=<<<EOT
					<tr id="equTab_$i" ><td width="5%">$i</td>
						<td><input type="text" name="serviceContract[serviceequ][$i][productNo]" id="" class="readOnlyTxtShort" value="$val[productNo]"/>
			                <input type="hidden" name="serviceContract[serviceequ][$i][unitName]" id="unitName" value="$val[unitName]" /></td>
			            <td><input type="hidden" id="isAdd$i" name="serviceContract[serviceequ][$i][proId]" value="$val[id]">
			                <input type="hidden" id="isDel$i" value="$val[productId]" />
			                <input type="hidden" id="productId$i" name="serviceContract[serviceequ][$i][productId]" value="$val[productId]"/>
			                <input type="text" name="serviceContract[serviceequ][$i][productName]" id="" class="readOnlyTxtNormal"  value="$val[productName]"/>
			                &nbsp<img src="images/icon/icon105.gif" onclick="conInfo('productId$i',$i);" title="查看配置信息"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][productModel]"  id="productModel$i" class="readOnlyTxtNormal" readonly="readonly" value="$val[productModel]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][number]" id="number$i" class="txtshort" value="$val[number]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][price]" id="price$i" class="txtshort formatMoney" onblur="FloatMul('number$i','price$i','money$i')" value="$val[price]"/></td>
                        <td><input type="text" name="serviceContract[serviceequ][$i][money]" id="money$i" class="txtshort formatMoney" value="$val[money]"/></td>
				        <td nowrap width="8%"><input type="text" class="txtshort" name="serviceContract[serviceequ][$i][warrantyPeriod]" id="warrantyPeriod$i" value="$val[warrantyPeriod]" /></td>
	...
	function initTableChange($object) {
		...
		foreach ($object as $key => $val) {
			...
			if(empty($val['originalId'])){
				$str.='<input type="hidden" name="serviceContract[serviceequ]['.$i.'][oldId]" value="'.$val['id'].'" />';
			}else{
				$str.='<input type="hidden" name="serviceContract[serviceequ]['.$i.'][oldId]" value="'.$val['originalId'].'" />';
			}
			if(!empty($val['isConfig'])){
                 $str .=<<<EOT
					<tr><td width="5%">$i</td>
						<td><input type="text" name="serviceContract[serviceequ][$i][productNo]" id="" class="readOnlyTxtShort" readonly="readonly" value="$val[productNo]"/>
			                <input type="hidden" name="serviceContract[serviceequ][$i][unitName]" id="unitName$i" value="$val[unitName]" /></td>
			            <td><input type="hidden" id="productId$i" name="serviceContract[serviceequ][$i][productId]" value="$val[productId]"/>
			                <input type="text" name="serviceContract[serviceequ][$i][productName]" id="" class="readOnlyTxtShort" readonly="readonly" value="$val[productName]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][productModel]"  id="productModel$i" class="readOnlyTxtShort" readonly="readonly" value="$val[productModel]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][number]" id="number$i" class="txtshort" value="$val[number]" onblur="FloatMul('number$i','price$i','money$i')"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][price]" id="price$i" class="txtshort formatMoney" onblur="FloatMul('number$i','price$i','money$i')" value="$val[price]"/></td>
                        <td><input type="text" name="serviceContract[serviceequ][$i][money]" id="money$i" class="txtshort formatMoney" value="$val[money]" onblur="FloatMul('number$i','price$i','money$i')"/></td>
				        <td nowrap width="8%"><input type="text" class="txtshort" name="serviceContract[serviceequ][$i][warrantyPeriod]" id="warrantyPeriod$i" value="$val[warrantyPeriod]" /></td>
                        <td><input type="hidden" id="licenseId$i" name="serviceContract[serviceequ][$i][license]" value="$val[license]"/>
		 			        <input type="button" class="txt_btn_a" value="配置" onclick="License('licenseId$i','edit');" />
		 			        <input type="hidden" name="serviceContract[serviceequ][$i][isCon]" id="isCon$i" value="$val[isCon]">
				            <input type="hidden" name="serviceContract[serviceequ][$i][isConfig]" id="isConfig$i" value="$val[isConfig]"></td>
				        <td width="4%"><input type="checkbox" name="serviceContract[serviceequ][$i][isSell]" id="isSell$i" $checked/></td>
		 			    <td><img src="images/closeDiv.gif" onclick="mydel(this,'invbody','serviceequ')" title="删除行" id="Del$i"/>
			            </td>
					</tr>
EOT;
			}else{
				$str .=<<<EOT
					<tr><td width="5%">$i</td>
						<td><input type="text" name="serviceContract[serviceequ][$i][productNo]" id="" class="readOnlyTxtMiddle" readonly="readonly" value="$val[productNo]"/>
			                <input type="hidden" name="serviceContract[serviceequ][$i][unitName]" id="unitName$i" value="$val[unitName]" /></td>
			            <td><input type="hidden" id="productId$i" name="serviceContract[serviceequ][$i][productId]" value="$val[productId]"/>
			                <input type="text" name="serviceContract[serviceequ][$i][productName]" id="" class="readOnlyTxtMiddle" readonly="readonly" value="$val[productName]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][productModel]"  id="productModel$i" class="readOnlyTxtMiddle" readonly="readonly" value="$val[productModel]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][number]" id="number$i" class="txtshort" value="$val[number]" onblur="FloatMul('number$i','price$i','money$i')"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][price]" id="price$i" class="txtshort formatMoney" onblur="FloatMul('number$i','price$i','money$i')" value="$val[price]"/></td>
                        <td><input type="text" name="serviceContract[serviceequ][$i][money]" id="money$i" class="txtshort formatMoney" value="$val[money]" onblur="FloatMul('number$i','price$i','money$i')"/></td>
				        <td nowrap width="8%"><input type="text" class="txtshort" name="serviceContract[serviceequ][$i][warrantyPeriod]" id="warrantyPeriod$i" value="$val[warrantyPeriod]" /></td>
...
EOT;
 ...
 if(!empty($val['isConfig'])){
                 			$str .=<<<EOT
					<tr id="equTab_$i" parentRowId="$val[isCon]"><td width="5%">$i</td>
						<td><input type="text" name="serviceContract[serviceequ][$i][productNo]" id="productNo" class="readOnlyTxtShort" value="$val[productNumber]"/>
			                <input type="hidden" name="serviceContract[serviceequ][$i][unitName]" id="unitName" value="$val[unitName]" /></td>
			            <td><input type="hidden" id="productId$i" name="serviceContract[serviceequ][$i][productId]" value="$val[productId]"/>
			                <input type="text" name="serviceContract[serviceequ][$i][productName]" id="productName" class="readOnlyTxtNormal"  value="$val[productName]"/>
			                &nbsp<img src="images/icon/icon105.gif" onclick="conInfo('productId$i');" title="查看配置信息"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][productModel]"  id="productModel" class="readOnlyTxtShort" readonly="readonly" value="$val[productModel]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][number]" id="number$i" class="txtshort" value="$val[amount]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][price]" id="price$i" class="txtshort formatMoney" onblur="FloatMul('number$i','price$i','money$i')" value="$val[price]"/></td>
                        <td><input type="text" name="serviceContract[serviceequ][$i][money]" id="money$i" class="txtshort formatMoney" value="$val[money]"/></td>
				        <td nowrap width="8%"><input type="text" class="txtshort" name="serviceContract[serviceequ][$i][warrantyPeriod]" id="warrantyPeriod$i" value="$val[warrantyPeriod]" /></td>
                        <td><input type="hidden" id="licenseId$i" name="serviceContract[serviceequ][$i][license]" value="$val[license]"/>
...
EOT;
           				...
           				$str .=<<<EOT
					<tr id="equTab_$i" ><td width="5%">$i</td>
						<td><input type="text" name="serviceContract[serviceequ][$i][productNo]" id="productNo$i" class="txtmiddle" value="$val[productNumber]"/>
			                <input type="hidden" name="serviceContract[serviceequ][$i][unitName]" id="unitName" value="$val[unitName]" /></td>
			            <td><input type="hidden" id="productId$i" name="serviceContract[serviceequ][$i][productId]" value="$val[productId]"/>
			                <input type="text" name="serviceContract[serviceequ][$i][productName]" id="productName$i" class="txt"  value="$val[productName]"/>
			                &nbsp<img src="images/icon/icon105.gif" onclick="conInfo('productId$i');" title="查看配置信息"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][productModel]"  id="productModel$i" class="txtshort" readonly="readonly" value="$val[productModel]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][number]" id="number$i" class="txtshort" value="$val[amount]"/></td>
			            <td><input type="text" name="serviceContract[serviceequ][$i][price]" id="price$i" class="txtshort formatMoney" onblur="FloatMul('number$i','price$i','money$i')" value="$val[price]"/></td>
                        <td><input type="text" name="serviceContract[serviceequ][$i][money]" id="money$i" class="txtshort formatMoney" value="$val[money]"/></td>
				        <td nowrap width="8%"><input type="text" class="txtshort" name="serviceContract[serviceequ][$i][warrantyPeriod]" id="warrantyPeriod$i" value="$val[warrantyPeriod]" /></td>
			...
			foreach ($rows as $key => $val) {
				...
				$str .=<<<EOT
						...
						<input type="hidden" id="productId$i" value="$val[productId]" />
							...
							<input type="hidden" equId="$equId" proId="$proId" value="oa_sale_service" name="lock[$i][objType]"/></td>
