<?php
class model_engineering_projecttype_projecttype extends model_base{
	function __construct(){
		$this->tbl_name = "oa_rd_projecttype";
		$this->sql_map = "engineering/projecttype/projecttypeSql.php";
		parent::__construct();
	}
		...
		if($rows){
			...
			foreach($rows as $key=>$val){
				...
				<tr class="$classCss" id="tr_$val[id]">
					...
					<td>
						<a href="?model=engineering_projecttype_projecttype&action=toEditProjectType&id=$val[id]&placeValuesBefore&TB_iframe=true&modal=false&height=195&width=750" class="thickbox">编辑</a>
					</td>
EOT;
			}
