 ...
 */
class model_engineering_template_rdprjtemplate extends model_base{
	/**
	 * 构造函数
	 */
	function __construct(){
		$this->tbl_name = "oa_rd_milestoneplanTemplate";
		$this->sql_map = "engineering/template/rdprjtemplateSql.php";
		parent::__construct();
	}
	/** ------------------------------以下为页面模板显示调用方法------------------------------------------*/
			...
			foreach($rows as $key=>$val){
				...
				$str .=<<<EOT
					<tr class="$classCss" name="tr_$val[id]">
						...
						<td>$i</td>
						<td>
							<a href="?model=engineering_template_rdprjtemplate&action=toviewtemplate&id=$templateId">
		...
		if(is_array($rows)){
			foreach($rows as $key=>$val){
				...
				<tr class="$classCss" id="tr_$val[id]">
					...
					<td>
						<a href="?model=engineering_baseinfo_rdmilestoneinfo&action=toEdit&id=$val[id]&parentId=$val[parentId]&placeValuesBefore&TB_iframe=true&modal=false&height=195&width=750" class="thickbox">
						编辑
						</a>
					</td>
				</tr>
...
//			print($tempObj);
			...
			//外键链接的关联关系
			$milestoneDao = new model_engineering_baseinfo_rdmilestoneinfo();
