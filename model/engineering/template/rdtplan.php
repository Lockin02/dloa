 ...
 */
class model_engineering_template_rdtplan extends model_base{
	function __construct(){
		$this->tbl_name = "oa_rd_template_plan";
		$this->sql_map = "engineering/template/rdtplanSql.php";
		parent::__construct ();
	}
			...
			foreach($rows as $key => $val){
				...
				$str.=<<<EOT
					<tr class="$classCss" title="$val[id]" id="$i">
						<td>
							$i<input type="hidden" id="status$i" value="$val[status]" />
						</td>
						<td>
							<a href="?model=engineering_template_rdtnode&action=readPlanTemplate&id=$val[id]">$val[templateName]</a>
						</td>
						<td>
							$status
						</td>
...
EOT;
	...
	function add_d($object){
		...
		$tknodeDao = new model_engineering_task_tknode();
		$rows = $tknodeDao->getNodeTkInPlan_d($object['planId']);

		$rdtnodeDao = new model_engineering_template_rdtnode();
		$rdttaskDao = new model_engineering_template_rdttask();
		$taskRows = array();
		$nodeRows = array();
		$nodeLeftKey = array();
		$idArray = array();
	...
	function templateImport($id,$projectId,$projectName,$planId,$planName){
		...
		$templateNodeDao = new model_engineering_template_rdtnode();
		$tNodeRows = $templateNodeDao->getNodes($id);

		$templateTaskDao = new model_engineering_template_rdttask();
