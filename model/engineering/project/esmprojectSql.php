...
$sql_arr = array (
  ...
  "select_esm_fee"=>"select c.id,c.projectCode,c.projectName,c.contractId,c.contractCode,c.contractType,
    ...
    "select_defaultAndFee"=>"select * @FROM
        (
            select c.id,c.id as proId,c.projectCode,c.projectName,c.contractId,c.contractCode,c.contractType,
                ...
                if(e.feeEqu is null,c.feeEqu,c.feeEqu + e.feeEqu) as feeEqu,c.feePayables,c.feeFlights,c.feeSubsidy,c.feeSubsidyImport,
                c.feeAllProcess,c.feeFieldProcess,c.projectProcess,c.earnedValue,c.perEarnedValue,c.maxLogDay,c.exgross,
                'esm' as pType,c.incomeType,c.incomeTypeName,'' as proLineMoney,'' as deliverySchedule,null as revenue,null as shipCost ,cp.state 
            from
                oa_esm_project c
                left join
                (SELECT projectId,SUM(fee) AS feeEqu FROM oa_esm_resource_fee GROUP BY projectId) e on e.projectId = c.id
                LEFT JOIN oa_contract_contract cp ON c.contractId = cp.id
            where 1
            union ALL
            select
               ...
               null as description,null as remark,
               if(p.proStatus = '关闭','GCXMZT03',if(p.proStatus = '在建','GCXMZT02',if(p.proStatus = '完工','GCXMZT04',''))) AS STATUS,
               p.proStatus AS statusName,
               ...
               null as customerLinkMan,null as salesmanaId,null as salesman,null as rdUser,null as rdUserId,cp.beginDate as planBeginDate,cp.endDate as planEndDate,p.actBeginDate,p.actEndDate,null as expectedDuration,
               ...
               null as maxLogDay,p.exgross,'pro' as pType,'' AS incomeType,p.earningsTypeName AS incomeTypeName,p.proLineMoney,p.deliverySchedule,p.revenue,p.shipCost,cp.state 
    ...
    "select_defaultAndFeeForUpload"=>"
        ...
        where c.contractType != 'GCXMYD-04'  
    "
);
$condition_arr = array (
  array(
