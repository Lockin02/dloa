 ...
 */
class model_produce_protask_strategy_serviceprotask implements taskstrategy {
...
//					. " and productId=" . $rows[$key]['productId'] . " and relDocType='oa_sale_service'";
...
-->						<td>
						...
						<td>
							...
							<input type="hidden" id="planNum$j" value='$protaskNum' class="txtmiddle"/>
						</td>
						<td>
	...
	function showItemEdit($rows) {
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				$j = $i + 1;
				//合同数量
				$contNumSql = " select number as contNum,issuedProNum from oa_service_equ where orderId=" . $rows[$key]['relDocId']
	...
	function showItemView($rows) {
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				$j = $i + 1;
				//合同数量
				$contNumSql = " select sum(number) as contNum from oa_service_equ where orderId="  . $rows[$key]['relDocId']
					. " and id=" . $rows[$key]['contEquId'];
				//计划数量
...
EOT;
	...
	function viewRelInfo( $paramArr,$skey ) {
		$str .= ' <img src="images\icon\view.gif" onclick="showOpenWin(\'?model=engineering_serviceContract_serviceContract&action=toViewTab&id='.$paramArr['relDocId'].'&perm=view&skey='.$skey.'\')">';
		return $str;
	}
	...
	function dealRelInfoAtAdd($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			$sql = " update oa_service_equ set issuedProNum = issuedProNum+" . $relItemArr[$key]['number'] . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['relDocId'];
	...
	function dealRelInfoAtEdit($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			...
			$sql = " update oa_service_equ set issuedProNum = " . $issuedShipNum . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['relDocId'];
	 ...
	 */
	 function getDocInfo( $id ){
		...
		if( trim($rows['orderCode']) == '' ){
			$rows['relDocCode'] = $rows['orderTempCode'];
			$rows['orderequ'] = $rows['serviceequ'];
			unset( $rows['serviceequ'] );
		}else{
			$rows['relDocCode'] = $rows['orderCode'];
			$rows['orderequ'] = $rows['serviceequ'];
			unset( $rows['serviceequ'] );
		}
		$rows['relDocName'] = $rows['orderName'];
