 ...
 */
class model_produce_protask_strategy_salesprotask implements taskstrategy {
	...
	function showItemEdit($rows) {
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				$j = $i + 1;
				//合同数量
				$contNumSql = " select number as contNum,issuedProNum from oa_sale_order_equ where orderId=" . $rows[$key]['relDocId']
	...
	function showItemView($rows) {
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				$j = $i + 1;
				//合同数量
				$contNumSql = " select sum(number) as contNum from oa_sale_order_equ where orderId=" . $rows[$key]['relDocId']
					. " and id=" . $rows[$key]['contEquId'];
				//计划数量
...
EOT;
	...
	function viewRelInfo( $paramArr,$skey ) {
		$str .= ' <img src="images\icon\view.gif" onclick="showOpenWin(\'?model=projectmanagent_order_order&action=toViewTab&id=' . $paramArr['relDocId'].'&perm=view&skey='.$skey.'\')">';
		return $str;
	}
	...
	function dealRelInfoAtAdd($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			$sql = " update oa_sale_order_equ set issuedProNum = issuedProNum+" . $relItemArr[$key]['number'] . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['relDocId'];
	...
	function dealRelInfoAtEdit($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			...
			$sql = " update oa_sale_order_equ set issuedProNum = " . $issuedShipNum . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['relDocId'];
