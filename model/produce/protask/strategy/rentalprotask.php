 ...
 */
class model_produce_protask_strategy_rentalprotask implements taskstrategy {
...
//					. " and productId=" . $rows[$key]['productId'] . " and relDocType='oa_sale_lease'";
	...
	function showItemEdit($rows) {
		...
		if (is_array ( $rows )) {
			...
			foreach ( $rows as $key => $val ) {
				$j = $i + 1;
				//合同数量
				$contNumSql = " select number as contNum,issuedProNum from oa_lease_equ where orderId=" . $rows[$key]['relDocId']
...
EOT;
	...
	function viewRelInfo( $paramArr,$skey ) {
		$str .= ' <img src="images\icon\view.gif" onclick="showOpenWin(\'?model=contract_rental_rentalcontract&action=toViewTab&id='.$paramArr['relDocId'].'&perm=view&skey='.$skey.'\')">';
		return $str;
	}
	...
	function dealRelInfoAtAdd($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			$sql = " update oa_lease_equ set issuedProNum = issuedProNum+" . $relItemArr[$key]['number'] . " where productId=" . $relItemArr[$key]['productId'] . " and orderId=" . $paramArr['relDocId'];
	...
	function dealRelInfoAtEdit($paramArr = false, $relItemArr = false) {
		...
		foreach ( $relItemArr as $key=>$vqal ){
			...
			$sql = " update oa_lease_equ set issuedProNum = " . $issuedShipNum . " where id=" . $relItemArr[$key]['contEquId'] . " and orderId=" . $paramArr['relDocId'];
