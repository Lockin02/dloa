	...
	function add_d($object) {
		try {
			...
			if (is_array ( $object ['items'] )) {
				...
				$itemsObj = $logsetDetailDao->saveDelBatch ( $itemsArr );
				$this->commit_d ();
				return $id;
			} else {
	...
	function addObjLog($tableName, $pkValue, $obj ,$operationType = '添加') {
		...
		if (is_array ( $logsettingObj )) {
			...
			if (is_array ( $logsetDetailArr )) {
				...
				foreach ( $logsetDetailArr as $key => $logsetDetail ) {
					//array_push($columnNameArr,$logsetDetail['columnName']);
				...
				$contentStr = '<table class="table-border"  width="100%" border="0" cellpadding="0" cellspacing="1" id="itemtable">' . '<thead><tr><td colspan="5" >' . '<font color="red">&nbsp;&nbsp;->添加</font></td></tr><tr class="table-title"><th align="center">序号</th><th align="center">字段</th><th align="center">名称</th><th align="center">旧值</th><th align="center">新值</th></tr></thead><tbody id="itembody">';
				$seNum = 1;
				foreach ( $columnNameTextArr as $cKey => $cText ) {
					$contentStr .= '<tr class="table-content" align="center" ><td align="center">' . $seNum . '</td><td align="center">' . $cKey . '</td><td align="center">' . $cText . '</td><td> </td><td>' . $obj [$cKey] . '</td></tr>';
					$seNum ++;
				}
	...
	function deleteObjLog($tableName, $deleteObj , $operationType = '删除') {
		...
		if (is_array ( $logsettingObj )) {
			...
			if (is_array ( $logsetDetailArr )) {
				...
				$seNum = 1;
				foreach ( $columnNameTextArr as $cKey => $cText ) {
					$contentStr .= '<tr class="table-content" align="center" ><td align="center">' . $seNum . '</td><td align="center">' . $cKey . '</td><td align="center">' . $cText . '</td><td>' . $deleteObj [$cKey] . ' </td><td></td></tr>';
					$seNum ++;
				}
				$contentStr .= '</tbody></table>';
				$operationObj ['logContent'] = $contentStr;
