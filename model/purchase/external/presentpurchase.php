...
class model_purchase_external_presentpurchase extends planBasic {

	private $externalDao; //外部对象dao接口
	private $externalEquDao; //外部对象设备dao接口

	function __construct() {
		$this->externalDao = new model_projectmanagent_present_present();
		$this->externalEquDao = new model_projectmanagent_present_presentequ();
					...
					<tr><td>$j</td>
						...
						<td>
						 <select name="basic[testType]" id="testType$j" class="txtshort">
						  <option value="0" selected>全检</option>
						...
						<td>
							<input type="text" id="remark" name="basic[equipment][$j][remark]" class='txt'/>
								...
								<input type="hidden" id="purchType$j" name="basic[equipment][$j][purchType]" value="oa_present_present" />
						</td>
						<td>
...
EOT;
		...
		/*if($i==0){
			$str="<tr align='center'><td colspan='50'>暂无采购申请清单信息</td></tr>";
		}*/
