...
/**
 * @description: 固定资产采购入库model
 * @date 2010-12-17 下午04:28:25
 * @author oyzx
 * @version V1.0
 */
class model_purchase_external_assets extends planBasic {
	...
	function showAddList($equs,$mainRows) {
		//TODO:追加需求保存到采购计划关联表的信息，待固定资产模块完成后，再来补充
		$str.=<<<EOT
					<input type="hidden" name="basic[objAssId]" value="" />
					<input type="hidden" name="basic[objAssName]" value="固定资产采购" />
					<input type="hidden" name="basic[objAssType]" value="assets" />
					<input type="hidden" name="basic[objAssCode]" value="" />
					<input type="hidden" name="basic[equObjAssType]" value="assets_equ" />
					</td>
EOT;
		return $str;
	}
	...
	function dealInfoAtPlan ($paramArr){
		//TODO:对于固定资产的采购计划添加时的业务处理，有需求时可以在此添加
		return true;
	}
